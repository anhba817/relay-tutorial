...
```text
  api up on 41533
  gateway 1 up on 39271
  gateway 2 up on 45118
  ...
  dispatcher connected to ws://127.0.0.1:39271
  tuan connected to ws://127.0.0.1:45118
  tuan sent "B2, north ramp" and lost the socket before any ack
  tuan connected to ws://127.0.0.1:39271 (resuming)
...
**Những con số cổng sẽ không phải là mấy con đó.** Harness bind cổng 0 và để hệ điều hành cấp
phát, rồi đọc lại con số ấy từ chính dòng `listening` của mỗi child. Trước đây nó lấy cố định
4100 rồi đếm lên, khiến ba cổng nằm lọt vào dải mà map của một suite khác đã đăng ký cho riêng
mình — và tệ hơn, biến một teardown không chờ con cháu mình thành một vụ tranh cổng. Thứ đáng đọc
trong transcript là **thứ tự các dòng**, không phải các con số.

...
Query schema cap `limit` ở 200 của FR-MSG-09 và *refuse* mọi giá trị lớn hơn —
`.max(200)` trên một zod number là validation chứ không phải transform, nên một
client hỏi 500 nhận 400 kèm tên field:
...
Thêm một shape decision đáng có một câu: page cap **refuse** thay vì clamp.
Một client hỏi `limit=500` nhận 400 kèm `field: "limit"`, không phải một
trang 200 — vì clamp sẽ trả lời một câu hỏi caller không hề đặt, và con số
họ nhận lại sẽ âm thầm khác con số họ gửi đi. Một caller định lấy 500 là
đang có bug, và một refusal nêu tên field là cách họ tìm ra nó trong một
request thay vì trong một paging loop không kết thúc như họ tưởng.

Đoạn này nói ngược lại suốt hai phần. Nó lập luận cho clamp, mô tả clamp, và
nằm ngay trên đoạn code luôn luôn refuse — `.max(200)` là validation. Chương
4.8 mirror schema này cho một surface mới, chạy thử các bound, và phát hiện
chữ sai chứ không phải code sai.

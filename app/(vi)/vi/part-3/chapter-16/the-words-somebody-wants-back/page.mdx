...
export const metadata = {
  ...
  alternates: {
    canonical: "/vi/part-3/chapter-16/the-words-somebody-wants-back",
    languages: {
      en: "/part-3/chapter-16/the-words-somebody-wants-back",
      vi: "/vi/part-3/chapter-16/the-words-somebody-wants-back",
    },
  },
};
...
-// chapter). The outbox arrived in the outbox chapter and is at the bottom of this file.
...
@@ -409,16 +418,26 @@ describe("the listing's keyset survives a shared last_activity_at
...
@@ -449,6 +468,72 @@ describe("the listing's tombstone rule and its clamp", () => {
...
@@ -581,3 +666,447 @@ describe("the repository's own refusals", () => {
...
+
   it("forwards per-channel truncation so the client pages history instead (FR-RTM-04)", async () => {
     const fanout = stubFanout();
     harness = await boot(
@@ -988,6 +1103,55 @@ describe("the socket's limits", () => {
...
Bánh cóc độ phủ nổ cuối cùng, trên `messages.controller.ts`, và câu trả lời của nó là xoá đi hai
thứ thay vì viết kiểm thử cho chúng: một `?? "unknown"` trên trường tác giả của frame xoá, vốn
không thể chạm tới **và** sẽ đặt đúng chữ ấy lên đường truyền như tên một người, cùng ba bản sao
của cùng hai giá trị dự phòng ngữ cảnh, mỗi chỗ publish một bản. Nhánh kết thúc cao hơn mức
chương này nhận được, nên cái chốt được nâng lên.

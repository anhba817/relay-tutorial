...
<Figure caption="Danh sách mục tiêu đến từ chính router của application đang chạy, không từ một danh sách ai đó bảo trì. Một route tồn tại mà không khớp phân loại nào sẽ làm build fail, và tự gọi tên nó ra." chart={figDerivedTargets} />
...
<Figure caption="Một end-user token mang theo một environment, nên đợt tấn công thông thường áp dụng được. Một platform credential không mang environment nào — dispatcher phục vụ mọi tenant — nên câu hỏi không phải nó với tới environment nào, mà là nó được gọi route nào." chart={figWhoMayCall} />

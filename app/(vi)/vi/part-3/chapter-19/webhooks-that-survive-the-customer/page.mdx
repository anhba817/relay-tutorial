...
export const metadata = {
  ...
  description:
    "Webhook phải sống qua endpoint, secret và dispatcher của khách hàng. Chương này xây subscription, signing, schedule, delivery claim, retry và dead letter trên ba service.",
  alternates: {
...
Webhook là promise gửi event ra hệ thống khách hàng, không phải chỉ là HTTP call. Nó cần
subscription durable, secret có thể hiển thị một lần, retry schedule, signature và dead letter.
...
## Pattern ngừng áp dụng, hai lần

Webhook không thể dùng cùng pattern với realtime fan-out: customer endpoint có thể chậm, sai TLS, trả 500 hoặc biến mất nhiều ngày. Nó cũng không thể dùng synchronous HTTP request từ write path, vì message của một user không được chờ hệ thống của khách hàng.
...
## Endpoint, và secret phải có thể trả lại

Subscription lưu endpoint và secret theo cách service có thể dùng để ký delivery nhưng không trả lại secret thô trong listing. Secret chỉ được hiển thị khi tạo/rotate; sau đó API trả metadata đủ để operator biết subscription nào đang tồn tại.
...
## Migration, và surface ở trên nó

Schema cần subscription, delivery attempt, trạng thái schedule và index cho worker claim. API surface được xây trên migration theo thứ tự đó, để không có handler nào tự phát minh state transient thay cho record durable.
...
**URL phải là HTTPS và không được trỏ vào trong.** Endpoint là URL platform sẽ fetch thay
khách hàng từ chính network của nó; không check destination chính là server-side request
forgery. Loopback, private range và link-local address bị từ chối bằng validation error
khách hàng đọc được thay vì timeout không giải thích được.

**Năm endpoint mỗi environment.** Năm có tính arbitrary, nhưng “unbounded” cũng là một con
số và là số sai: expansion write một row cho từng endpoint match, nên mười nghìn endpoint
biến một event thành mười nghìn row trong một transaction.

**Secret chỉ được trả đúng một lần:** lúc creation và không bao giờ nữa. Column lưu
ciphertext; plaintext chỉ tồn tại đủ lâu để response serialize.

**Event type ngoài declared set bị từ chối, và set để check không phải set platform đang
emit.** Đây là hai list khác nhau; khoảng cách giữa chúng là subscription của customer.
`WEBHOOK_EVENT_TYPES` khai báo tám name FR-WHK-02 gọi tên; năm name `emitted: true`, ba name
`channel.created`, `user.connected`, `user.disconnected` là `false` vì spec đã publish nhưng
chưa có producer.

Check subscription theo năm emitted name sẽ từ chối `user.connected` của customer đọc docs và
subscribe một documented event. Customer không sai; platform đã đo được 741 stored row gọi tên
một event như thế. Vì vậy validation dùng tám declared name: name ngoài set là typo và nhận
`webhook_event_type_unknown` với accepted set trong message; declared name chưa producer vẫn
được lưu và đơn giản là chưa bao giờ fire.

Thứ nó thay thế còn tệ hơn refusal: typo từng được lưu, endpoint trả `201`, customer thấy
subscription trong list rồi không bao giờ nhận delivery vì không event nào match
`mesage.created`. Validation error đọc được tốt hơn subscription trông đúng mãi mãi.

<Why title="Tại sao dùng flag trên type thay vì hai list">

Hai constant—declared name và emitted name—là shape dễ rot: chúng chỉ khớp khi ai đó nhớ đổi
cả hai. Repo đã trả giá khi lint exemption list drift khỏi tree nó exempt. Một table có boolean
mỗi row không thể bất đồng với chính nó; ship producer chỉ flip `false` thành `true` trên line
đã gọi tên event, và compiler tìm mọi reader qua `satisfies Record<string, { emitted: boolean }>`.
...
## Ký đúng thứ thực sự gửi

Signature phải được tính trên bytes của body thực sự gửi, cùng timestamp và secret của subscription. Ký JSON trước khi serializer đổi whitespace hoặc encoding tạo verification failure khó đoán ở customer side.
...
## Schedule là một column, và lý do là measurement

`next_attempt_at` là column có index vì dispatcher cần tìm đúng delivery đến hạn mà không scan toàn bộ lịch sử. Measurement của query shape quyết định schema này; một worker loop không nên dựa vào bảng nhỏ trong test lane để biện minh cho full scan.
...
Một event có thể fan out thành nhiều delivery theo subscription. Expansion phải diễn ra trong hoặc sau claim có ownership rõ, để hai dispatcher không cùng tạo/gửi duplicate delivery khi cùng nhìn thấy event.
...
## Tại sao dispatcher là service riêng

Dispatcher có lifecycle, retry, credential và concurrency khác API/gateway. Tách service giữ latency của user path độc lập với customer endpoint và cho phép scale delivery worker theo backlog thay vì theo traffic chat.
...
## Seam, và credential đi qua nó

HTTP transport là seam injectable để tests điều khiển timeout, status và body. Credential cho internal dispatch đi qua seam có scope hẹp; không đưa tenant secret hay public credential vào một worker có quyền rộng hơn cần thiết.
...
Outbox relay chuyển fact đã commit thành công việc dispatcher có thể claim. Nó vẫn cần retry và observability như những relay trước: publish transient không được là nơi delivery contract biến mất.
...
Dispatcher claim một batch, gửi request có signature, rồi atomically ghi attempt và schedule lần sau. Worker crash giữa các bước là case thiết kế, nên claim có lease/timeout để công việc được nhận lại mà không bị kẹt vĩnh viễn.
...
Sau policy retry, delivery đi vào dead letter với reason và lịch sử attempt. Dead letter không phải thùng rác: nó là record để customer support, metric và eventual replay có owner rõ ràng.
...
## Một lượt rà tìm ra bug thật

Rà toàn bộ boundary thường lộ bug mà feature test không chạm: endpoint được canonicalize sai, signature dùng secret cũ sau rotation, hoặc query claim không scope đúng status. Những review này biến “đã chạy demo” thành một contract có thể vận hành.
...
## Suite thực sự giữ gì

Suite giữ các invariant: một event tạo đúng delivery, body ký khớp bytes gửi, retry không duplicate claim, và failure cuối cùng được lưu. Nó không cố giả lập internet; transport seam cung cấp failure có kiểm soát để test policy.
...
API, gateway và dispatcher là ba service với dependency/lifecycle riêng. Compose và test harness phải khởi động đúng container, health check và credential để integration test không vô tình chạy dispatcher trong process API.

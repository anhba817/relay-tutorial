...
<Figure caption="Response của `HttpException` là `unknown`, nên tám chỗ gọi tự gõ error code của mình thành một chuỗi trơn. Một lỗi chính tả compile được, ship được, rồi thành một URL." chart={figFourTypeGates} />
...
<Figure caption="Ba cấp, và cấp đầu không phải một rule: node_modules cô lập của pnpm không có thư mục @relay ở gốc workspace, nên import đó không resolve. Điều không cấp nào chặn được là đọc source bằng mắt, và đó là một kỷ luật chứ không phải một cơ chế." chart={figThreeLevels} />
...
<Figure caption="Đạt một phần. Phần đạt được thì đã đo; phần không đạt là hai thứ khác loại nhau, và chỉ một trong hai là khiếm khuyết mà ai đó có thể vá." chart={figVerdict} />
...
gửi qua REST không tới được socket và chương của FR-RTM-05 sở hữu quyết định ấy; còn

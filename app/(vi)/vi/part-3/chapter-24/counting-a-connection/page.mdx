...
export const metadata = {
  ...
  description:
    "Connection-minute là quota dimension khác message. Chương này thêm usage service không sở hữu table, total-based metering, shutdown accounting và enforcement ở socket door.",
  alternates: {
...
Connection usage được tính theo phút kết nối, không phải số message. Counter phải cộng total
đã quan sát, xử lý socket shutdown và từ chối connection mới trước khi nó tiêu quota.

## Connection-minute là gì

## Service không sở hữu table, cũng không sở hữu identity

## Total, không phải delta
...
## API không thể nói gì, và không cần nói gì

## Dimension thứ ba tốn gì

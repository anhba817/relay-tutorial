...
export const metadata = {
  ...
  description:
    "Part 3 biến một chat backend đang hoạt động thành hạ tầng mà khách hàng có thể đăng ký: organisations, applications, environments, OAuth signup và tenant boundary tách biệt hai nhóm user.",
  alternates: {
...
Part 2 khép lại với một test suite xanh và một khẳng định có thể bảo vệ được: message
đến đúng một lần, đúng thứ tự, xuyên qua nhiều instance và qua một tunnel. Nhưng nó
chưa tạo ra một sản phẩm mà người khác có thể dùng. Mọi request trong Part 2 đều tự
khai tenant bằng một header do client gửi, còn mọi tenant đều do test helper tạo. Chưa
có cách nào để *trở thành* khách hàng.

Chương này xây phần còn thiếu đó: các container chứa thế giới của một khách hàng và
một hành động duy nhất tạo ra chúng. Nó cũng trả món nợ bạn đã thấy tôi vay. Chương
2.1 cần một row `applications` để thoả foreign key nhưng chưa có định nghĩa cho nó,
nên đã để lại đoạn sau:
...
Đây chính là chương xử lý việc đó, và stub sẽ biến mất.

<SkipAhead>

Nếu buộc phải bỏ qua: schema có thêm ba table phía trên tenant boundary —
`organisations`, `humans`, `memberships`; stub `applications` từ 2.1 trở thành table
thật và biết owner của nó; `environments` nhận `UNIQUE (application_id, kind)`. Kết
hợp với CHECK hiện có trên kind, constraint này *chính là* quy tắc “đúng hai
environment” của FR-TEN-04. Signup là OAuth authorization-code flow được viết trực
tiếp: `GET /auth/github/start` tạo `state` trong HttpOnly cookie;
`GET /auth/github/callback` xác minh ràng buộc đó trước khi gọi ra ngoài, rồi provision
organisation, application, environment `development` và membership `owner` trong một
transaction (FR-TEN-01, FR-TEN-02). Không thêm dependency. Chưa có session — session
thuộc dashboard ở Part 5, vì session tồn tại để một console sử dụng và hiện chưa có
console. Trạng thái cuối chương là tag **`part3-ch1`**.

</SkipAhead>

## Ba container — tại sao không phải hai?

SRS yêu cầu một cấu trúc cụ thể qua bốn requirement, dễ hiểu hơn nếu đọc thành một câu.
Một người tạo **organisation** bằng cách authenticate với provider mà không cần cung
cấp gì thêm (FR-TEN-01). Organisation có thể chứa nhiều **application**, mỗi application
có data, key và quota độc lập (FR-TEN-03). Mỗi application có đúng hai **environment**,
`development` và `production`, với credential, quota và datastore riêng (FR-TEN-04).
Việc tạo organisation tự động tạo một application cùng một environment `development`,
không cần input nào khác (FR-TEN-02).

Ba tầng nhiều hơn sơ đồ mà đa số mọi người sẽ vẽ một tầng, vì vậy cần hỏi mỗi tầng
*dùng để làm gì*. Organisation là nơi billing và quyền sở hữu của con người tồn tại —
nó đại diện cho một công ty. Application là nơi một *sản phẩm* tồn tại: công ty phát
hành hai app muốn hai bộ channel, hai bộ key nhưng chỉ một invoice. Environment là nơi
data tồn tại, bởi phát hành bất cứ thứ gì cũng cần một chỗ để làm hỏng nó trước.

Tầng cuối cùng chính là tầng bạn đã xây.

<Figure
  caption="Boundary mà chương này vạch ra. Phía trên, các row cho biết ai sở hữu account và không có tenant column. Phía dưới, mọi row đều mang environment_id và repository từ chối query nếu thiếu nó."
  code={figBoundary}
/>

<Why source="FR-TEN-04 · FR-TEN-06 · constitution I">

Tại sao **environment** là tenant thay vì application hay organisation? Vì đây là tầng
duy nhất nơi câu trả lời cho “request này có được thấy row này không?” chỉ là một giá
trị, và Part 2 đã chứng minh điều đó. Mọi table chương 2.1 viết đều mang
`environment_id`; mọi repository constructor đều bắt buộc giá trị này; isolation suite
chỉ tấn công column đó. Nếu tenant là application, `development` và `production` sẽ
dùng chung dataset và điều đầu tiên khách hàng làm với Relay sẽ là test trên production.
Nếu tenant là organisation, hai sản phẩm của cùng công ty sẽ dùng chung channel.
Requirement giới hạn hai environment mỗi application (FR-TEN-04) thực chất nói về
blast radius: bạn có đúng một nơi được phép bất cẩn, và nó tên là `development`.

</Why>

## Những table SAD chưa từng viết

SAD §6.1 định nghĩa chính xác bằng SQL cho `environments` và mọi thứ bên dưới. Nó không
định nghĩa `applications` hay bất cứ thứ gì phía trên — `environments` tham chiếu
`applications(id)` rồi tài liệu dừng lại. Vì vậy ba table dưới đây **được suy ra từ
SRS**, không phải trích từ SAD; schema nói rõ điều đó ngay nơi reader gặp chúng. Đây là
cùng cơ chế 2.1 dùng cho `members`: khi tài liệu hết, chương ghi lại một decision thay
vì trình bày một phát minh như specification.
...
`UNIQUE (application_id, kind)` trên `environments` **chính là** FR-TEN-04. CHECK cho
kind từ 2.1 đã giới hạn column còn hai giá trị hợp lệ; unique index trên
`(application_id, kind)` vì thế chỉ cho tối đa một row mỗi loại, tức tối đa hai
environment. Không trigger, không `SELECT count(*)` để request đồng thời có thể race
qua — rule là một shape, và shape không cần ai ghi nhớ.

`humans` cũng không có `environment_id`. Sự vắng mặt đó là dòng quan trọng nhất của
chương và có ADR riêng.

<Trap>

Đây là shape gần như ai cũng nghĩ tới đầu tiên, và cũng là shape âm thầm tháo tung
Part 2: **đặt những người signup vào table `users`.** Đã có sẵn users table, display
name và identifier trông như email. Thêm column `provider`, cho `environment_id` nhận
null với “platform humans” chỉ mất mười phút.

Rồi hãy nhìn điều vừa xảy ra với `environment_id`: giờ nó nullable. FR-TEN-06 nói mọi
operational record được persist phải mang tenant identifier non-null, bởi chương 2.1
đã xây repository whose constructors *bắt buộc* tenant — không thể biểu diễn query
thiếu tenant. Khi column có thể null, guarantee đó không còn mang tính cấu trúc. Hôm
nay chưa gì hỏng; isolation suite vẫn pass; nhưng requirement quan trọng nhất hệ thống
(FR-TEN-05: “không operation nào được trả về, sửa đổi hay để lộ data của tenant khác”)
âm thầm biến từ điều type system từ chối thành điều reviewer phải nhớ để ý.

Hai population, hai table, không row nào đi qua boundary. Bên dưới boundary, user là
end user của khách hàng, được nhận diện bằng `external_id` của khách hàng, scoped theo
environment và không bao giờ đăng nhập Relay. Phía trên, human là người có provider
account, có thể sở hữu nhiều organisation và hoàn toàn không thuộc tenant nào. Họ chỉ
chia sẻ từ “user”, ngoài ra không có gì chung. **ADR-18** ghi lại quyết định này vì mọi
chương sau đều thừa hưởng nó — audit trail, dashboard và isolation gauntlet đều phải
biết identifier thuộc population nào.

</Trap>

<Figure
  caption="ADR-18 trong một hình: shape rẻ hơn cần tenant column nullable, trong khi chính column này làm isolation có thể được enforce by construction."
  code={figTwoPopulations}
/>

## Migration không thể apply

Sau khi viết schema, drizzle-kit sinh SQL và — theo workflow của ADR-16 — ta đọc nó
trước khi chạy. Việc đầu tiên nó muốn làm là bất khả thi:
...
Statement đó không thể chạy thành công trên database đã có application row — và
database của mọi reader đều có, vì `createEnvironment` từ 2.1 đã tạo chúng suốt Part 2.
Database của tôi có 237 row. Một column NOT NULL không thể xuất hiện trên table đã có
data nếu không ai nói các row hiện tại phải chứa gì; generator không thể tự biết.

Vì vậy statement được sửa bằng tay theo shape quen thuộc của bài toán này: thêm column
nullable, backfill, rồi mới áp constraint.

Backfill tạo một organisation riêng cho mỗi application mồ côi và dùng lại UUID của
application làm UUID organisation. Nhờ đó lineage vẫn đọc được: organisation có id
trùng application là organisation do migration tạo. Sau khi apply, cả 237 application
đều có organisation và column đã NOT NULL — đó là bằng chứng duy nhất có ý nghĩa.

## Signup: một hành động, năm row

Provisioning thuộc *admin surface* của repository layer — function duy nhất trong file
này luôn được phép chạy mà không có tenant scope, vì chính operation đó tạo tenant.
Nó đã tồn tại và 2.1 đã giải thích: `createEnvironment` tạo application cùng environment.
Giờ nó tạo thêm organisation và có một sibling làm toàn bộ công việc signup.
...
Hãy để ý điều *không* xảy ra: không có cánh cửa thứ hai để tạo tenant. Principle I giữ
isolation tại một nơi, và “thứ tạo ra tenant” chính là loại operation không nên có thể
được gọi từ hai hướng.

<Why source="FR-TEN-01 · FR-TEN-07 · FR-MSG-04's shape">

Tại sao signup idempotent theo organisation mà người đó *sở hữu*, thay vì đơn giản
“theo identity”? Vì “trả về organisation hiện có” không được định nghĩa với human là
member của organisation người khác. FR-TEN-07 cho phép trạng thái đó — organisation có
member và role — nên ngay khi invitation tồn tại, một identity có thể đã được hệ thống
biết đến nhưng không sở hữu gì. Rule chỉ keyed theo identity khi ấy phải chọn một
organisation để trả về, và chọn cái nào cũng sai: trao workspace của người khác cho
người vừa signup là tenancy bug có vẻ ngoài thân thiện.

Vì vậy rule được phát biểu cho mọi trường hợp có thể đạt tới. Identity chưa biết nhận
năm row. Identity đã biết và có membership `owner` nhận lại organisation đó, không có
write nào. Identity đã biết nhưng chỉ có membership không phải `owner` nhận bốn row —
một organisation mới của riêng họ, không đụng tới membership hiện có. Trường hợp thứ
ba chỉ có thể xảy ra khi chương sau xây invitation, nhưng được ghi ngay bây giờ để
không phải phát hiện sự mơ hồ trong production cùng data thật. Cơ chế là bài học 2.3:
unique index quyết định, không phải read-then-write check mà hai cú click đầu tiên đồng
thời đều có thể vượt qua.

</Why>

<Figure
  caption="Toàn bộ flow. State được kiểm tra trước mọi provider call — callback chưa xác minh tuyệt đối không được khiến server gọi GitHub thay cho attacker."
  code={figSignup}
/>

## Tham số state không phải vật trang trí

Authorization-code flow có ba bước và được viết trực tiếp ở đây vì cùng lý do chương
2.5 tự nối WebSocket upgrade: strategy library sẽ che đúng phần bạn đến để học và thêm
dependency vào service mà constitution yêu cầu “boring by design”. `fetch` của Node
thực hiện hai HTTP call; zod validate cả hai response, vì payload không validate sẽ
không fail tại nơi nó đến — và ở đây payload quyết định *một người là ai*.
...
Tiếp theo là phần rất dễ sai theo cách khó nhận ra. `state` chứng minh callback thuộc
về browser đã bắt đầu flow. Ta dễ muốn làm nó stateless — ký một giá trị random, verify
signature khi quay lại, không cần cookie. Phiên bản đó verify rất đẹp nhưng không ngăn
được gì: attacker tự bắt đầu flow, lấy state hoàn toàn hợp lệ do server tạo rồi đưa URL
callback cho victim. Signature hợp lệ vì đúng là server đã tạo nó. Thứ browser của
victim không có chính là cookie.

Chọn `SameSite=Lax` thay vì `Strict`, vì redirect từ provider quay lại là cross-site
navigation và `Strict` sẽ giữ cookie lại đúng lúc cần nó. Dùng `Secure` ở mọi nơi trừ
base URL HTTP thường, vì reader chạy localhost nếu không sẽ chẳng bao giờ nhận cookie
và không có đầu mối khi flow fail.

Một nét riêng của provider đáng một dòng code và một câu giải thích: GitHub trả lời
code sai bằng **HTTP 200** kèm error object. Code chỉ parse success shape sẽ đọc
`access_token: undefined` như token rồi tiếp tục. Vì vậy error shape được parse trước,
và hai lỗi được phân biệt khi trả ra — người dùng từ chối là 400; provider trả dữ liệu
ngoài contract là 502 vì caller không làm gì sai.

Hãy nhìn thứ controller này *không* mang: `EnvironmentContextGuard` mà mọi controller
khác đã dùng từ 2.2. Hai route này tồn tại để xác lập một người là ai, khi chưa có
tenant nào cho header gọi tên. Guard được áp theo từng controller, nên pre-tenant route
đơn giản không dùng nó; seam mà guard bảo vệ không bị chương này động tới.

## Tự thiết lập

Các endpoint là configuration, không phải constant. Vì vậy test lane có thể trỏ tới
stand-in local còn deployment thật trỏ tới GitHub Enterprise. Để chạy flow thật, đăng
ký OAuth application với provider, đặt callback thành
`http://localhost:4000/auth/github/callback`, rồi export:
...
Nếu thiếu client id và secret, `/auth/github/start` trả 404 thay vì redirect vào flow
không thể hoàn tất. Quy tắc áp dụng cho `DATABASE_URL` từ 1.2 cũng áp dụng cho client
secret: chỉ là runtime configuration — không bao giờ là build argument, không bake vào
image layer và không xuất hiện trong client bundle.

## Những điều chương này chủ ý để lại cho sau

Ba thứ sản phẩm thật sẽ có, cùng chương chịu trách nhiệm cho chúng:

**Chưa có session.** Sau callback, bạn nhận JSON body mô tả những gì vừa tạo rồi server
quên bạn. Session là credential; credential là chủ đề của chương credentials (“hai
credential, một sai lầm”), còn dashboard sử dụng nó nằm ở Part 5. Bịa ra credential
thứ ba ở đây để chương credentials sửa lại ngay sau đó chính là điều Principle VII
ngăn cản.

**Chưa quản lý role.** Signup đặt human đang authenticate làm `owner`; vocabulary role
(`owner`, `admin`, `member`) tồn tại trong schema vì FR-TEN-07 gọi tên nó. Invitation,
promotion và removal chưa được xây.

**Các dev-mode seam vẫn còn.** Mọi surface từ Part 2 vẫn gọi tên tenant bằng header
`x-relay-environment`, gateway vẫn verify token bằng dev secret. Tenancy thật phía trên
không khiến chúng đáng tin hơn; chương credentials sẽ loại bỏ cả hai. Không có điều gì
trong chương này là lý do để tin header đó.

## Các test và điều chúng giữ vững

Bảy invariant được chia giữa các lane theo thứ chúng cần. Binding và provider contract
là pure function của input, nên chạy trong lane không Docker mà reader dùng sau mỗi
lần save:
...
Phần còn lại cần database thật, và một test cần provider — nên suite khởi động một
provider trên port thật, trả lời đúng điều test yêu cầu. Flow không bị stub; chỉ bên ở
đầu kia được thay thế.

Hai test trong số đó từng sai trước khi đúng, và lý do đáng chú ý. Cả hai đều giả định
identity mới — nhưng signup idempotent theo identity, còn database không bị truncate
giữa các lần chạy. Phiên bản đầu của returning-owner test assert `created: true` cho
identity mà test trước trong cùng file đã dùng; environment-cap test kế thừa application
mà *lần chạy suite trước* đã thêm production environment. Không lỗi nào là product bug;
đó là test đang assert lịch sử của suite thay vì behaviour. Cách sửa là bài học 2.1 ở
tầng cao hơn: cho mỗi test identity riêng, giống như mỗi integration suite có environment
riêng.

Tại tag này, các lane có **86 unit test** không cần Docker (config 6, service-kit 3,
protocol 26, API 18, gateway 33) và **60 integration test trong 11 file** — 44 test API
với Postgres, 8 test gateway với Redis, cùng 8 test journey từ 2.8 không thay đổi.

## Chạy thử

Walk không cần provider account: nó mở một stand-in trên port cố định và trỏ API vào đó.
...
> **ĐƯỢC SỬA bởi chương credentials.** Transcript này là output tại tag `part3-ch1`.
> Từ chương credentials, callback còn trao API key đầu tiên của environment — thêm một
> dòng `api key rk_dev_… (shown once)`, chỉ xuất hiện khi `created` là `true`. Không điều
> gì phía trên thay đổi; chỉ thêm field. Nếu bạn chạy HEAD thay vì tag của chương, đây
> là lý do có dòng đó.

Hãy đọc hai giá trị `created` cùng nhau. Một lần authentication đã tạo tenant; lần thứ
hai không tạo gì và trả lại đúng ba id đó. Dòng cuối cho thấy trap phía trên bị từ chối:
state thật mà không có cookie đứng sau thì không có giá trị.

<ForwardRef part="Part 3 and beyond">

Environment chương này tạo có `signing_secret` nhưng chưa có API key — đó là mở đầu của
chương credentials: key có prefix và hash, user token, dev-token endpoint (FR-AUT-09),
cùng việc loại bỏ hai dev-mode seam. Outbox ở chương outbox sẽ khiến mọi committed write
publish event, khép REST–fan-out gap mà 2.6 đã nêu. Quota ở chương retry-and-disable được
đếm theo application, đó là lý do tầng này tồn tại. Chương deduplication tấn công trực
tiếp boundary của chương này bằng cross-tenant gauntlet trên mọi endpoint và test external
developer thứ hai. Xa hơn, FR-DSH-01 muốn development API key trên màn hình đầu tiên sau
signup — việc đó cần key từ chương credentials và dashboard Part 5; chương này khiến
“màn hình đầu tiên sau signup” có thể tồn tại.

</ForwardRef>

## Đến lượt bạn

Bài tập chính là build. Sau đó buộc boundary tự chứng minh:

1. Thử cấp application một environment thứ ba trực tiếp từ `psql`. Quan sát database
   từ chối và để ý không có application code nào tham gia.
2. Thêm `environment_id` nullable vào `humans`, chạy lại isolation test. Chúng vẫn pass —
   đó chính là điểm của TRAP. Viết ra guarantee nào vừa bị chuyển từ schema vào trí nhớ
   của một người.
3. Hoàn thành signup, rồi tự tạo callback thứ hai với cùng `state` từ terminal khác và
   không có cookie. Kết quả phải là 400 trước mọi network call; xác nhận trong log API
   rằng provider chưa từng được gọi.
4. Signup hai lần bằng hai provider account khác nhau và xác nhận mỗi account nhận
   organisation, application, environment riêng; rồi dùng environment header của tenant
   B để đọc channel của tenant A và nhận empty answer mà FR-TEN-05 yêu cầu.

Nếu mắc kẹt, tag chứa đáp án: `part3-ch1`.

## Điều cần ghi nhớ

- **Ba container, mỗi cái một việc**: organisation cho billing và ownership,
  application là sản phẩm, environment là tenant — tầng duy nhất nơi isolation chỉ là
  một column (FR-TEN-03/04).
- **Hai population, không bao giờ gộp** (ADR-18): single-table shape rẻ hơn cần tenant
  column nullable, và chính column đó biến structural guarantee thành review comment.
- **Unique index có thể chính là requirement**: `UNIQUE (application_id, kind)` cùng
  CHECK hai giá trị *chính là* FR-TEN-04; không race nào vượt qua được.
- **Migration do generator tạo vẫn phải được đọc**: `ADD COLUMN … NOT NULL` không thể
  apply lên table có data, và generator không biết row hiện tại phải chứa gì.
- **`state` cần cookie, không phải signature**: giá trị server tạo chỉ chứng minh server
  đã tạo nó, không chứng minh browser này đã yêu cầu.
- **Signup trao workspace của chính bạn**: idempotency keyed theo organisation được sở
  hữu được định nghĩa cho mọi trường hợp, kể cả trường hợp invitation sẽ mở ra.

<Checkpoint>

Trước khi bắt đầu chương credentials, bạn cần có: ba tenancy table và stub
`applications` đã biến mất; `UNIQUE (application_id, kind)` từ chối environment thứ ba;
signup provision organisation, application, environment `development` và membership
`owner` trong một transaction, idempotent theo identity; state binding từ chối callback
không cookie trước mọi provider call; ADR-18 đã được ghi; hai lane xanh với 86 unit test
và 60 integration test. Trạng thái đó là tag `part3-ch1`. Chương credentials sẽ tháo
các dev-mode seam bên dưới và thay chúng bằng credential có ý nghĩa.

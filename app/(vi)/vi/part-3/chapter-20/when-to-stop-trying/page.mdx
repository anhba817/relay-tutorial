...
export const metadata = {
  ...
  description:
    "Webhook retry cần biết khi nào dừng. Chương này thêm failure stream, policy có đo lường, disable idempotent, notification và observability cho delivery đã cạn retry.",
  alternates: {
...
Retry không phải lời hứa thử mãi. Mỗi failure phải thành record trước, policy quyết định lần
thử sau, và endpoint được disable đúng một lần khi không còn delivery hữu ích.

## Record phải có trước

Một delivery thất bại phải có record durable trước khi policy quyết định retry hay dừng. Nếu failure chỉ tồn tại trong log của worker chết, không ai có thể biết customer đã nhận gì, đã thử bao nhiêu lần hay có cần thông báo không.

## Seam đã mang nó sẵn

Transport seam từ chương webhook đã trả status, timeout và error đủ để phân loại failure. Chương này dùng seam đó thay vì tạo đường retry song song, giữ tất cả delivery outcome đi qua cùng một nơi.
...
Failure stream tách khỏi event và delivery stream vì audience của nó là policy/observability, không phải customer endpoint. Naming stream rõ ràng giúp worker biết event nào cần retry và event nào chỉ để đo lường.
...
## Publish được phép fail, và chương phải nói rõ

Publish failure không thể được nuốt; record vẫn phải còn claimable hoặc worker phải trả nó về trạng thái retry. Contract nói rõ điều này để một outage của stream không âm thầm biến thành delivery bị mất.
...
## Publish đi đâu, chính xác

Failure event mang delivery id và reason tới subject/consumer dành cho retry policy, không broadcast lại payload webhook một cách vô tình. Route chính xác giữ secret-bearing data trong boundary cần thiết.
...
## Giờ là quyết định — và measurement làm nó thay đổi

Backoff và số lần retry là policy dựa trên cost, customer experience và backlog measurement. Một con số hợp lý ở lane có thể làm queue production phình to, nên decision phải gắn với metric và có đường điều chỉnh.
...
## Failure run là hai column

`failure_count` và `next_attempt_at` mô tả run retry: một cho policy biết đã thử bao nhiêu, một cho scheduler biết khi nào thử tiếp. Không suy ra chúng từ log hay timestamps rải rác; worker cần query shape đơn giản, indexed và atomic.
...
## Policy, không có gì khác trong đó

Policy chỉ nhận outcome và attempt count rồi trả retry time hoặc terminal decision. Giữ nó thuần giúp test boundary — không để policy tự gửi HTTP, tự update database hoặc tự disable subscription.
...
## Disabling, nhiều nhất một lần

Terminal failure có thể disable subscription, nhưng action này phải idempotent. Nhiều worker/retry nhìn cùng record không được gửi nhiều notification hay thay đổi state qua lại; conditional update tạo đúng một winner.
...
## Sweep đi nhờ loop đã tồn tại

Scheduler quét delivery đến hạn trong loop dispatcher đã có thay vì thêm cron process cạnh tranh claim. Một owner cho queue giúp concurrency, shutdown và metric dễ hiểu hơn.
...
## Notification không ai nhận

Disable notification được tạo như work durable, không gọi email đồng bộ trong worker. Customer có thể không nhận email; điều hệ thống hứa là notification đã được schedule/audit, không hứa inbox của bên thứ ba.
...
## Chứng minh nó hoạt động lại

Khi endpoint hồi phục trước terminal limit, delivery phải được gửi thành công, failure count reset/được ghi đúng và subscription không bị disable. Recovery test quan trọng như exhaustion test; retry system chỉ chứng minh giá trị nếu nó biết dừng thử khi đã thành công.
...
Metric cần phân biệt retry scheduled, delivery succeeded, terminal failure và subscription disabled. Không có các counter/trace này, operator chỉ thấy customer than phiền mà không biết queue đang chậm, transport lỗi hay policy đã dừng.
...
## Stream, và một event ngoài nó

Một số event như notification có lifecycle riêng và không nên bị consumer retry hiểu nhầm là webhook delivery. Tách vocabulary ngăn policy reprocess công việc không thuộc nó.
...
Suite kiểm tra transition durable qua mọi arm: transient failure, retry, success, terminal disable và notification. Mỗi test quan sát row và event, không chỉ mock function đã được gọi.
...
Sabotage cases cố ý làm publish, claim, transport và notification fail để kiểm tra không có delivery nào kẹt hoặc bị đánh dấu thành công giả. Đây là nơi retry design được chứng minh, không phải ở happy path một lần 200.

...
export const metadata = {
  ...
  description:
    "channels.type đã là một cột hai giá trị từ chương 2.1 và chẳng quyết định nào tra tới nó. Làm cho giá trị thứ hai có ý nghĩa đòi hỏi một route chưa bao giờ được dựng, một phép kiểm mà lệnh gửi công khai duy nhất không với tới được, và một thứ tự từ chối làm lộ ra chính thứ nó từ chối.",
  alternates: {
...
Hai mươi hai chương sau, cột đó vẫn giữ `'public'` trong mọi dòng của mọi
environment. Chương trước dựng `POST /v1/channels` rồi cho cái field một
enum một-giá-trị với lần sửa sắc sảo nhất trong đó — `z.enum(["public"])` — dựa trên
lý do rằng một endpoint chấp nhận `private` sẽ bán một bảo đảm mà nền tảng
không giữ.

Đó là quyết định đúng và cái lý do được nêu thì hơi sai, theo một kiểu đáng
đem ra mở màn. Tuyên bố đằng sau lời từ chối là *chẳng gì đọc cái cột ấy*. Đếm
các tham chiếu thì có mười một: `ChannelRow` khai báo nó, hai câu query repository
chọn nó, và `channels.controller.ts` **trả nó về trong response tạo channel**.
Trả về một cột chính là đọc nó. Thứ thực sự đúng thì hẹp hơn và sắc hơn —
**chẳng quyết định nào tra tới nó.** Chỗ duy nhất có `.type ===` trong code không phải test thì nằm trên một
protocol frame, không phải trên một channel.

Mười lăm lượt phân tích đọc lướt qua cụm "năm cột chết" mà chẳng ai đem đo. Phép đo
chỉ tốn một lệnh `grep`. Đây là lần thứ hai trong tính năng này mà một cột bị gọi là chết trong khi một
response body trả nó về, và cả hai lần sự sửa chữa đều làm phát biểu mạnh lên chứ không yếu đi: một
giá trị có thể hiện ra với
mọi client mà vẫn chẳng cho phép điều gì.

<Why source="docs/04-srs.md FR-CHN-05">

FR-CHN-05 là một câu: message của một channel riêng tư chỉ hiện ra với các thành viên
của nó. Công việc không nằm ở cái câu. Công việc nằm ở chỗ "hiện ra" có bốn cái cửa —
đường gửi, đường đọc-theo-id, đường history, và socket session — và một
bảo đảm chỉ tốt bằng cái cửa yếu nhất của nó. Chương này nới enum ra **sau cùng**,
sau khi cả bốn đều vững, và đó là thứ tự ngược với thứ tự nghe có vẻ tự nhiên.

</Why>

## Cái động từ không có handler

Trước khi viết một dòng nào, một câu hỏi: mọi động từ phải tôn trọng quy tắc mới có
chỗ nào để tôn trọng nó không?

`GET /v1/channels/:channelId` không hề tồn tại. `channels.controller.ts` mang một lệnh tạo
và một lệnh thêm thành viên và chẳng có lệnh đọc nào, nên một khách hàng có thể tạo một channel rồi chẳng bao giờ
đọc lại được bốn field của nó. Ba tài liệu dựa vào việc route đó có mặt — một tiêu chí
thành công gọi tên "đọc theo id" là một trong bốn động từ, một yêu cầu nói "mọi lần đọc",
và một hợp đồng có một dòng cho nó.

Chẳng gì tìm ra chuyện đó bằng cách so đặc tả với kế hoạch với các task. Nó tới từ việc hỏi
repository một câu hỏi có câu trả lời có-hoặc-không: *động từ này có handler không?*
Hai trong tám phát hiện then chốt của tính năng này tới từ loại câu hỏi đó, và
cái còn lại cũng cùng hình dạng — một lần đăng ký module mà chẳng task nào gọi tên, thiếu nó thì tám
route sẽ không được gắn vào.

<Trap>

Bốn cái bảng mà danh sách task của tính năng này mang — route theo handler, credential,
phương thức repository và cái ghim; cột theo migration, bên ghi, bên đọc và bài test gỡ bỏ —
đóng lại đúng cái lớp defect đó và chỉ lớp đó. Chẳng bảng nào hỏi xem một route có
khớp với kịch bản đã thúc đẩy nó không. Một lượt riêng tìm ra một yêu cầu đòi
gỡ bỏ hàng loạt "tới 100 người trong một request" trong khi một hợp đồng đặc tả
`DELETE …/members/:userExternalId`, số ít. Mười lượt đã đọc cả hai và độ phủ định danh
đọc ra 100% suốt cả thời gian đó, bởi cả hai tài liệu đều nói "gỡ bỏ".

Hình dạng chỉ trở nên nhìn thấy được khi bạn đặt một **kịch bản nghiệm thu** cạnh một **route**.

</Trap>

## Phép kiểm mà lệnh gửi công khai duy nhất không với tới được

Phép kiểm membership thuộc về tầng truy cập dữ liệu — hiến pháp đặt sự cô lập tenant
ở đó, và cùng lập luận ấy mang theo cả việc phân quyền bên trong một tenant, bởi một phép kiểm trong
một service có thể bị bên gọi kế tiếp đi thẳng vào repository né mất. Ghi chú
nghiên cứu nói sáu bên gọi thừa hưởng nó.

Đếm từ đồ thị gọi thì có ba. Và cái quan trọng nhất thì chẳng cung cấp
gì cả:
...
`MessagesController` chẳng khai báo `@Accepts` nào, nên guard lùi về chấp nhận
cả hai lớp credential, và một user token lọt vào. Handler rồi gọi một service
gọi một hàm repository mà tham số `userId` của nó là tuỳ chọn — và
phép kiểm bị gác sau việc tham số đó có mặt. Nên một user có thể gửi vào một channel
riêng tư mà họ không phải thành viên và chẳng có gì ở đâu nhìn tới.

**Một tham số chẳng ai điền thì chẳng mã hoá điều gì.** Lập luận đó đúng với
chữ ký hàm và sai với bên gọi, và viết ra đồ thị chính là thứ tìm ra nó. Sáu
lượt phân tích đã đọc con số sáu.

<Figure caption="Một phép kiểm trên bên gọi cần ba chỗ, và tính năng này có hai route còn lỗ hổng" code={figThreePlaces} />

File task giờ mang một bảng năm dòng — mỗi dòng một route mà hành vi của nó
phụ thuộc vào ai đang gọi, với ba thứ kia làm cột. Hai trong năm dòng có lỗ hổng.
Cái thứ hai chỉ được tìm ra vì cái thứ nhất đã được tìm ra: `messages.controller.ts` có đúng hai
route, và sau khi sửa lệnh gửi thì chẳng ai hỏi xem đường đọc có cùng hình dạng không.
Nó có. `listMessages(channelId, {beforeSeq, afterSeq, limit})` chẳng có chỗ nào để đặt một
`userId`, nên cái task nói "thêm cùng phép kiểm đó vào đường history" là đang đòi một
phép kiểm chẳng có gì để mà kiểm.
...
Cả hai route giờ đều phân giải bên gọi từ principal, và cả hai đều từ chối một user
external id chẳng gọi tên dòng nào — một token mint cho một user từ đó đã bị gỡ bỏ
là một request hỏng, không phải một lần gửi nặc danh.

<Trap>

**Một bài test repository chứng minh một phép kiểm tồn tại. Chỉ một bài test route mới chứng minh nó nổ.**

Đường gửi có một bài test repository xanh suốt sáu lượt phân tích trong khi
controller của nó chẳng cung cấp bên gọi nào. File task nói bài test đó tồn tại "để một controller
không che được nó", mà đó chính xác là điều ngược với chuyện đã xảy ra. Một bài test lái
thẳng repository, một bài lái route nội bộ, và cả hai đều cung cấp một `userId` mà
route công khai chẳng bao giờ gửi.
...
Đường history là cái đáng đọc hai lần. Cách sửa đầu tiên trả về một danh sách rỗng
cho một channel riêng tư mà bên gọi không thấy được — thứ đó rò rỉ, bởi một channel vắng mặt
trả lời 404 còn một trang rỗng trả lời 200. Một điều kiện duy nhất tạo ra cả hai
lời từ chối thay vào đó: `channelVisibleTo` trả về false cho một channel không tồn tại
**và** cho một channel riêng tư mà bên gọi không ở trong, còn handler bên trên nó biến một
câu trả lời thành một phong bì.

Lựa chọn đó được đền đáp về sau theo một cách chẳng phải lý do để đưa ra nó. Xem
phần gauntlet, bên dưới.

## Một lời từ chối phơi ra chính thứ nó đang từ chối

Ba task và một hợp đồng đặc tả lời từ chối gửi của một channel riêng tư là
`403 not_a_member`. Một tiêu chí thành công đòi hỏi câu trả lời của lệnh gửi phải giống từng byte với
một channel không tồn tại. Cả hai không thể cùng đúng: một 403 thông báo rằng channel
tồn tại.

Nên lệnh gửi vào một channel riêng tư trả lời bằng phong bì không-tìm-thấy — và `not_a_member` hoá
ra **chẳng có bên phát nào trong chương này**. Bên phát duy nhất của nó là route vị trí đọc
trên một channel công khai, nơi một vị trí đọc là trạng thái theo từng thành viên và từ chối một
người không phải thành viên là cùng quy tắc mà phần còn lại của bảng giữ, và route đó là của chương
kế tiếp. Mục registry nói thế, bởi nói ra điều đó tiết kiệm cho người đọc kế tiếp một lần
tìm kiếm.

<Trap title="Ten hits and no emitter">

Grep `not_a_member` trong nền tảng ở chương này thì bạn nhận mười kết quả. Mỗi
cái trong số đó đều là một **giá trị kết quả miền**, không phải một mã trên đường truyền:

```ts title="services/api/src/db/repository.ts (excerpt)"
  ): Promise<Map<string, "removed" | "not_a_member">> {
```

Việc gỡ thành viên hàng loạt báo một kết cục cho mỗi user, và một trong các kết cục được viết
giống hệt mã lỗi. Service biến nó thành một lời từ chối một dòng sau đó — và
lời từ chối nó ném ra là `notFound()`, vì cái lý do phần trên đã nêu. Nên mười
kết quả kia nói điều ngược với thứ một người đọc sẽ rút ra từ chúng.

Hai trong ba mã mà chương này đăng ký đều ở vị trí đó: **`channel_archived`
thì được gửi**, `not_a_member` chờ route của chương kế, còn `user_banned` chờ
lệnh cấm. `user_banned` xuất hiện đúng một lần trong nền tảng, trong một khẳng định rằng
lời từ chối lưu trữ **không phải** nó.

Một mã được đăng ký trước bên phát của nó cũng cùng hình dạng với mã đóng 4008, thứ mà dự án
này có một bài test cho: đã đăng ký, đã ghi tài liệu, và chẳng gì gửi. Registry là
bộ từ vựng, không phải bản kiểm kê những gì đã được nói ra.

</Trap>

<Figure caption="Cấm, rồi membership và hiển thị, rồi lưu trữ — và thứ tự kia để lộ ra điều gì" code={figRefusalOrder} />

Thứ tự là nửa thứ hai của cùng cái defect. Cấm, rồi lưu trữ, rồi membership
nghĩa là một người không phải thành viên của một channel riêng tư **đã lưu trữ** sẽ nhận `channel_archived` và biết
channel đó tồn tại. Phép kiểm lệnh cấm chạy trước cả khi channel được phân giải, nên một
user bị cấm nhận cùng một câu trả lời cho mọi channel id, thật hay bịa.

<Why source="the isolation harness, fifth analysis pass">

Phát hiện sắc sảo nhất của chương isolation harness là `POST /v1/channels` sắp chấp nhận
`type: "private"` trong khi chẳng gì trong nền tảng đọc cái cột ấy. Cả hai phát hiện ở đây đều là
chính cái đó chỉ về phía trước: một lời từ chối phơi ra thứ nó đang từ chối. Phải có một
sản phẩm chính xác mới tìm ra chúng. Năm lượt đọc các sản phẩm đối chiếu nhau
chẳng cho ra gì ngoài các lỗi số học; cái lượt đổi "cả ba động từ" thành
"cả bốn động từ mà SC-001 gọi tên" đã đặt lời từ chối của lệnh gửi cạnh tiêu chí không-phân-biệt-được,
và mâu thuẫn rơi ra. **Sự chính xác tạo ra mâu thuẫn ở chỗ mà
sự mơ hồ đã giấu chúng.**

</Why>

Đây là tầng repository với tất cả đã vào chỗ — điều kiện hiển thị,
phép kiểm membership trên lệnh gửi, phép kiểm lưu trữ bên dưới nó, việc gỡ hàng loạt, các vai trò, và
lệnh đọc theo id vốn chẳng có handler.
...
`removeMembers` đầu tiên tôi viết dựng danh sách `IN` của nó bằng `sql.raw`:

```ts
sql.raw(`ARRAY['${userIds.join("','")}']::uuid[]`)
```

Những id đó đến từ một request body. Đó là một lỗ hổng SQL injection trong đúng cái tầng của
nền tảng này được phép nói SQL, viết ra trong lúc đang cài đặt một chương về
phân quyền. `inArray(members.userId, userIds)` tham số hoá nó và đọc cũng dễ hơn.
Chẳng gì trong lane bắt được nó: mọi bài test đều truyền các id vốn đã là
uuid.

</Trap>

## Hai bộ từ vựng vai trò, cách nhau một từ

`memberships.role` đã là `('owner','admin','member')` từ chương tenancy — vai trò của một con người
trong một tổ chức. Vai trò của một thành viên channel là `('owner','moderator','member')`.
Cách nhau một từ, và một migration tái dùng ràng buộc của tổ chức sẽ chấp nhận
`admin` trên một thành viên channel, từ chối `moderator`, và trông có vẻ đúng lúc review.
...
Cả hai ràng buộc CHECK giờ đều mang một lời chú thích gọi tên cái kia, theo cả hai hướng.
Migration là mới, nên nó xuất hiện nguyên vẹn:
...
Bởi bản thiết kế sinh ra nó giữ ba thứ, và lý do cho việc nằm vắt qua
thì thôi còn đúng.

`members.role` được thiết kế trong cùng pha với `users.deleted_at` và
`read_positions`, và cả ba tới dưới dạng hai migration tách theo CHỦ ĐỀ — một file
hoạt-động-và-vị-trí-đọc và một file vai-trò-và-xoá-user. Chẳng file nào
thuộc về một chương. `0011` chẳng giữ thứ gì chương này dùng; `0012` giữ
`members.role` cho chương này và `users.deleted_at` cho chương sau.

**Nằm vắt qua thì rẻ khi cả hai chương được viết trong cùng một pha và bất khả thi
khi không.** Migration áp dụng theo thứ tự, nên chương này không thêm được `0012`
mà không có `0011`, và `0011` sẽ tới nơi mang hai cột mà chẳng gì đọc suốt cả một
chương — một schema mà người đọc checkout ra rồi không giải thích nổi. Gom nhóm lại là
phương án khi cả hai đang được viết cùng lúc, và nó tốn một thứ tự pha; ở đây
nó chẳng tốn gì, còn việc nằm vắt qua thì tốn một migration không áp dụng riêng được.

Nên con số đi theo chương: `0006` là một cột duy nhất của chương này, còn
`last_activity_at`, `read_positions` và `users.deleted_at` tới cùng nhau ở
chương sau dưới dạng `0007`. **Số hiệu của một migration nên đi theo chương giới thiệu nó**,
và điều đó chỉ đúng nếu chẳng migration nào phục vụ hai chương.

</Why>

**Cái gì đọc vai trò của một thành viên.** Phần liệt kê trả nó về. **Chẳng thao tác nào được phân quyền theo
nó** — chẳng quyết định về quyền nào ở đâu trong nền tảng tra tới cái cột ấy. Sự
phân biệt đó là phát biểu đáng nói, và suốt mười ba lượt phân tích thì các
tài liệu nói "chẳng gì đọc nó" trong khi bảng field của chính một hợp đồng thì trả về
cái field ấy. Tìm ra bằng cách đặt hai thứ cạnh nhau, điều mà chưa lượt nào trước đó làm.
...
Body của lệnh thêm là một union chứ không phải một hình dạng mới. Chương trước ship
`{"user_ids": ["a", "b"]}` và server của một khách hàng đang gửi cái đó ngày hôm nay, nên một mục
hoặc là một external id trơn hoặc là một id kèm một vai trò:

```json
{"user_ids": ["ana", {"user": "bo", "role": "owner"}]}
```

Một hợp đồng viết trong một lượt phân tích đề xuất đổi tên field thành `users`, thứ sẽ
là một thay đổi phá vỡ với một route đã ship, quyết trong một buổi review.
Cái tên đã ship thắng và hợp đồng được sửa lại.

**Việc gỡ bỏ là một POST tới `.../members/remove`, không phải một DELETE với một id trong đường dẫn.**
Yêu cầu nói tới 100 user trong một request và báo cáo theo từng user, và đó cũng là
hình dạng mà chương trước chọn cho lệnh thêm. Một đoạn đường dẫn chỉ giữ được một id.

## Cái enum, sau cùng

Giờ mới tới lần nới rộng, trong cùng một lần sửa cũng sửa luôn một câu:

```ts
type: z.enum(["public", "private"]),
```

Lời chú thích của chương trước về field đó nói "chẳng có phép kiểm membership nào trên bất kỳ đường
đọc nào". Điều đó sai ngay lúc nó được viết và nó đã ship suốt ba chương.
`repository.backfill` join `members` theo user id của bên gọi, còn socket session
dựng danh sách channel của nó từ `channelsForUser` — cả hai đều là đường đọc và cả hai đều kiểm
membership. Thứ đúng thì hẹp hơn: các route history và send **công khai** thì chẳng kiểm
gì, còn lệnh gửi nội bộ thì phân giải một user rồi chẳng kiểm gì.

Phần sửa chữa sống ngay bên trong cái fence ở trên, cạnh lần sửa nới rộng enum,
bởi một câu sai nằm trong một fence có tiêu đề thì không chờ được một pha sau —
chương nào xuất bản cái file thì cũng xuất bản luôn cái câu.

<Why source="docs/04-srs.md traceability">

Một sự sửa chữa thứ hai là một dòng trong chính bảng truy vết của chương isolation harness, thứ đã ghi
FR-CHN-04 — vai trò thành viên channel — là đã giao. Nó chưa; `members` chẳng có cột role
nào cho tới migration của chương này. Cả hai sự sửa chữa đều được nói ra ở đây chứ không chỉ
được làm, bởi cả hai chủ đề đều là của chương này, và một cách sửa chẳng ai thông báo thì
không phân biệt được với một tài liệu vốn luôn đúng.

</Why>

## Đòn tấn công mà gauntlet chẳng có fixture nào cho

Chương isolation harness dựng bốn hình dạng tấn công trên một sấm truyền. Cả bốn đều lấy identifier
của **một tenant khác**. Mỗi cái trong số đó đều vô dụng ở đây: kẻ tấn công mà chương này
cần là một user của chính tenant của bạn mà không phải thành viên.

<Figure caption="Bốn hình dạng xuyên tenant, một hình dạng cùng tenant, và cái đối chứng mà thiếu nó thì chẳng cái nào có nghĩa gì" code={figSameTenant} />
...
Sáu bài test đối chứng, và chúng mới là con số quan trọng hơn số đòn tấn công.
Chương isolation harness ship mười bốn bài test xanh so hai lời từ chối — mà hai
lời từ chối vì một lý do chẳng liên quan thì cũng giống nhau từng byte. Ba trong số này chứng minh rằng
token của chính thành viên chạy được trên cùng channel đó trước khi có gì khẳng định rằng token của một
người không phải thành viên thì không.

Khối thứ hai là một trường hợp **mà chính tính năng này tạo ra**. Hai tenant dùng cùng một
`external_id`, một channel công khai và một channel riêng tư: trước chương này thì mọi channel
đều công khai, nên "phân giải về channel của chính tenant bạn" và "trả lời bạn theo cùng một cách"
khớp nhau một cách tầm thường. Giờ các loại khác nhau, và hai câu trả lời khác nhau *bởi các loại
khác nhau, chứ không phải bởi các tenant khác nhau* — và đó mới là tính chất đáng một bài test. Fixture
hiện có làm cho việc viết nó thành bất khả, bởi nó gắn nhãn mọi id bằng tên của chính tenant sở hữu
và hai tenant thì chẳng bao giờ dùng chung một id.
...
Danh sách mục tiêu được suy dẫn từ router đang chạy, nên một route được dựng mà không được
phân loại sẽ làm suite đỏ ngay ở bản build thêm nó. Con số được khẳng định như một
**tổng** chứ không phải một hiệu: `sau − trước` tính bên trong một lần chạy là một đồng nhất thức
và không thể đỏ được.

    gauntlet targets: 30 derived, 27 attacked, 3 exempt

Hai mươi bốn trước tính năng này, sáu route đã thêm tới giờ, ba phần miễn trừ không đổi
và mỗi cái vẫn mang một lý do được viết ra. Chẳng gì chương này thêm vào được miễn trừ.

## Gỡ từng phép kiểm ra xem suite có nhận ra không

Một suite toàn các lời từ chối thì không tự chứng minh được rằng nó đang test thứ gì. Nên mỗi phép kiểm mới
đều được gỡ ra lần lượt, kèm một lần build lại và cả bộ gauntlet:

    tắt phép kiểm lệnh gửi             2 đỏ | 30 xanh
    tắt phép kiểm đọc theo id          2 đỏ | 30 xanh
    tắt phép kiểm hiển thị của history 1 đỏ | 31 xanh

Mỗi cái làm đỏ đúng đòn tấn công của nó và không cái nào khác, và **ở cả ba lần thì các đối chứng đều
xanh** — đó chính là thứ phân biệt một kẻ tấn công chạy được với một kẻ tấn công hỏng.

Việc bài test va chạm đỏ cùng với phép kiểm đọc theo id là đúng chứ không cẩu thả: nó
khẳng định hai tenant được trả lời khác nhau *bởi các loại khác nhau*, nên một lần gỡ bỏ
khiến loại thôi quyết định thì phải làm nó đỏ.

<Trap>

**Chẳng gì bị che, và lý do là một cách sửa làm ra vì một mục đích khác.**

Chương isolation harness tìm ra rằng `channelExists` chạy trước và có giới hạn phạm vi, nên một lệnh đọc không giới hạn
bên dưới nó chẳng bao giờ với tới được — một suite thì nhạy với phép kiểm ngoài cùng trên một đường và
mù với một phép kiểm bên trong mà một phép kiểm ngoài đang sống che mất. Đó là lý do danh sách task của chương này
nói hãy *ghi lại* những đòn tấn công nào bị che chứ không phải nắn lại chúng.

Chẳng cái nào bị che. Phép kiểm ngoài cùng của đường history **chính là** phép kiểm hiển thị bây giờ, bởi
`channelVisibleTo` thay thế `channelExists` chứ không phải được thêm bên dưới nó. Một
điều kiện duy nhất tạo ra cả hai lời từ chối, nên chẳng còn phép kiểm ngoài nào để mà che. Nếu
thêm bên dưới — cách hiển nhiên để thêm một phép kiểm — thì lần gỡ bỏ ấy sẽ quay về
xanh và suite sẽ báo thành công.
...
## Socket chẳng có frame nào để mà tấn công

Task nói hãy thêm "một frame subscribe của một người không phải thành viên cùng tenant, từ chính
union frame của giao thức". Đọc cái union thì rõ: có đúng một thành viên đi vào,
`message.send`. **Chẳng có frame subscribe nào.** Thứ một socket được thấy thì được quyết ở lúc
connect, từ danh sách channel của session, và một cursor là thứ duy nhất một client được
khẳng định về nó.

Nên đòn tấn công là một cursor gọi tên channel riêng tư, và khẳng định là tập cursor của
ack không chứa nó. Một task chỉ nói "thêm một bài test subscribe"
sẽ tạo ra một bài test cho một thứ không tồn tại; chính chỉ dẫn phải
suy dẫn nó từ cái union mới biến nó thành một bài test viết được.

<Trap>

Một frame đã khai báo mà chẳng có bên gửi, trong một tính năng nói về các cột đã khai báo mà chẳng có bên đọc:
`presenceChangedSchema` nằm trong union frame và **chẳng gì phát ra nó**. Ghi chú
nghiên cứu hoãn presence lại, "chỉ trong phạm vi: socket của một người không phải thành viên thì không được
đăng ký, nên nó chẳng nhận presence nào cho channel đó" — một tuyên bố đúng một cách rỗng tuếch. Nó được
ghi lại đúng như thế chứ không để người ta đọc ra như thể presence đang chảy.

</Trap>

## Sáu route được phân loại, và chẳng ai viết các đòn tấn công

Gauntlet kết thúc bằng một bài test so thứ đã chạy với thứ mà phân loại
nói lẽ ra phải chạy. Thêm các route của chương này vào `targets.ts` làm nó đỏ sáu
lần liền:
...
Các đòn tấn công có tồn tại. Chúng được viết cùng lúc với các endpoint channel, trong một commit
cũng mang theo sáu dòng mục tiêu `/v1/webhooks*` — và các dòng webhook thuộc về
chương dựng webhook, nên chúng bị hoãn. **Các đòn tấn công đi theo chúng, và
chẳng gì nói ra điều đó.** Một lần hoãn là một quyết định về thứ gì được dời đi; nó chẳng có cách nào biết
thứ gì đang dựa vào thứ nó dời.

Thứ nói ra điều đó là một suite tự kế toán được chính nó. Không có bài test ấy thì sáu route
sẽ được phân loại, được ghi tài liệu, chẳng được miễn trừ gì, và chẳng bị ai tấn công —
và mọi con số trong phần tổng kết của chương này đều sẽ đọc ra đúng đắn.

<Why title="One parameterised block, not six tests">

Cả sáu đều cùng một hình dạng: channel id của nạn nhân với một id chẳng tồn tại ở
đâu, dùng chính credential của bên gọi. Sáu cái `it` riêng biệt sẽ lặp lại cái cặp đó sáu
lần và giấu mất chuyện nó chỉ là một tính chất.
...
**Trạng thái được đọc là cả dòng channel lẫn số thành viên**, bởi hai trong các
route này thay đổi membership còn hai cái thay đổi channel. Một lời từ chối mà vẫn lưu trữ
channel thì để nguyên danh sách message và vẫn là một lần vi phạm — nên
trạng thái của nạn nhân phải là bất cứ thứ gì mà route đang bị tấn công có thể đã đổi.

</Why>

Và khẳng định bên cạnh nó là phép tính trên con số của một cái cây khác:
...
Hai mươi bốn là con số kết thúc của một chương khác, mang sang đây như một hằng số viết cứng. Nó
đỏ với `expected 17 to be 30`, và **chẳng con số nào gọi tên một route** — nửa hữu ích của
lần đỏ ấy là cái nào đang thiếu, mà một phép trừ thì không nói được. Giờ nó là một danh sách có tên,
đối chiếu với router đang chạy.

## `PORT=0` an toàn với một trong hai service

Hành trình đầu-cuối đỏ hai lần với **cả tám bài test bị bỏ qua**, ngay sau
một lane spawn các tiến trình api con, và xanh khi chạy một mình. Mọi thứ về chuyện đó
đều chỉ vào một cái cổng: harness bind cố định `4100` rồi cho mỗi gateway
`apiPort + 1 + i`, đó là một dải, và một dải là một cái bảng chẳng gì kiểm.

Thay nó bằng `PORT=0` không sửa được.
...
Api báo cái cổng nó đã **bind** — nó đọc ngược địa chỉ ra, bởi ghi log
giá trị bạn được đưa thì in ra `0`. Gateway thì ghi log giá trị nó được đưa. Nên
`PORT=0` tạo ra một lần dò sức khoẻ tới cổng không, và lần đỏ rơi vào
hành trình chứ không rơi vào service nào cả.

<Trap title="Nothing noticed because nothing asked">

Gateway đã ghi log sai con số suốt mười một chương và mọi lần đọc nó đều
đúng. Mọi suite spawn một gateway đều đưa cho nó một cổng cố định, nên giá trị
được ghi log chính là giá trị được truyền vào.

Đó cũng cùng hình dạng với một phần miễn trừ chẳng bao giờ nổ và một dòng mồi mà chẳng thao tác toàn cục nào
giành được: **một tính chất chỉ trở nên quan sát được vào lần đầu tiên có thứ gì đó
cần câu trả lời.** Cách sửa không phải hai dòng trong `main.ts` — mà là hỏi, trong một
bài test, theo cách không thể được thoả mãn một cách tình cờ.
...
Danh sách service được suy dẫn từ `services/*/src/main.ts` chứ không được viết ra, nên
một service thứ ba sẽ tới nơi bên trong phép kiểm mà chẳng ai phải nhớ. Và case
đầu tiên khẳng định rằng phép suy dẫn tìm ra nhiều hơn một file, bởi một phép suy dẫn chỉ tìm ra
một file thì xanh một cách rỗng tuếch với file kia.

</Trap>

Bốn cổng cố định ra đi cùng nó: cái dải này, `4123` của suite socket, `4100` của
hành trình, và cái dải gateway suy ra từ nó. `RELAY_E2E_API_PORT` và
`RELAY_SESSION_ITEST_API_PORT` rời khỏi danh sách env cho phép của `turbo.json`. Những chuỗi cổng
duy nhất còn lại trong repository là hai giá trị mặc định production của chính hai service.

## Hai ký tự đọc lên như một secret bị rò rỉ

Trong lúc các cổng đang bị truy đuổi, suite credential đỏ một lần:

```
AssertionError: expected '[{"public_id":"cbd76832c8bcb15cf50def…' not to contain 'WA'
```

Bài test chứng minh rằng một secret đã mint thì không phục hồi được từ cái dòng nó để lại, và nó
lấy secret ra như thế này:

```ts
    const secret = minted.credential.split("_").at(-1)!;
```

Secret là base64url, và **bảng chữ cái của base64url có cả `_`**. Nên khẳng định đó nằm
trên bất cứ thứ gì theo sau dấu gạch dưới cuối cùng của chính secret — thường là một cái đuôi dài, và
bài test xanh vì đúng lý do. Thi thoảng là hai ký tự, và nó xanh chẳng vì
lý do gì cả. Rồi hai ký tự ấy xuất hiện bên trong cái salt lưu cạnh
hash:
...
Thứ đọc lên y hệt như api trả về một secret mà nó vừa băm xong.

`minted.prefix` vốn đã nằm sẵn trên object mà lệnh mint trả về. **Lần sửa đầu tiên dùng
`lastIndexOf("_")` và đó là cùng cái lỗi ấy lần thứ hai** — dấu gạch dưới cuối cùng trong
credential có thể thuộc về chính secret. Có đúng một câu trả lời chính xác và nó vốn đã
nằm ở đó.

<Why title="Why a length guard as well">

Bởi toàn bộ lần đỏ là một khẳng định trên một chuỗi ngắn, và cách sửa gỡ bỏ
nguyên nhân mà không gỡ bỏ cái lớp. Một secret mà bài test này đem so phải đủ dài
để một lần trùng ngẫu nhiên không phải là thứ đang được đo:

```ts
    expect(secret.length, "the secret is too short to assert on").toBeGreaterThan(20);
```

Hai mươi lần chạy liên tiếp xanh. Cái flake ấy đã nằm trong cây từ chương
credential và sẽ nổi lên khoảng một lần trong mỗi vài trăm lần chạy — tức là
trong loạt chạy của bất kỳ chương chẳng liên quan nào tình cờ đi qua nó.

</Why>

## Con số không dịch chuyển, và cái file vẫn không đổi

Bản danh mục chứng minh rằng mọi bảng đều có một đường tới đúng một tenant báo **13
bảng gốc — 3 trực tiếp, 2 bước nhảy, 8 xương sống** — và `tenant-scope.itest.ts` không đổi
một ký tự nào.

Chương này chẳng thêm bảng nào. Nó thêm một **cột** vào `members`, và `members` vẫn là
`hop`: nó chẳng mang `environment_id`, nên nó được với tới qua `channels` và chẳng
trigger nào canh nó. Một cột vai trò thì *cảm giác* như trạng thái tenant còn phép phân loại thì không
nói về cảm giác của một cột — nó được suy dẫn từ `information_schema` ở mọi lần chạy.

<Why title="Why members will never join the guard's array">

Guard cài một trigger cho mỗi bảng mang `environment_id`, và mệnh đề WHEN của nó
đọc `OLD.environment_id`. Trên `members` thì biểu thức đó không biên dịch được.
Migration nói thế ngay trên chính cái ràng buộc, bởi người kế tiếp thêm một
cột theo phạm vi thành viên sẽ có cùng bản năng ấy:
...
Cả hai ràng buộc giờ đều gọi tên cái kia, theo cả hai hướng, bởi một lời cảnh báo ở một phía
của một cái bẫy là một lời cảnh báo mà người kế tiếp không tìm thấy.
...
**Hai mươi trong hai mươi bảy file đã đổi của chương này được fence trong các phần
ở trên** — schema, migration, bốn cái cửa, các endpoint, gauntlet.
Tiếp theo là bảy file tới nơi trong lúc chương đang được xác minh chứ không phải
trong lúc nó đang được dựng: một helper được nhấc ra khỏi một suite để mọi route
dùng chung, phần việc về cổng, và một cái flake đã nằm trong cây từ chương
credential.

### Cái sấm truyền, được nhấc ra
...
### Mọi service đều báo cái cổng nó đã bind
...
- `pnpm coverage` — 44 file, 397 test, mọi ratchet theo file đều đạt
- `gauntlet targets: 18 derived, 14 attacked, 4 exempt`; 51 bài test cô lập, sáu đối chứng
- ba phép kiểm được gỡ lần lượt: 2, 2 và 1 lần đỏ, các đối chứng xanh cả ba lần
- `tenant paths: 13 base tables — 3 direct, 2 hop, 8 spine` — không đổi bởi chương này
- `POST /v1/channels` chấp nhận `private`, và mỗi cái trong bốn cánh cửa đều từ chối một
  người không phải thành viên bằng đúng cái phong bì mà một channel không tồn tại nhận
- trong ba mã mà chương này đăng ký, **một cái được phát ra**: `channel_archived`

</Checkpoint>

<ForwardRef part="the next chapter">

Những gì một user thấy: các channel của họ sắp theo hoạt động, kèm số chưa đọc và một cursor;
profile của họ, được tạo ngầm lúc xác thực lần đầu; và phép đo chỉ
sai hướng — sắp theo message cuối tốn 0.87 ms trên lane test và
159 ms ở mức một triệu message, kèm một lần quét tuần tự qua mọi message trong
environment ở mọi lần liệt kê.

Nó cũng gom lại ba thứ mà chương này để nợ, và mỗi thứ đều được viết ra ở nơi
chương sau sẽ tìm thấy chứ không để phải suy ra: `read_positions` và
`channels.last_activity_at` dưới dạng migration `0007`, `users.deleted_at` đi cùng chúng, lệnh
xoá vị trí đọc phải đi cùng một membership bị gỡ, và cái mục đặt
`read_positions` ra sau guard trong cùng lần sửa tạo ra nó.

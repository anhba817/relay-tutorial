...
import { figDerivedTargets, figWhoMayCall } from "./figures";

export const metadata = {
  title: "The isolation harness — Building Relay",
  description:
    "The constitution has required a cross-tenant suite since it was written. This builds the one that derives its own target list from the running router, so a route that exists and is unattacked fails the suite.",
  alternates: {

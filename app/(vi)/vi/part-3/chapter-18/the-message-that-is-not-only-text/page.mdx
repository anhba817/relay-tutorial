...
export const metadata = {
  ...
  description:
    "Message content là union: text hoặc attachment metadata. Chương này đặt shape ở protocol, enforce qua REST/socket, giữ NULL khác empty list và xác định deletion để lại gì.",
  alternates: {
...
Message không chỉ là text. Trước khi thêm field, chapter quyết định content model, compatibility
và nơi nào phải từ chối shape không hợp lệ.

## Một quyết định trước mọi thứ khác

Message content là union: text hoặc attachment metadata. Quyết định này phải được đặt ở protocol trước khi thêm column hay endpoint, vì nó xác định mọi caller được phép gửi gì và mọi reader phải nhận shape nào.
...
## Điều compiler không thể thấy

Type có thể buộc caller chọn một arm, nhưng không thể chứng minh object storage có file thật, content type an toàn hay attachment thuộc tenant đúng. Runtime validation và authorization vẫn là phần của contract.
...
## Shape, và arm từ chối

Union phải từ chối arm không hợp lệ rõ ràng: text rỗng, attachment thiếu metadata, hoặc cả text lẫn attachment khi policy chỉ cho một. Một shape permissive hôm nay thành compatibility burden ở mọi reader ngày mai.
...
REST send, socket send và internal service đều phải dùng cùng validator. Sao chép rule ở ba door gần như đảm bảo một door sẽ chấp nhận payload mà hai door còn lại từ chối.
...
## Ba chỗ socket send làm rơi field

Socket path thường serialize request, domain command rồi frame broadcast. Attachment phải đi qua cả ba; mất field ở một bước tạo bug chỉ xuất hiện với realtime client trong khi REST history vẫn đúng.
...
## NULL và empty list là hai giá trị khác nhau

`NULL` nghĩa là message không có attachment arm; empty list có thể nghĩa attachment arm được chọn nhưng không có item. Database và API không nên làm phẳng hai trạng thái nếu product semantics cần phân biệt chúng.

## Deletion để lại gì

Khi message bị tombstone, attachment metadata cũng không được tiếp tục lộ qua history. Xóa object vật lý có thể là workflow riêng, nhưng reader phải ngừng trả capability hoặc URL ngay khi deletion contract có hiệu lực.

## Required field đáng giá gì

Một field `NOT NULL` chỉ đáng giá khi write path, migration và reader cùng tôn trọng nó. Required-by-schema không thay thế validation của union hoặc backfill cho message lịch sử.
...
Các diff phía dưới nối protocol, migration, endpoint, gateway và test. Mục tiêu là một content shape duy nhất, không phải một attachment feature chỉ hoạt động ở một con đường.

...
Và còn một cái nữa, lộ ra sau tất cả những thứ trên, bởi chính coverage lane mà phần close-out
của chương này chạy.

Cùng quy tắc ấy đã được áp lên `outboxEventSchema` — thứ mà platform không hề dựng ra. Nó là cái
`consumer/runtime.ts` parse ra từ một durable queue, và một lần parse thất bại ở đó được trả lời
bằng `message.term()`: dừng redelivery vĩnh viễn. Vào đúng lần deploy đưa chương này lên, mọi
`message.created` còn nằm trên stream đều do binary chạy trước đó ghi, không hề có key
`attachments`. Required ở đây nghĩa là bị huỷ.

Sáu bài test trong `consumer.itest.ts` đỏ vì đúng chuyện đó, và chúng đỏ được là nhờ hai fixture
không ai cập nhật vẫn còn viết đúng payload cũ. Mười một lượt analysis không tìm ra nó, và chính
header của file đã nói lý do: suite của api chạy với `RELAY_EVENT_CONSUMER=off`, nên không có gì
trong đó động tới consumer.

Vậy nên quy tắc có một đường biên, và đường biên ấy đáng được nói sắc gọn như chính quy tắc.
**Required là một khẳng định về thứ bạn ghi ra. Còn kẻ đọc bất cứ thứ gì bền vững — một queue,
một cột, những byte do người khác ghi — thì không thể đòi một field mà người ghi nó chưa từng
có.** `.default([])` cho hai nhánh event, còn interface phía producer vẫn giữ required, bởi hai
câu ấy nói về hai chiều khác nhau.
...
+
 describe("the deleted frame carries an identity and no text", () => {
   const tombstone = {
     id: message.id,
@@ -182,6 +215,19 @@ describe("the deleted frame carries an identity and no text", ()
     ]);
   });
...
@@ -65,6 +70,7 @@ describe("the revision fabric payload", () => {
...
@@ -1110,3 +1110,317 @@ describe("deleteMessage", () => {
...
@@ -512,15 +771,24 @@ describe("PATCH /v1/channels/:channelId/messages/:messageId", ()
   });
...
+    };
   ...
   const patch = (
@@ -1012,6 +1280,127 @@ describe("PATCH /v1/channels/:channelId/messages/:messageId", ()
...
@@ -957,6 +957,10 @@ describe("a typing signal on its way out", () => {
               seq: 9,
               user: "tuan",
               text: "backfilled",
...
@@ -1227,6 +1231,10 @@ describe("a typing signal on its way out", () => {
         seq: 4_242,
         user: "tuan",
         text: "one message",
+        // This payload goes onto the fabric as JSON and the gateway

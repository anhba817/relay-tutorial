...
export const metadata = {
  title: "Webhook sống sót trước hệ thống của khách hàng — Building Relay",
  description:
    "Handler ở chương 3.4 chỉ ghi một dòng log rồi gọi đó là effect. Chương này trao cho nó một effect thực sự — một HTTP request tới cỗ máy không thuộc quyền kiểm soát của chúng ta — và chứng kiến pattern từng bảo đảm tính đúng đắn của chương 3.4 không còn áp dụng được ở hai nơi.",
  alternates: {
    canonical: "/vi/part-3/chapter-18/webhooks-that-survive-the-customer",
    languages: {
      en: "/part-3/chapter-18/webhooks-that-survive-the-customer",
      vi: "/vi/part-3/chapter-18/webhooks-that-survive-the-customer",
    },
  },
};
...
Khi ấy nó được gọi là scaffold, và từ này được lựa chọn rất cẩn thận. Mọi
guarantee mà chương 3.4 chứng minh — claim event, chạy effect, commit cả hai cùng
lúc để một cú crash rollback claim cùng với công việc — đều được chứng minh trên
một effect chỉ ghi một dòng ra standard output.

Chương này thay nó bằng một effect nằm trên cỗ máy của người khác.

Chỉ một thay đổi ấy đã khiến pattern không còn áp dụng được ở hai nơi riêng biệt,
và không trường hợp nào là bug cần sửa. Effect không thể tham gia transaction
của claim nữa vì nó xảy ra trên một server không thuộc quyền kiểm soát của chúng
ta. Claim cũng không còn do chính đoạn code thực hiện công việc tạo ra, bởi
Constitution IV dành riêng quyền ghi PostgreSQL cho API service — còn component
gửi request lại không phải API.

<SkipAhead locale="vi">

Nếu buộc phải đọc lướt: webhook endpoint được lưu cùng một signing secret dùng
**envelope encryption** (AES-256-GCM) và có thể rotate. Cả current secret lẫn
previous secret đều hợp lệ trong window 24 giờ để khách hàng rotate secret mà
không cần deploy đồng bộ. Request được ký bằng HMAC-SHA256 trên canonical string
`v1:{timestamp}:{raw body}` — **đúng các raw byte được truyền đi**, tuyệt đối
không phải object đã parse rồi serialize lại. Mỗi event được expand thành một
row `webhook_deliveries` cho từng endpoint phù hợp, và các row được ghi **bên
trong** claim transaction của chương 3.4; vì vậy một event được redeliver không
thể nhân đôi webhook của khách hàng. Retry schedule nằm trong column
`next_attempt_at`, được một relay thứ hai trong API drain bằng chính loop
`SELECT … FOR UPDATE SKIP LOCKED` của chương 3.3 với thêm một predicate. Phép đo
cho thấy broker-held delay vẫn chiếm acknowledgement slot trong lúc chờ, khiến
endpoint đã chết có thể bỏ đói endpoint khỏe mạnh. Hệ thống thực hiện bảy
attempt (`now → 1s → 5s → 30s → 5min → 30min → 2h`), sau đó tạo dead letter và
giữ lại trong bảy ngày. Dispatcher là một **deployable mới**, không ghi vào
PostgreSQL và chỉ truy cập state qua internal HTTP seam. Last hop có guarantee
**at-least-once** được công bố rõ ràng, với event id được trao cho khách hàng làm
deduplication key. Trạng thái cuối chương mang tag **`part3-ch5`**.

</SkipAhead>

## Pattern không còn áp dụng được — ở hai nơi

Lập luận về tính đúng đắn của chương 3.4 chỉ có một moving part. Claim và effect
nằm trong cùng một transaction nên không hề có khoảng trống giữa chúng:
...
Vì vậy câu hỏi đã thay đổi. Khoảng trống không thể khép lại, nên chỉ còn một điều
phải quyết định: **ta chấp nhận sai theo hướng nào**:
...
<Figure caption="Khoảng trống không thể khép lại. Điều ta có thể chọn là cú crash sẽ rơi về phía nào." code={figThePatternStops} />

Nền tảng chọn chấp nhận duplicate. Một mất mát không ai phát hiện được tệ hơn
một duplicate mà recipient đã được cung cấp cách xử lý. Chương 3.3 đã dành trọn
một chương để loại bỏ đúng loại failure đầu tiên khỏi publish path; đưa nó trở
lại ở last hop — nơi khách hàng thực sự cảm nhận hậu quả — sẽ phá bỏ thành quả
ấy ngay tại điểm quan trọng nhất.

Quyết định ấy đi kèm một nghĩa vụ, và chính nghĩa vụ này khiến nó trở thành lựa
chọn trung thực thay vì chỉ tiện lợi. **Nếu chọn at-least-once, bạn phải trao cho
recipient phương tiện để deduplicate.** Event id nằm trong envelope, được ghi rõ
trong documentation và giữ nguyên qua mọi attempt của cùng một delivery. Khách
hàng lưu id rồi bỏ qua duplicate là hành vi đúng; nếu không làm vậy, họ đã được
...
Cả hai trường hợp đều là at-least-once, vì vậy cần nói chính xác guarantee thay
đổi ở đâu. *Bên trong* nền tảng, ledger của chương 3.4 khiến việc handling trở
thành once-in-effect: delivery là at-least-once, còn claim `consumed_events` hấp
thụ duplicate. Ở hop của khách hàng, chúng ta không kiểm soát ledger nào nên
duplicate thực sự đến nơi. Guarantee yếu đi không phải do thiết kế kém hơn, mà
do boundary đã chuyển sang một cỗ máy không chạy code của chúng ta.

</Why>

## Endpoint và secret bắt buộc phải khôi phục được

Một endpoint gồm URL, danh sách event type và một signing secret. Secret là
column đáng chú ý, bởi đây là giá trị đầu tiên trong nền tảng **bắt buộc phải
khôi phục được ở dạng plaintext**.

Chương 3.2 lưu API key dưới dạng salted hash, và cách đó hoàn toàn phù hợp: key
được trình ra để so sánh chứ không bao giờ cần tái tạo. Signing secret thì khác.
Dispatcher phải dùng chính secret để tính HMAC ở mỗi attempt; một hash không thể
dùng để ký.

Vì vậy secret dùng envelope encryption chứ không phải hashing — đúng hai nhánh
mà NFR-SEC-02 đã dự liệu: một cho credential cần so sánh và một cho secret cần
được sử dụng:
...
Fallback cho development environment cố ý phát warning thật rõ và tuyệt đối
không tồn tại trong production:
...
```
Một default im lặng ở đây sẽ khiến production âm thầm encrypt signing secret của
mọi khách hàng bằng một key được in ngay trong tutorial. Warning này chỉ buộc ta
thêm đúng một environment variable khi deploy, đổi lại nó loại bỏ cả một nhóm
incident tiềm ẩn.
...
Ba table xuất hiện cùng lúc, và migration được review trước khi chạy — kỷ luật mà
chương 2.1 thiết lập sau khi một migration được generate tự động từng bị apply mà
không ai đọc:
...
```
**URL bắt buộc phải dùng HTTPS và không được trỏ vào internal network.** Nền tảng
sẽ fetch endpoint thay mặt khách hàng từ bên trong chính network của mình — đúng
định nghĩa của server-side request forgery nếu không kiểm tra URL trỏ tới đâu.
Loopback, private range và link-local address đều bị từ chối bằng validation
error mà khách hàng đọc được, thay vì một timeout không thể giải thích.

**Năm endpoint cho mỗi environment.** Con số năm không có gì đặc biệt, nhưng
"không giới hạn" cũng là một con số — và là con số sai. Expansion ghi một row
cho mỗi endpoint phù hợp, nên một environment có mười nghìn endpoint sẽ biến một
event thành mười nghìn row trong cùng một transaction.

**Secret chỉ được trả về một lần duy nhất.** Đó là lúc tạo endpoint, và không bao
giờ lặp lại — column lưu ciphertext, còn plaintext chỉ tồn tại trong khoảng thời
gian response được serialize:
...
Signature bao phủ một canonical string được dựng từ **raw byte của body** — chính
xác các byte sẽ được truyền qua network:
...
```
Timestamp nằm bên trong dữ liệu được ký chứ không chỉ đặt cạnh signature; nhờ vậy
một request bị bắt lại không thể bị replay vô thời hạn. Body được serialize
**đúng một lần**, rồi cùng một string vừa được ký vừa được gửi. Đây không phải
một optimization.

<Trap locale="vi">

**Nếu ký một representation nhưng gửi một representation khác, integration sẽ
fail vì nguyên nhân gần như không thể nhìn thấy.** Đây là cách phổ biến nhất làm
một webhook integration đang hoạt động bỗng hỏng, và biểu hiện của nó gần như
không bao giờ giống nguyên nhân thực sự:
```js
// Both of these are "the same JSON".
const body = JSON.stringify(payload);
const sig  = sign(JSON.stringify(JSON.parse(body)));   // ← a different string
```
Trong Node và cùng một process, vòng lặp parse-rồi-stringify thường trả lại đúng
các byte ban đầu — chính vì vậy bug này sống sót qua mọi test được viết bằng cùng
ngôn ngữ với service. Nó chỉ lộ ra khi khách hàng verify bằng Go, khi một proxy
normalize JSON, hoặc khi ai đó spread object sang object mới để thêm field.

Ngay cả walk script của chương này ban đầu cũng mắc đúng bug ấy. Flag
`--print-signing-material` ký payload object theo cách script dựng nó, trong khi
nền tảng ký output từ `jsonb` — mà PostgreSQL không bảo toàn key order. Signature
được in ra thuộc về một representation chưa bao giờ được gửi; chỉ khi chạy với
endpoint thật ta mới phát hiện ra.

</Trap>

Test cho contract này được viết từ *documentation*, không dùng implementation
của signer. Đây là cách duy nhất để test chứng minh điều nó tuyên bố:
...
```
Một test gọi `signDelivery` hai lần rồi so sánh kết quả chỉ chứng minh function
...
Server của khách hàng đôi lúc sẽ ngừng hoạt động. Nền tảng retry theo schedule
giãn dần rồi cuối cùng từ bỏ — requirements đã quy định phần đó. Câu hỏi đáng
quan tâm là *trạng thái chờ* nằm ở đâu.

Câu trả lời hiển nhiên là broker. NATS cho phép consumer nack một message kèm
delay; broker giữ message và trả lại khi delay kết thúc. Không cần table mới,
không cần loop mới. Chương 3.4 đã xây dựng sẵn consumer.
...
```
Q1 pass hoàn toàn — delayed redelivery sống sót qua một lần process restart với
sai số ba mili giây, tốt hơn mức thiết kế yêu cầu. Nếu chỉ xét Q1, broker-held
delay là câu trả lời đúng.
...
**Kế hoạch được làm lại thay vì chắp vá.** Kết quả đo xuất hiện sau khi thiết kế
đã hoàn thành. Phản ứng trung thực trước phát hiện "cơ chế dự định sử dụng có một
property vi phạm requirement" là lập kế hoạch lại, không phải thêm một ghi chú
rồi tiếp tục. Research R1 và R13 ghi lại thay đổi cùng lý do.

Giải pháp thay thế thậm chí còn nhỏ hơn. Trạng thái chờ trở thành column
`next_attempt_at`, được loop relay của chương 3.3 drain với thêm đúng một
predicate — một cơ chế reader đã hiểu rõ.
...
+);
```
<Figure caption="Delivery row chính là retry schedule. Không có gì ngủ trong broker; một delivery chưa tới hạn đơn giản là một row chưa được query nào chọn." code={figTheSchedule} />

Partial index gói trọn lập luận về performance trong một dòng: drain query chỉ
...
Một event có thể khớp nhiều endpoint. Fan-out từ một thành nhiều là thời điểm
duplicate trở nên đắt đỏ: nếu một event redelivered được expand lần nữa, mọi
khách hàng phù hợp đều nhận thêm một webhook.

Kế hoạch ban đầu publish N message lên broker, mỗi delivery một message. Đây là
một dual write: claim commit vào PostgreSQL rồi N lần publish tới NATS; một cú
crash ở giữa sẽ khiến hai phía bất đồng. Thay vào đó, ghi N **row** sẽ đặt
expansion vào claim transaction, nơi guarantee của chương 3.4 đã bao phủ sẵn:
...
+
   async createUser(externalId: string, displayName?: string): Promise<UserRow> {
     ...
     await this.db.insert(users).values({
       ...
       externalId,
```
Cả số `created` lẫn flag `duplicate` đều được trả về và dispatcher log cả hai.
...
**Subscription filtering diễn ra trong expansion, không phải delivery.** Endpoint
không subscribe `message.created` sẽ không có row nào, thay vì tạo row rồi bỏ qua
về sau. Một delivery row tồn tại nhưng không bao giờ được gửi tương đương với một
retry schedule chứa no-op vĩnh viễn; operator nhìn vào table sẽ không thể phân
biệt nó với công việc thực sự đang bị kẹt.

</Why>

## Vì sao dispatcher là service riêng

Chương 3.3 thêm outbox relay, chương 3.4 thêm consumer runtime, và cả hai đều chạy
**bên trong** API. Chương này bổ sung background worker thứ ba nhưng đặt nó trong
một deployable mới. Sự khác biệt đó cần được giải thích; nếu không, nó chỉ là sở
thích cá nhân.

Lý do là đây là worker đầu tiên có effect không phải một database write.

Relay đọc outbox rồi publish; consumer claim event rồi ghi row. Cả hai đều truy
cập PostgreSQL, còn Constitution IV quy định API sở hữu các database write, nên
chúng phải nằm cùng nơi với write operation. Effect của dispatcher là HTTP
request tới server khách hàng. Dispatcher không cần quyền truy cập database;
cấp quyền ấy cho nó sẽ tạo ra service thứ hai ghi PostgreSQL chỉ vì tiện lợi.

<Figure caption="Ba service, một writer. Dispatcher chỉ truy cập state qua internal seam; nhờ vậy Constitution IV được bảo đảm bằng cấu trúc thay vì phụ thuộc vào một quy tắc con người phải nhớ." code={figWhereItRuns} />

Điều service split này *mua được* là một property vốn chỉ có thể tranh luận bằng
lý thuyết. Nếu dispatcher nằm trong API, câu "webhook không làm chậm end user"
chỉ là khẳng định về event loop và connection pool — điều ta lập luận rồi hy
vọng. Khi có process boundary, nó trở thành một khẳng định có thể kiểm chứng bằng
cách không khởi động process ấy:
...
```
Dispatcher không được khởi động trong file này. Nó hoàn toàn không tồn tại trong
suốt journey, còn platform được kỳ vọng không nhận thấy điều đó: message vẫn đến
end user, trong khi endpoint của khách hàng không nhận được gì.

Cái giá của service split cũng cần được nói rõ; một chương chỉ liệt kê lợi ích
của quyết định do chính nó đưa ra thì chỉ là quảng cáo:

- **Một network hop thay cho function call trước đây.** Mọi thay đổi state mà
  dispatcher thực hiện đều cần một HTTP round trip tới API. Điều này tạo thêm
  latency, một connection pool và một failure mode mới — API không thể truy cập —
  cần được xử lý rõ ràng.
- **Một credential mới phải được quản lý.** Trước đây platform chưa có
  service-to-service authentication; giờ có thêm một secret cần quy trình rotate.
- **Một deployable cần vận hành.** Thêm một image, một health check và thêm một
  component có thể chết. Compose file lớn hơn, CI cũng vậy.
- **Internal API trở thành một contract.** `/internal/dispatch` có hai consumer —
  dispatcher và test — nên thay đổi nó đồng nghĩa phải thay đổi cả hai phía.

Trade-off có đáng hay không phụ thuộc vào property nhận được. Ở đây, property ấy
là isolation giữa server hỏng của một khách hàng và service phục vụ mọi khách
hàng khác; nó đáng giá một network hop. Outbox relay không nhận được lợi ích đủ
lớn như vậy, đó là lý do relay vẫn nằm trong API.

## Internal seam và credential đi qua nó

Dispatcher không được phép ghi PostgreSQL, vì vậy mọi state change đều phải đi
qua API. Điều đó tạo ra một caller type mới và một principal type mới:
...
`InternalModule` phải tự provide `"DB"` thay vì chỉ import module khác; provider
thiếu này đã tiêu tốn trọn một buổi chiều. `MessagesModule` *provide* `"DB"` nhưng
không *export* nó, nên module tưởng rằng mình truy cập được provider thực ra nhận
zero provider. Nest chỉ báo runtime error tại route đầu tiên chạm database, thay
vì wiring error khi khởi động. Unit lane tụt từ 156 test xuống 40 và hai suite
fail hoàn toàn trước khi nguyên nhân lộ diện.

</Trap>

## Relay, một lần nữa

Delivery relay chính là loop của chương 3.3 với thêm một predicate. Chỉ có vậy;
việc không cần abstraction mới chứng minh chương 3.3 đã đặt seam đúng chỗ:
...
+    await ensure(nc);
   ...
   return {
```
Nếu thiếu parameter này, lần publish đầu tiên vào `deliveries.*` sẽ nhận `503` từ
một broker chưa từng được cấu hình để tạo stream tương ứng.

## Dispatcher

Hai consumer đảm nhiệm hai việc khác nhau. Consumer thứ nhất expand event thành
delivery row; consumer thứ hai POST các delivery mà relay xác định đã đến hạn:
...
```
## Các test suite thực sự bảo vệ điều gì

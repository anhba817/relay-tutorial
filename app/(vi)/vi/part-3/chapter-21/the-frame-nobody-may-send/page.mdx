...
@@ -989,3 +989,38 @@ describe("the socket's limits", () => {
...
<ForwardRef>
Chương 3.22 xây trần năm connection của FR-RTM-09. **SAD** — chứ không phải SRS, nơi khoá này
không hề được nhắc tới — mô tả `conn:{env}:{user}` là một **set** Redis với một TTL, mà TTL thì
theo khoá chứ không theo từng phần tử, nên một instance làm tươi khoá sẽ giữ mãi mục của một
instance đã chết. **Và chính dòng đó đã công bố sẵn một cách sửa**, một sorted set tỉa lúc đọc,
khiến việc đầu tiên của chương ấy hẹp hơn một quyết định: nó phải quyết xem có chấp nhận một quyết
định đã nằm sẵn trong cây mã hay không. Nó không chấp nhận, và ADR-23 là lập luận — vì sao mỗi chỗ
có một khoá riêng, và vì sao lựa chọn kia cần một ngôn ngữ mà kho mã này không nhận vào nếu chưa có
bằng chứng nó không thể thu thập.
</ForwardRef>

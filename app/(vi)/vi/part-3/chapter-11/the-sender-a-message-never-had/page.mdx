...
export const metadata = {
  ...
  description:
    "Message từng không có sender. Để mọi message được attributed cần bot identity, thay đổi compile-time trên mọi caller, thứ tự refusal đúng và migration cho các row lịch sử.",
  alternates: {
...
Mọi message phải có sender. Requirement tưởng như mới, nhưng nền tảng từng thỏa nó theo
chiều ngược lại: API key có thể gửi “thay mặt bất kỳ user nào” mà không cần sender thật.

## Requirement đã ở đó, và từng được thỏa theo chiều ngược

Requirement luôn nói message phải có sender, nhưng protocol ban đầu chỉ mang content và channel. Những caller đầu tiên vô tình thỏa requirement bằng cách để database hoặc context ngầm đoán author. Khi bot, REST và socket cùng dùng một write path, sự vắng mặt ấy không còn có thể che giấu.

FR-MSG-13 được amend để backend chỉ gửi thay mặt bot user của tenant; FR-MSG-15 yêu cầu
mỗi message có sender. Sửa câu requirement dễ, nhưng sender chạm identity, schema, caller,
history và socket frame.

<Figure caption="Sender bắt buộc làm thay đổi blast radius của mọi caller." code={figBlastRadius} />
<Figure caption="Refusal order phải giữ resource chưa thấy được indistinguishable." code={figRefusalOrder} />
<Figure caption="Bot identity là trạng thái được mô tả, không phải user absence." code={figAbsence} />

<Why>Compile-time signature là cách tìm toàn bộ caller đáng tin hơn prose.</Why>
<Why>Bot dùng cùng tenant model với user, nhưng không có authentication capability.</Why>
<Why>Historical row cần attribution có thể giải thích được.</Why>
<Why>Refusal order là một phần của isolation contract.</Why>
<Why>Migration chỉ đúng khi caller mới và row cũ cùng có meaning rõ ràng.</Why>

<Trap>Sender optional biến omission của caller thành message hợp lệ.</Trap>
<Trap>Chỉ từ chối lúc mint vẫn để token đã tồn tại một cửa sổ bypass.</Trap>
<Trap>Prose không được compiler hay test runner đọc như contract.</Trap>
...
## Identity từng là sự vắng mặt

`sender_id` phải trở thành một phần bắt buộc của message ở protocol, storage và response. Không nên dùng `NULL` để nghĩa là “server gửi”: đó là một identity thiếu dữ liệu, không phải một loại identity. Bot cần user record thật để history, authorization và audit có cùng shape với người dùng.

Bot không phải user đăng nhập. Hai column trên `users` diễn tả bot identity và description
của nó mà không tạo `bots` table thứ hai cùng nghĩa tenant-scoped.
...
## Compile error, không phải test

Đổi constructor để sender là bắt buộc biến TypeScript compiler thành inventory của mọi caller. Test chỉ chứng minh những path ta nhớ để chạy; compile error liệt kê cả worker, fixture và helper mà chương này phải quyết định identity. Đây là một thay đổi nên để type system ép hoàn thành thay vì vá bằng default im lặng.

`sendMessage` không còn nhận sender optional. Type signature bắt mọi caller cung cấp sender;
đây là proof mạnh hơn test vì không caller nào có thể bị quên mà vẫn compile.
...
## Những guard không còn nghĩa, và guard vẫn còn nghĩa

Khi sender được resolve từ principal, handler không được tin `senderId` do client gửi. Một vài guard cũ trở nên thừa vì caller không còn chọn identity, nhưng guard về tenant, membership, channel và permission vẫn phải chạy trước khi bất cứ row nào được ghi.

Sau khi sender là bắt buộc, các guard từng chặn senderless message trở thành dead branch.
Chỉ những guard còn phân biệt bot, user bị xóa và membership mới giữ lại.
...
## Resolve sender, và thứ tự refusal

Thứ tự refusal là contract: authenticate, resolve user, kiểm tra environment và membership, rồi mới validate payload và insert message. Thứ tự này tránh cho malformed request hay foreign id trở thành oracle về resource không được phép thấy, đồng thời giữ error nhất quán giữa REST và socket.

Sender được resolve trong tenant trước write. Refusal order giữ indistinguishability: foreign
hay absent resource không được lộ qua lỗi sender, archive hoặc membership.
...
## Bot ở mọi nơi trừ cánh cửa

Bot là service identity, không phải credential mà public client được quyền chọn. Worker có thể resolve bot user ở bên trong service; HTTP và WebSocket door chỉ resolve principal đã authenticated. Tách hai đường đi này giữ tiện ích automation mà không mở endpoint mạo danh.

Bot được phép là sender nhưng không authenticate. Mint endpoint, credential checks và gateway
cùng enforce rule để token đã mint trước đó không tạo một cửa sổ bypass.
...
Migration phải xử lý message lịch sử trước khi `sender_id` thành `NOT NULL`. Backfill dùng identity bot hoặc author đã biết theo một quy tắc được ghi rõ, sau đó foreign key và constraint mới có ý nghĩa. Không được thêm constraint rồi hy vọng production không có row cũ.

Migration xử lý historical senderless rows bằng identity có thể giải thích được, thay vì bịa
ra end user. Mọi row mới từ thời điểm này phải có sender thật.
...
## Caller hiện có phải đổi gì

Mỗi caller phải truyền identity đúng: REST lấy từ session, gateway lấy từ socket principal, dispatcher dùng bot service account và test fixture tạo user thực. Sự lặp lại này là bằng chứng message không còn có một đường tắt vô danh.

Socket, internal API và REST caller đều truyền sender explicit. Đây là change contract;
prose không được checker đọc nên type và test phải mang phần enforceable của nó.
...
## Hai mươi file chương này đổi nhưng không dạy

Nhiều file chỉ là propagation mechanical của type mới. Chúng vẫn cần được review, nhưng bài học nằm ở boundary nơi identity được tạo, ở thứ tự guard và ở migration lịch sử; danh sách diff không nên che những quyết định đó.

Các file dưới đây là wiring, generated boundary và test support. Chúng cần cho change nhưng
không bổ sung concept ngoài identity, sender và refusal order.
...
## Cái giá của thay đổi

Sau chương này, mọi message đều có author có thể query, authorize và hiển thị. Cái giá là migration, bot identity và compiler-guided updates; lợi ích là không còn một loại message bí ẩn buộc mọi reader phải đoán sender là ai.

Chi phí là một migration, mọi caller phải compile lại, và test cần phân biệt bot với user.
Đổi lại, message frame, history và fan-out đều có author mà client tin được.
...
- mọi message mới có sender
- bot không authenticate và historical row được migrate rõ nghĩa
- compile buộc mọi caller truyền sender

</Checkpoint>

<ForwardRef part="chương kế tiếp">

Sender đã có; phần còn nợ là message đi từ API tới socket. Chương kế tiếp khép fan-out
edge giữa API, Redis và gateway.

...
export const metadata = {
  ...
  description:
    "Một message do server của khách hàng gửi mà chẳng gọi tên ai. Yêu cầu phải gọi tên một ai đó đã nằm trong SRS từ bản v1, và chương outbox thoả mãn nó bằng cách làm điều ngược lại — thứ đó là quyết định đúng khi chưa gì đọc tới người gửi.",
  alternates: {
...
Server của một khách hàng post một kết quả build vào một channel. Nó xác thực bằng một API key,
bởi nó là một server và chẳng có con người nào đứng sau, và message rơi xuống với `user_id` null.

Suốt mười bốn chương chuyện đó vẫn ổn. Chẳng gì đọc tới người gửi.

Rồi chương channel-control làm cho người gửi quyết định một channel riêng tư có hiện ra hay không, chương user-surface
làm nó quyết định một danh sách channel render ra gì, và đường resume
làm nó quyết định một message có giao được hay không. Một dòng không có người gửi trở thành một dòng mà ba
chương đó không mô tả nổi — và có 19,818 dòng như thế trong lane test, trên tổng 83,806.

Chương này trao cho message đó một người gửi. Phần lớn công việc không phải cái người gửi.

## Yêu cầu vốn đã ở đó, và nó đã được thoả mãn ngược

Kế hoạch cho chương này nói rằng SRS chẳng có khái niệm nào về người gửi của một message và sẽ cần một
lần sửa đổi. Mười ba lượt phân tích đọc câu đó. Rồi một lượt mở §4.5 ra và đọc
các điều khoản thay vì các định danh:
...
*P2, xác minh bằng test, có trên sổ sách từ v1.*

Năng lực ấy đã được đòi hỏi từ khi tài liệu tồn tại. Chương outbox thoả mãn nó
bằng cách gửi không ghi danh — và `messages.controller.ts` đã ghi lại cách đọc đó từ bấy tới giờ, trong
một lời chú thích mà một người đọc sẽ tin ngay: *"Server của chính một tenant gửi thay mặt một khách
hàng là FR-MSG-13, không phải một sai lầm."*

<Why title="A requirement met by doing the opposite is harder to see than one nobody wrote">

Một yêu cầu thiếu thì lộ ra trong một phép kiểm độ phủ: chẳng gì truy vết tới nó. Một yêu cầu
được thoả mãn ngược thì truy vết tới code, có một lời chú thích trích dẫn nó, và qua mọi cổng. Cái cân đo
duy nhất tìm ra nó là một con người đọc điều khoản đó bên cạnh đoạn code tuyên bố nó.

Điều khoản nói *thay mặt bất kỳ user nào*. Code thì chẳng gọi tên ai. Cả hai câu đều chứa
cụm "thay mặt", và từ chương thêm route ấy cho tới chương này, chừng đó là
đủ.

</Why>

Nên phần sửa đổi là hai điều khoản mới và **hai lần sửa vào các điều khoản đã có** — và điều đó
quan trọng, bởi một lần sửa đổi chỉ-thêm sẽ để tài liệu này khẳng định cả rằng một
key có thể gửi với tư cách bất kỳ ai lẫn rằng nó chỉ được gửi với tư cách một bot:
...
## Một danh tính vốn là một sự vắng mặt

Người gửi mà một key gọi tên phải là một cái gì đó. Ba ứng viên, và cái thứ ba là cái được chọn:

<Figure caption="Ba cách gọi tên phần mềm, và mỗi cách tốn gì" code={figAbsence} />

Một **người gửi cho phép null** giữ nguyên sự vắng mặt rồi bắt mọi bên đọc xử lý nó — đó chính là thứ
đang tồn tại và là thứ mà ba chương vừa rồi phải trả giá. Một **user tổng hợp** do
nền tảng tự tạo là lựa chọn khiến một khách hàng mất tiền mà chẳng ai hỏi họ: mức dùng được tính tiền
theo user hoạt động, và một nền tảng bịa ra các danh tính thì đang đặt phát minh của chính nó lên hoá đơn
của người khác. Chương quota sẽ đo đúng cái chiều đó, và đó là lý do quyết định này
thuộc về đây chứ không phải ở đó — một thứ được đo phải là một thứ được khai báo trước đã. Một **bot user** do
khách hàng tạo ra, đặt tên và mô tả thì là một danh tính được khai báo, và khác biệt so với một
user tổng hợp nằm ở chỗ ai quyết định rằng nó tồn tại.

Phần mô tả không phải trang trí. Nó là thứ làm cho người gửi trả lời được: bộ công cụ hỗ trợ
của một khách hàng nói được cái gì đã post và vì sao. Một bot không có mô tả chính là người gửi nặc danh mà chương này
tồn tại để loại bỏ, nên database từ chối nó:
...
Mọi bên đọc dựng từ chương channel-control đều đọc `users`. Một bảng riêng sẽ nghĩa là phải dạy
mỗi bên đọc một chỗ thứ hai để tìm, và `messages.user_id` sẽ phải tham chiếu một trong
hai bảng. Một bot là một user có một thuộc tính được lưu (FR-003 đòi đúng thế), và thứ duy nhất
nó không làm được là xác thực.

`environments.kind` là cột duy nhất tên là `kind` trong schema này mà không có CHECK nào. Nó đã
giữ `development` hoặc `production` từ chương 2.1 và không từ chối nổi một lỗi gõ. Được gọi tên trong
migration chứ không được sửa: một ràng buộc trên một cột mười bảy chương tuổi thì không phải thay đổi của
chương này, còn một sự bất đối xứng không được nhắc tới là thứ mà người đọc kế tiếp mặc nhiên bỏ qua.
...
Migration được đo chứ không được giả định, và câu trả lời tách ra theo một cách mà kế hoạch
không lường trước:
...
"Không cần backfill" là đúng với các cột — 3.4 ms trên 79,149 dòng là chỉ-metadata, đúng như
Postgres 11 đã hứa và chương user-surface đã đo cho `last_activity_at`. Nó **không**
đúng với các CHECK: mỗi cái đều xác thực mọi dòng đang có. Ở kích cỡ này thì đó là chẳng gì; hình dạng
là O(n), và "migration không cần backfill" là cái câu mà ai đó sẽ đem đi trích dẫn.

Bốn con số đó được lấy trên chính cái lane mà chương này được dựng lên, bên trong một transaction rồi
roll back, nên phép đo là trên cái bảng thật chứ không phải trên một bản sao của nó. **Hãy đọc chúng như một hình dạng
chứ đừng bao giờ như một ngân sách** — ba mili giây nói rằng các cột là chỉ-metadata ở mức tám mươi
nghìn dòng và chẳng nói gì hết về bảng của một khách hàng ở mức tám mươi triệu, nơi
CHECK là cái duy nhất trong ba thứ vẫn còn quét.

## Một lỗi biên dịch, không phải một bài test

Quy tắc là mọi message đều có một người gửi. Cơ chế là một ký tự:
...
`userId?: string` thành `userId: string`. Với `exactOptionalPropertyTypes` thì một bên gọi cũng không
truyền `undefined` được — truyền một `string | undefined` thì bị trình biên dịch gọi tên, chứ không được lặng lẽ
chấp nhận. Một lệnh ghi không người gửi thì không build được.

<Why title="There is no failing test for this, and there cannot be">

Mọi bảo đảm khác trong chương này đều có một bài test gỡ bỏ: lấy phép kiểm ra, xem cái gì đó
đỏ, rồi đặt lại. Một bảo đảm ở thời điểm biên dịch thì chẳng có bài test đỏ nào — gỡ ràng buộc đi và
mọi thứ vẫn biên dịch, đó là sự vắng mặt của bằng chứng chứ không phải bằng chứng.

Nên bằng chứng là bản ghi lại lần gỡ nó ra. Hoàn nguyên cái `?` sẽ gọi tên mọi chỗ gọi
sẽ thiếu người gửi, và bản kiểm đếm đó chính là hình dạng của cái bảo đảm nhìn từ bên ngoài.

</Why>

Một lệnh grep ngây thơ cho `sendMessage(` trả về **71** chỗ trên mười sáu file. Làm `userId`
thành bắt buộc rồi chạy trình biên dịch thì gọi tên **27** chỗ, trong tám file, mỗi chỗ đều nằm trong
`@relay/api` — gateway và gói đầu-cuối đã niêm phong thì với tới cái này qua HTTP và chẳng
nhìn thấy được cái kiểu chút nào.

Hai mươi sáu trên hai mươi bảy chỗ là cùng một lỗi, `TS2345`: một bài test gọi `sendMessage` với
`{ text }` và không có người gửi. Chỗ thứ hai mươi bảy là `TS2379`, một **mã khác**, và nó là
`messages.service.ts` — bên gọi duy nhất trong code sản phẩm nằm trong danh sách. Nó không bỏ sót `userId`; nó
truyền `string | undefined`, thứ mà `exactOptionalPropertyTypes` từ chối cũng dứt khoát y hệt.

Sự bất đối xứng đó mới là phần hữu ích. Một bản kiểm đếm đếm các chỗ *bỏ sót* một thuộc tính thì không nhìn thấy
cái chỗ truyền `undefined`, mà cái chỗ truyền `undefined` lại là chỗ duy nhất mà
cách sửa là một quyết định chứ không phải một fixture. **Trình biên dịch trả lời một câu hỏi mà lệnh grep
không thể được hỏi.**

<Figure caption="Lệnh grep đếm ra gì, và trình biên dịch gọi tên gì" code={figBlastRadius} />

## Những cái guard thôi còn ý nghĩa, và cái không thôi

Làm một tham số thành bắt buộc sẽ khiến các guard lúc chạy của nó thành chết. `sendMessage` có một cánh cổng quanh
phép kiểm lệnh cấm mà tồn tại chỉ vì một lần gửi bằng key chẳng mang user nào: không user, chẳng ai để mà bị cấm. Nó
trở nên không với tới được ngay khoảnh khắc cái kiểu đổi. Đây là lần thứ tư dự án này gặp một guard thôi
còn ý nghĩa — hai cái trong `upsertUser` ở chương user-surface, và `rowCount ?? 0` của
`addMember`, thứ đã mang một lời chú thích nói rằng nó biến mất từ chương ấy và mãi
tới giờ mới thực sự được gỡ đi.

Kế hoạch nói hãy grep cái pattern đó thay vì sửa một trường hợp đã biết. Bản năng tốt.
Lệnh grep trên `repository.ts` ở tag của chương trước trả về **bốn** chỗ, và chúng không phải
đều mang cùng một ý nghĩa:
...
Một trong bốn cái là chết, một cái trông có vẻ chết mà không phải, và hai cái là những hàm mà bên gọi thực sự
có thể không có user. **Một quy tắc đối xử với cả bốn như nhau sẽ chỉ đúng với một trong số
chúng.**

<Trap title="Sweeping the pattern would have deleted a capability the channel-control chapter shipped">

Dòng 1732 đọc `if (channel.type === "private" && userId !== undefined)`. Một lần gửi bằng key bỏ qua
phép kiểm membership bởi chẳng có user nào — và đó chính là FR-005 của chương channel-control, được giao có
chủ đích: *"Một application credential hành động thay cho khách hàng, chẳng mang user nào, và thấy được các
channel riêng tư."*

Bắt buộc `userId` thì nửa thứ hai luôn đúng, phép kiểm nổ, và một bot không phải
thành viên thì bị từ chối với một 404 mà theo thiết kế thì không nói được vì sao. `messages.itest.ts` có một bài test
khẳng định điều ngược lại, đích danh. Mọi tích hợp hiện có post vào một channel riêng tư sẽ
bắt đầu đỏ.

Quy tắc không phải là *cái pattern đó đã chết*. Nó là *`userId` là bắt buộc trong `sendMessage` và chưa ở
đâu khác* — và một phép tổng quát hoá cần bước xác minh của riêng nó. Lệnh grep được viết vào
kế hoạch và mãi ba lượt sau mới được chạy.

</Trap>

Thứ mà cú suýt nữa ấy tạo ra chính là sự phân biệt mà phần còn lại của chương dựa lên:

**Người gửi ghi danh; nó không phân quyền.** Token của một con người làm cả hai cùng lúc, và đó là lý do
chẳng gì từng cần gọi tên tách chúng ra. Một key gọi tên một bot có đúng thẩm quyền mà
key ấy có hôm nay; tên của bot là thứ xuất hiện trên message và không gì hơn. Nên cánh cổng
membership bật theo *người gửi là cái gì*, chứ không theo việc có một người gửi hay không — và một người không phải
thành viên thì vẫn bị từ chối, một cách không phân biệt được.

## Phân giải người gửi, và thứ tự của các lời từ chối
...
Một user token ghi danh cho chính subject của nó và không được gọi tên ai khác — một token vừa là một
sự phân quyền vừa là một sự ghi danh, nên một trường `user` trong body đặt cạnh nó thì hoặc là một sai lầm hoặc là một mưu toan
post với tư cách người khác. Một application credential chẳng mang user nào của riêng nó, nên trường `user` trong body
là thứ duy nhất có thể gọi tên một user, và gọi tên chẳng ai thì bị từ chối với field được gọi tên.

Rồi tới lời từ chối cần một mã riêng của nó:
...
`ProtocolErrorFilter` ánh xạ một 403 trống thành `forbidden`, và đây là mã duy nhất trong chương
va vào cái thang đó. `codes.ts` đã lập luận chuyện này hai lần rồi —
`wrong_credential_type` là sai *lớp* credential, `wrong_credential_service` là sai
*service*, còn đây là đúng lớp mà gọi tên sai *loại user*.

"Bạn thiếu một quyền" và "một key được nói với tư cách phần mềm chứ không phải với tư cách bất kỳ con người nào" là hai
sự thật khác nhau, và chỉ cái thứ hai mới nói cho một người tích hợp biết phải đổi gì. Lời của chính registry: *một
client không phân biệt được chúng thì sẽ thử lại sai cái mãi mãi.*

Thông điệp chẳng gọi tên con người được hỏi tới lẫn các bot lẽ ra sẽ chạy được. Những
định danh nào tồn tại trong một tenant chính là thứ mà sấm truyền cô lập tồn tại để giữ ra ngoài một lời từ chối.

</Why>

Hợp đồng cho route này ghi tài liệu năm lời từ chối theo thứ tự: cấm, hiển thị, lưu trữ, rồi
người-gửi-có-phân-giải-được, rồi credential-này-có-được-gửi-với-tư-cách-đó. **Thứ tự đó không đạt được như đã
viết**, và việc tìm ra vì sao đã đổi chỗ mà phép kiểm cuối cùng sống.

<Figure caption="Năm lời từ chối, và vì sao việc phân giải người gửi không thể đứng thứ tư" code={figRefusalOrder} />

Phép kiểm lệnh cấm đọc dòng của người gửi — một lệnh cấm áp lên *người gửi được gọi tên*, chứ không lên bên gọi,
và đó là thứ làm cho việc cấm một bot có ý nghĩa. Nên người gửi phải được phân giải xong trước khi
phép kiểm đầu tiên chạy được. Phân giải là bước số không, không phải bước bốn.

Thứ sống sót là phần mà hợp đồng thực sự lập luận cho: **phân giải trước phép kiểm loại**,
để một lời từ chối gọi tên một sự thật về một user không thể bị khiêu khích ra cho một channel mà bên gọi
vốn không với tới được. Hai thứ đó không còn liền nhau — phân giải là bước số không còn phép kiểm loại
là bước thứ tư trong năm, trên lệnh lưu trữ một bậc:
...
Và đó là thứ quyết định phép kiểm loại sống ở đâu. Kế hoạch nói là service chứ không phải repository,
bởi repository không được biết một credential là gì. Nhưng service chạy *trước*
repository, nên thực thi nó ở đó sẽ đặt nó trước lệnh cấm, phép kiểm hiển thị và lệnh lưu trữ
— và làm rò rỉ đúng cái thứ mà thứ tự kia bảo vệ. Nó nằm trong repository, được bảo `senderMustBeBot`:
một ràng buộc, không phải một lớp credential. Repository vẫn không biết một credential là gì.

## Một bot ở mọi nơi trừ ở cửa

Một bot là một user, nên nó thừa hưởng mọi thứ khoá theo user: membership kèm một vai trò, một danh sách
channel, một con số chưa đọc, một lệnh cấm, một lần xoá giữ lại các message vẫn ghi danh nó. Thứ nó không
làm được là xác thực:
...
Một token sống tới 24 giờ. Một user được thăng thành bot lúc 09:00 vẫn giữ một token hợp lệ tới 09:00
ngày mai, còn route session thì đọc `banned_at` mà không đọc `kind`. Đóng lệnh mint mà
không đóng session thì cũng cùng hình dạng với một lệnh cấm chỉ có hiệu lực ở lần connect kế tiếp.
...
Đây cũng là lời từ chối duy nhất trong chương không thể làm cho không-phân-biệt-được. Ở mọi
chỗ khác, một lời từ chối cho một định danh lạ thì giống từng byte với một lời từ chối cho một định danh chẳng
tồn tại ở đâu. Trên lệnh mint, một định danh chưa biết trả lời **200 kèm một token** — chương user-surface làm nó
tạo ra cái dòng — nên chẳng có gì để một lời từ chối giống với. Bất kỳ lời từ chối nào cũng nói
"định danh này tồn tại và không phải một con người". Chỗ rò đó không đóng được trên route này, và
nói ra thì hơn là một khẳng định giả vờ ngược lại.

## Những dòng có từ trước

19,818 message trong lane này chẳng có người gửi, trải trên 1,047 environment. Quy tắc của chương
nói về các lệnh ghi mới; những dòng đó ở lại, và FR-012 đòi rằng cả bốn đường đọc vẫn chạy được với
chúng. Yêu cầu đòi thêm một thứ nữa: rằng câu trả lời phải *như nhau* trên cả bốn.

Nó không thể như nhau, và lý do nằm ở các hợp đồng chứ không phải ở code:
...
`messageSchema.user` là `z.string().min(1)` — một frame không mang nổi một giá trị null, và đổi
điều đó sẽ làm vỡ mọi client đã xuất bản. Nên thứ như nhau trên cả bốn là *quyết định*:
**đừng bao giờ bịa ra một người gửi.** Ở đâu một hợp đồng diễn đạt được "chẳng ai" thì nó nói null; ở đâu không được,
thì dòng đó không được giao. Đó là một câu yếu hơn thứ yêu cầu đòi hỏi và nó là câu
đúng.

`MessageCreatedData.user` vẫn là `string | null` vì cùng lý do. Chẳng gì tạo ra được một
null mới — trình biên dịch cấm — nhưng một lần thử lại webhook chạy tới hai giờ, và hàng đợi thì
đã đầy sẵn khi quy tắc đổi. Thu hẹp cái kiểu sẽ là khẳng định rằng một thứ đang
bay thì không thể xảy ra.

Đó là tuyên bố duy nhất trong chương này có một bài test buộc phải *đỏ* nếu ai đó siết cái
kiểu lại sau này, nên nó có một bài như thế. `FR-WHK-02` giao `message.created` tới endpoint của chính một khách hàng và
`FR-WHK-03` thử lại trong hai giờ: một dòng cũ có thể rời khỏi nền tảng, rồi rời đi lần nữa, sau khi
chương này ship.

```ts title="services/api/src/outbox/event.test.ts"
...
Chương isolation harness ghi lại rằng một message gửi bằng REST chẳng tới socket nào, và liệt kê hai nguyên nhân
độc lập: chẳng gì publish, và lệnh gửi chẳng truyền user nào. Trao cho mọi lệnh gửi một người gửi sẽ gỡ bỏ
nguyên nhân thứ hai. Đã đo: việc giao trực tiếp vẫn chẳng tới đâu, bởi chỉ gateway mới publish vào
fan-out — nhưng **lần resume giờ giao được nó**, bởi `toFrame` chẳng còn lý do gì để bỏ cái dòng.

Bài test ghim cả hai cùng lúc giờ đỏ ở đúng nửa đã đổi, và đó là cách người ta phát hiện ra
lỗ hổng đã được đóng một nửa. Chương fan-out là nửa còn lại.

</Why>

## Một bên gọi hiện có phải đổi gì

Đây là một thay đổi phá vỡ với một route đã ship ở chương 2.2, và gọi nó bằng bất cứ tên nào khác cũng sẽ
là không trung thực. Một application key post tới `POST /v1/channels/:channelId/messages` mà không
gọi tên một `user` thì nhận **400** ở chỗ nó từng nhận 201. Cách sửa là hai request: tạo
một bot một lần, rồi gọi tên nó ở mọi lần gửi.

Lệnh gửi của một user token thì không bị ảnh hưởng — nó chẳng gọi tên ai và được ghi danh cho chính subject của nó, đúng như
trước đây. Cú vỡ rơi hoàn toàn lên các bên gửi xác thực bằng key, và đó chính là cái lớp mà yêu cầu
luôn nói tới.

Ba mươi mốt file trong repository nền tảng đã đổi để điều này thành đúng, và mỗi file đều được
fence — trong chương này, bởi chẳng chương nào khác nhận chúng. Hai mươi trong ba mươi mốt file nằm
bên dưới, trong một khối, chẳng có gì được nói về chúng.

<Trap title="No checker in this repository reads prose">

`check:fences` phát lại code đã fence lên nền tảng. `check:figures` khẳng định một hình có một
nguồn. `check:docs` so mỗi tài liệu được soi gương với bản chuẩn của nó. `check:srs` đọc các
định danh. **Một câu đã xuất bản khẳng định một điều sai thì qua được cả bốn.**

Chương này tìm ra một câu như thế của chính nó. Một lời chú thích trong `repository.ts` nói rằng chương user-surface
đã gỡ `(inserted.rowCount ?? 0)` của `addMember` "vì đúng lý do này" — mà nó chưa
hề được gỡ. Cả hai nhánh SQL đều đã mang `RETURNING`, bài test cho chúng vẫn đọc
`rowCount`, và cái câu mô tả lần gỡ bỏ ấy đã ship từ một chương rưỡi trước. Mọi cổng
đều xanh suốt: chuỗi fence so các byte của lời chú thích với repository và
chúng khớp, bởi một lời chú thích mô tả sai thứ thì vẫn giống hệt chính nó từng byte.

Thứ tìm ra nó là việc viết một câu *thứ hai* về cùng đoạn code rồi nhận ra câu thứ nhất
không thể đúng. Cách sửa nằm trong chương này — cái nhánh giờ biến mất, và lời chú thích lập luận cho
phản bác thay vì ghi công cho ai đó đã hành động theo nó. Lý do được nêu của chương channel-endpoints
cho một đường nối không còn đứng vững cũng được sửa ở đây, theo cùng cách: bởi một con người đọc,
bởi chẳng có cân đo nào cho chuyện đó.

Và một bộ kiểm trong repository này tìm ra được một cách để chẳng báo gì cả. `driver-exempt.test.ts`
khẳng định rằng danh sách miễn trừ của quy tắc driver khoét ra đúng một thư mục và gọi tên mọi
file khác theo đường dẫn — một phép kiểm đáng có, bởi một pattern sẽ lặng lẽ hút luôn file kế tiếp thêm vào
dưới nó. Nó định vị danh sách bằng cách đếm ngược cố định 2,000 ký tự từ chỗ quy tắc. Chương
này thêm sáu dòng chú thích vào danh sách ấy; danh sách dời tới mốc 2,027 ký tự ngược lên, tức 27
ký tự ngoài cửa sổ; phép tìm thấy danh sách *kế tiếp* thay vào đó rồi báo `[]`. **Một kết quả rỗng
đã thoả mãn một khẳng định về việc mục nào là glob.** Giờ nó quét ngược từ chỗ quy tắc, và
từ chối xanh trên một lần parse chẳng tìm ra mục nào cả.

```ts title="packages/test-harness/src/driver-exempt.test.ts"
...
## 20 file mà chương này đổi và không dạy

Mọi chỗ gửi trong workspace đều phải gọi tên một người gửi, và phần lớn chúng là test. Một fixture
có thêm một bot thì không phải một bài học, nên chẳng thứ nào trong đó đáng một đoạn văn xuôi — nhưng chuỗi fence chẳng
quan tâm vì sao một file đổi. Trạng thái của một đường dẫn được tuyên bố phải bằng trạng thái của repository, nên chúng ở đây
trong một chỗ chứ không rải rác qua các phần mà chẳng có gì để nói về chúng.

Chương user-surface gặp cùng hình dạng này từ phía bên kia, và trong thứ tự này thì nó không
còn gặp nữa. Trong trình tự mà cuốn sách này được viết lần đầu, mười một file của nó đổi đúng
một từ — một trích dẫn được sửa — và điều đó cho nó mười một fence chẳng có chủ đề. Quy ước
đặt tên đã xoá bỏ cái commit tạo ra chúng, và các fence đi theo nó. Thứ nằm
bên dưới là hình ảnh ngược của những cái đó: các chủ đề mà người đọc không cần, trên các đường dẫn mà chuỗi
vẫn phải kế toán cho.
...
```ts title="services/api/src/messages/history.itest.ts"
import { beforeAll, describe, expect, it } from "vitest";
...
```ts title="services/api/src/messages/idempotency.itest.ts"
import { afterAll, beforeAll, describe, expect, it } from "vitest";
import { randomUUID } from "node:crypto";
...
**Năm lần chạy, không phải hai mươi sáu, và sự khác biệt đáng được nói ra chứ không phải giấu đi.** Năm là
đủ để nói rằng lane hôm nay không chập chờn và còn xa mới đủ để bắt một flake nổ
một lần trên hai mươi. Thứ mà năm lần chạy có xác lập được là một độ biến thiên nhỏ tới mức đáng ngạc nhiên:
139.10 tới 139.30 giây, độ lệch chuẩn **0.07 s**, hệ số biến thiên
0.05%. Một lane lặp lại tới mức một phần mười giây là một lane làm cùng một khối lượng công việc mỗi lần.

Con số đó không so được với con số của bất kỳ chương nào khác, và lý do không phải nhiễu. Lane này chạy
23 suite; các suite webhook, quota, đo-kết-nối và mail thì chưa có ở đây, và gói người ngoài
đã niêm phong cũng chưa. **Một lane nhanh hơn không phải một nền tảng nhanh hơn — nó là một
cân đo nhỏ hơn**, và mọi phép so sánh xuyên các chương này đều phải nói rõ nó muốn nói cái nào trong hai.

Con số DUY NHẤT so được là ratchet nhánh, bởi nó là một tỉ lệ đo trên một
file. `repository.ts` đọc ra 92.66% hai lần, cách nhau hai mươi phút, giống hệt nhau tới cả số dòng
chưa phủ — nên cái ghim dời tới 92 với 0.66 khoảng dư. Hai lần đọc chứ không phải một, bởi
dự án này đã chứng kiến `session.ts` báo 87.80% và 85.36% trên code không đổi, và một ratchet
ghim vào một lần đọc may mắn duy nhất là một ratchet dạy người đọc kế tiếp cách hạ ratchet.

<Checkpoint>

Server của một khách hàng gọi tên phần mềm đã post. Cái tên là một dòng user có một mô tả,
mô tả là bắt buộc theo database, và một message không có người gửi thì không biên dịch được.

</Checkpoint>

<ForwardRef>

Message vẫn chẳng tới được socket sống nào. Api chẳng publish vào fan-out nào — đó là nửa của
lỗ hổng mà chương isolation harness nêu và chương này không đóng, còn chương fan-out mới là phần publish.

Presence là động từ thứ ba của FR-CHN-05 và nó chưa bao giờ được dựng. Điều khoản có ba: đọc, gửi,
quan sát presence. Hai trong số đó đã đúng trước chương này và vẫn đúng sau nó. Chương presence là cái
thứ ba.

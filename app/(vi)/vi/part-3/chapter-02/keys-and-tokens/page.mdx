...
export const metadata = {
  title: "Key và token — Building Relay",
  description:
    "Hai loại credential được phân biệt bằng prefix: API key có secret chỉ hiện một lần và được hash khi lưu, cùng end-user token do API verify bằng signing secret của environment.",
  alternates: {
    canonical: "/vi/part-3/chapter-02/keys-and-tokens",
    languages: {
...
<ChapterHeader id="3.2" locale="vi" />

Chương tenancy đã xây những container chứa thế giới của khách hàng và flow signup tạo
ra chúng. Nó chưa xây cách để *chứng minh* bạn sở hữu một container. Mọi request từ 2.2
đều gọi tên tenant bằng header do chính request tự khai:

```text
x-relay-environment: 3f2a…
```

Bất kỳ ai chạm được tới API đều có thể gõ header đó, gọi tên environment bất kỳ rồi đọc
data của nó. Khi ấy tôi gọi đây là dev-mode seam và hứa Part 3 sẽ thay thế nó. Đây là
chương thực hiện lời hứa, và seam không sống sót qua chương này.

Còn một seam thứ hai cùng tuổi. Gateway đã tự verify WebSocket token bằng HS256 với
`RELAY_DEV_JWT_SECRET` dùng chung mà mọi service và test đều biết. Hai seam chết trong
hai increment riêng vì blast radius khác nhau; tôi muốn bạn thấy cả hai con số.

Thứ thay thế chúng là hai credential — nhưng chủ đề thật của chương không phải bản thân
credential nào. Chủ đề là sai lầm mọi người thường mắc với cặp đó.

<SkipAhead>

Nếu buộc phải bỏ qua: table `api_keys` được thêm dưới tenant boundary, chứa public lookup
id, salted SHA-256 của secret chỉ hiện đúng một lần, và prefix `rk_dev_`/`rk_live_` phản
ánh environment kind. Authentication chạy trong **middleware** — đã được đo, vì Nest
khởi tạo request-scoped provider trước guard — và tạo một `principal` mang
`environmentId`; repository factory giờ đọc giá trị này thay vì header. End-user token
được **API** verify, không phải gateway: gateway gửi token tới `POST /internal/session`
và nhận lại caller là ai cùng những gì họ được nghe. Call này thay memberships call vốn
đã tồn tại, nên connect path không thêm round trip. Signup trao key đầu tiên của
environment, vì khi chưa có console session thì không còn cách nào bootstrap key. Trạng
thái cuối chương là tag **`part3-ch2`**.

</SkipAhead>

## Hai population, hai credential

Chương tenancy vạch một đường qua data: `humans` phía trên tenant boundary là người đăng
nhập Relay; `users` phía dưới là người trong sản phẩm của khách hàng; hai nhóm không bao
giờ gộp (ADR-18). Đường đó kéo theo một đường thứ hai. Có hai loại caller và họ yêu cầu
hai điều khác nhau.

Một **application** — backend của công ty, giữ credential trong environment variable —
nói *tôi là environment này*. Nó gửi message thay bất kỳ ai vì nó chính là khách hàng.
Một **end user** — tài xế trong xe hay browser tab — nói *tôi là người này trong
environment kia*. User chỉ được hành động với tư cách chính mình.

Một credential không thể thành thật mang cả hai claim, vì vậy ta cần hai.

<Figure
  caption="Hai credential class, nơi mỗi loại được trình ra, và hai lời từ chối tạo nên chủ đề của chương này."
  code={figTwoCredentials}
/>

Cả hai resolve về một shape duy nhất — đó là điểm mấu chốt. Authentication tạo
`principal`; principal mang `environmentId`; mọi thứ downstream đọc đúng field đó. Hai
class chỉ khác ở hai nơi: mỗi class được phép chạm tới đâu và chuyện gì xảy ra khi ai
đó nhầm lẫn chúng.
...
## Handle có thể lookup, secret thì không

API key phải giải bài toán không query nào khác trong codebase gặp phải. Mọi repository
method từ 2.1 bắt đầu bằng environment; method này lại *tạo ra* environment. Authentication
xảy ra trước khi tenant scope tồn tại, vì vậy key lookup là unscoped query duy nhất của
hệ thống.

Điều đó loại bỏ thiết kế hiển nhiên. Nếu hash toàn bộ credential, sẽ không còn gì để
lookup và mỗi request phải scan mọi key trong platform. Vì vậy credential gồm hai phần:
public handle có index và không bí mật; secret được hash và không bao giờ lưu nguyên bản.
...
Prefix là requirement, không phải trang trí. FR-AUT-03 yêu cầu key mang environment
marker nhìn thấy được. Lý do là support ticket không ai muốn viết: thấy `rk_live_` trong
file config staging là nhận ra lỗi ngay; opaque blob là lỗi chỉ phát hiện về sau.

<Trap>

Tôi từng viết parser split theo underscore cuối cùng vì design note nói vậy. Key đầu tiên
vừa mint đã parse thất bại.

Cả hai nửa đều dùng base64url, mà **alphabet của base64url có `_`**. Secret chứa
underscore khiến “split ở separator cuối” sai; public id chứa underscore khiến “split
ở separator đầu” sai. Không có điểm split an toàn khi cả hai vế đều có thể chứa delimiter.

Cách sửa là loại delimiter khỏi một alphabet: public handle dùng hex, không có `_` và
có chiều dài cố định, nên boundary luôn chính xác bất kể secret chứa gì. Chi phí là một
character class và một test.

Design document nói “split ở separator cuối”. Lần mint đầu tiên chứng minh điều ngược
lại. Đây là lần thứ ba trong series một artifact Phase 1 được sửa bằng cách chạy code;
việc sửa rẻ chính vì xảy ra trước khi bất cứ thứ gì được publish.

</Trap>

<Why source="NFR-SEC-02 · FR-AUT-02 · research R3">

**Tại sao dùng salted SHA-256 khi mọi người đều nói bcrypt?**

Vì “luôn dùng password KDF” là lời khuyên cho *password*, còn đây không phải password.

bcrypt và argon2 cố ý chạy chậm. Sự chậm đó mua đúng một thứ: khả năng chống đoán secret
có entropy thấp — password do con người chọn từ không gian đủ nhỏ để search. Với password,
đó là lựa chọn đúng và là lý do không bao giờ lưu password bằng SHA-256.

Key secret ở đây là 32 byte từ `crypto.randomBytes`: 256 bit phân bố đều. Không work
factor nào khiến nó dễ đoán và cũng không work factor nào khiến nó “khó đoán hơn”. KDF
không mua được gì, nhưng tạo độ trễ có chủ ý trên **mọi authenticated request**, cộng
thêm native dependency, ngay trong code path chạy trước tất cả.

Vì vậy ta dùng SHA-256 trên random salt riêng cho từng key và so sánh bằng
`timingSafeEqual`. Salt xứng đáng có column riêng vì khiến hai secret giống nhau có hash
khác nhau. NFR-SEC-02 yêu cầu salted hash và đây đúng là salted hash — requirement được
thoả bằng cách đọc nó, không phải chọn công cụ nghe có vẻ mạnh nhất.

Cạm bẫy thật nằm cạnh đó, ở phép so sánh chứ không phải hash: `timingSafeEqual` throw
khi hai buffer khác độ dài. So sánh *secret* cho phép caller làm crash endpoint bằng
chuỗi ngắn; so sánh *hash* thì cả hai phía luôn là 32 byte.

</Why>

Table lưu key mang environment giống mọi thứ khác dưới boundary. Điều nó không mang là
constraint giới hạn số key của environment — nhiều key active cùng lúc là feature chứ
không phải thiếu sót, vì rotation không downtime cần đúng khả năng đó.

## Secret chỉ nhìn thấy một lần

Organisation vừa tạo chưa thể authenticate request để xin key đầu tiên. Không có console
session — chương tenancy chủ ý chưa xây — nên phải có thứ bootstrap credential, và signup
là hành động duy nhất đã biết bạn là ai.

Vì vậy `provisionOrganisation` mint key đầu tiên của environment trong cùng transaction
với tenant chứa nó; callback trả plaintext đúng một lần. FR-DSH-01 muốn development key
trên màn hình đầu tiên sau signup; key đó đến từ đây.
...
Walk từ chương tenancy giờ cho thấy nó trao gì — và quan trọng hơn, lần thứ hai nó
*không* trao gì.

Owner quay lại không nhận secret mới. Secret cũ không thể khôi phục by construction;
câu trả lời cho key bị mất là rotation, không phải retrieval — câu trả lời chỉ khả dụng
vì nhiều key có thể active cùng lúc.

## Token do API ký còn gateway thì không thể

End-user token được ký bằng `signing_secret` riêng của environment. Column đó nằm trong
Postgres, còn ADR-05 quy định gateway không bao giờ chạm Postgres.

Có hai lối: gửi signing secret của mọi environment tới gateway, hoặc hỏi service sở hữu
chúng. Gửi secret đồng nghĩa đặt tenant-scoped secret vào process chủ ý không giữ tenant
state, nhân lên trên mọi gateway instance, cộng thêm bài toán rotation mỗi khi secret
đổi. Hỏi service tốn một HTTP call.

Gateway vốn đã thực hiện call đó. Từ 2.5, lúc connect nó hỏi API “user này được nghe
channel nào?”. Thay câu hỏi bằng “đây là ai và họ được nghe gì?” tốn cùng một round trip,
đồng thời chuyển identity decision tới service duy nhất có thể đưa ra quyết định.

<Figure
  caption="Connect sau chương này. Gateway chuyển tiếp token được đưa vào và nhận lại caller là ai — vẫn đúng một call cũ, nhưng đặt câu hỏi tốt hơn."
  code={figConnect}
/>

<Trap>

JWT vulnerability nổi tiếng nhất không phải signature hỏng, mà là hỏi token cách tự
kiểm tra chính nó.

Header JWT tự gọi tên algorithm. Verifier đọc field đó rồi làm theo sẽ chấp nhận
`{"alg":"none"}` — token có signature rỗng mà ai cũng viết được. Đổi sang `RS256` trước
verifier đang đợi HMAC, một số library còn vui vẻ dùng public key làm HMAC secret, trong
khi key đó là public.

Phòng thủ chỉ là một argument: danh sách algorithm được phép. `jose` từ chối mọi thứ
ngoài danh sách trước khi đọc key material. Test chứng minh việc này tự dựng unsigned
token vì library không cho mint token như vậy.

Đây là toàn bộ lý do chương này thêm dependency sau khi chương tenancy cố tránh thêm
bất kỳ dependency nào. Tự viết HS256 mất khoảng ba mươi dòng và dính ba lỗi kinh điển:
lỗi này, quên `exp`, và so sánh signature bằng `===`. Convenience có thể đáng để tự
type. Security code thì không.

</Trap>

Thứ tự operation có một điểm tinh tế. API phải biết dùng secret của *environment nào*
để verify signature, trong khi thông tin duy nhất nằm trong token — chưa verify. Vì vậy
environment claim được đọc trước, không tin cậy, chỉ để chọn key; signature sau đó quyết
định có tin claim hay không. Đọc claim *sau* verify thì không thể verify; dùng claim cho
bất kỳ việc gì khác thì token environment A có thể đi vào environment B.

Route ghép mọi thứ lại thay thế hoàn toàn `GET /internal/memberships`.

Gateway door trở thành một cuộc gọi.

Type đó nói ba outcome chứ không phải hai, vì chuyển verification sang API tạo thêm
failure gateway chưa từng có: verifier bị *down*. Token bị từ chối và API không reachable
đều không mở socket, nhưng trả lời giống nhau sẽ nói dối theo một hướng. 4001 bảo client
credential sai nên đừng retry; 1011 bảo hệ thống hỏng nên hãy retry. Chương 2.5 đã vạch
ranh giới này cho memberships lookup; chuyển lookup không được âm thầm xoá nó.
...
## Authentication chạy ở đâu — và tôi biết bằng cách nào

Chương 2.6 khiến tôi mất một buổi chiều để biết NestJS dựng request-scoped provider
**trước** enhancer chain. Vì vậy repository factory từ 2.2 tự đọc request thay vì tin
guard đã đặt dữ liệu vào đó. Phát hiện này ràng buộc chương hiện tại: authentication giờ
có database lookup, còn guard vẫn chạy quá muộn cho factory cần kết quả.

Middleware chạy sớm hơn. Nhưng “chạy sớm hơn” là điều tôi tin chứ chưa đo trên path này,
nên trước khi xây dựa trên nó tôi log một dòng ở mỗi trong ba nơi rồi gửi một request.

Kết quả: middleware, factory, rồi guard. Design đứng vững, fallback đã gọi tên không
cần dùng, phép đo mất bốn phút.

<Figure
  caption="Thứ tự được đo thay vì giả định. Authentication phải xong trước khi factory đọc scope; công việc của guard hẹp hơn trước."
  code={figAuthOrder}
/>
...
Guard còn lại nhỏ hơn guard nó thay thế. `EnvironmentContextGuard` resolve tenant từ
header; `CredentialGuard` không resolve gì. Nó hỏi đúng một câu — credential class này
có được dùng route này không? — và tạo hai lời từ chối là chủ đề của chương.

## Sai lầm SRS đã dự báo

SRS có một design note dưới các authentication requirement, cụ thể khác thường:

> Nhầm lẫn API key với user token là lỗi first-integration phổ biến nhất. FR-AUT-03 và
> FR-AUT-09 tồn tại riêng để giảm lỗi này; error message khi dùng sai credential type
> phải gọi đích danh sai lầm.

Một `401 unauthorized` chung chung thoả status code nhưng hoàn toàn không thoả requirement.
Người đang giữ credential hợp lệ nhưng trình nó ở sai cửa chẳng học được gì từ câu “bạn
chưa được authorize” — họ sẽ kiểm tra secret, rotate key và đọc lại docs, trong khi
không việc nào là vấn đề thật.

Vì vậy class sai có code riêng và message gọi tên cả hai phía.

Hãy để ý message *không* chứa gì: nó gọi tên **class**, không bao giờ gọi credential.
“Key `rk_dev_abc…` không hợp lệ” là cách live secret đi vào support ticket, screenshot
và log aggregator; NFR-SEC-06 cấm đúng việc đó. Một test grep mọi log line và error body
tạo ra trong batch failure có chủ ý, rồi fail nếu bất kỳ nơi nào chứa secret.

Walk cho thấy hai credential hoạt động và cả hai lời từ chối được giữ vững.

Endpoint giữa transcript là FR-AUT-09. Nó tồn tại vì nếu không có nó, authenticated
message đầu tiên của developer bị chặn sau việc tự implement JWT minting. Endpoint nhận
API key và trả token, chỉ tồn tại trong `development`; ở production nó trả `404` thay
vì `403`, có chủ ý. 403 nói *bạn thiếu permission*, mời người đọc đi tìm permission để
mở khoá. 404 nói *route này không tồn tại ở đây* — đó mới là sự thật.

<Why source="NFR-SEC-06 · EIR-WS-05">

**Tại sao token của socket nằm trong query string khi credential không bao giờ được ở
trong URL?**

Vì browser không thể đặt header trên WebSocket upgrade. Constructor `new WebSocket(url)`
nhận URL và không gì khác — không header, không body. Mọi realtime product chạy trong
browser đều có vấn đề này; các lựa chọn thành thật là query parameter, cookie, hoặc
first frame sau khi socket mở.

Ta dùng query parameter (shape EIR-WS-05, không đổi từ 2.5) và cần nói thẳng thay vì để
nó giống mâu thuẫn. Các mitigation khiến lựa chọn này bảo vệ được: token có lifetime
ngắn by construction (tối đa 24 giờ, FR-AUT-07), chỉ trao quyền của một end user, và là
*nơi duy nhất* trong hệ thống credential xuất hiện trong URL. Mọi nơi khác dùng bearer
header; API từ chối credential trong query string đơn giản bằng cách không bao giờ đọc
nó từ đó.

<ForwardRef>Phương án first-frame — mở socket chưa authenticate rồi authenticate bằng
frame — cũng là thứ protocol cần để client trao token mới ngay giữa connection. Đó là
vế thứ hai của FR-AUT-11 và sẽ đến cùng các chương SDK ở Part 5.</ForwardRef>
...
## Cả hai seam đều biến mất

Đầu tiên là header. Loại bỏ nó là một increment vì seam mới retire một nửa còn tệ hơn
cả hai trạng thái; blast radius được đo thay vì đoán: **mười một file**, trong đó ba file
là production code path, còn lại là suite gửi header hoặc các identity header đi kèm.

Shared development secret của gateway là phần thứ hai và chạm thêm bảy file: gateway
door, hai suite, e2e harness, cùng ba walk script từng tự mint token bằng secret chúng biết.

Phép kiểm tra hoàn toàn cơ học, và tôi viết nó để đợi zero thay vì “chỉ vài chỗ”.

Không còn gì. Không phải “không còn ngoài comment” — comment mô tả seam đã retire mà
không quote tên nó, nên check không cần exclusion vẫn sạch. Check cần caveat là check
mà rồi sẽ có người giải thích để bỏ qua.

File bắt đầu toàn bộ chuyện này cũng biến mất.

Factory từ 2.2 đọc principal tại nơi trước đây đọc header. Đây là one-line swap mà
chương đó hứa; nó giữ được một dòng vì seam đã được gọi tên và cô lập ngay từ ngày đầu.
...
**Refresh token trên live connection (vế hai FR-AUT-11).** Socket đã thiết lập sống lâu
hơn token — verification xảy ra lúc connect, không timer nào check lại. Đó là vế một và
đã có test. Nhưng internal hop forward chính token ấy, nên client giữ socket quá expiry
vẫn *receive* được nhưng không còn *send* được. Thay vì để failure thành “internal error”,
socket nói điều client thật sự có thể làm.

Khép gap đúng cách cần protocol frame cho phép client trao token mới trên connection
đang mở. Đây là thay đổi wire shape, và wire shape thuộc SDK sẽ nói nó.

**Rate-limit authentication failure (FR-AUT-12).** Quota và rate limit là chủ đề chương
retry-and-disable; xây nửa limiter ở đây đồng nghĩa xây hai lần.

**Key management.** Tạo, list và revoke key qua API cần console session để authenticate
người thực hiện; session là chương dashboard ở Part 5. Revocation đã implement và test
ở repository layer; thứ còn thiếu là door, không phải mechanism.

**Service-to-service credential trên internal hop.** Gateway và API vẫn tin network
giữa chúng đúng như 2.5 ghi lại. Điều thay đổi là gateway không còn *tự khai* identity:
nó forward end-user token để API quyết định. Trust boundary đã dịch đúng hướng nhưng
chưa biến mất.

*Được thu hẹp bởi chương connection-metering.* Gateway có credential riêng —
`RELAY_INTERNAL_CREDENTIAL_GATEWAY`, một platform credential cùng class mà chương
webhook dispatcher tạo cho dispatcher — vì usage report không phải user action và không
có token để forward. Vì vậy một trong bốn API call của gateway giờ authenticate như
service thay vì được tin như hàng xóm. Ba call còn lại vẫn forward end-user token; network
giữa chúng vẫn được tin cậy.
...
Mười hai invariant, mỗi cái có tên nói rõ điều nó bảo vệ. Chín chạy qua HTTP thật với
Postgres, hai là pure, một cần socket.

Invariant 4 là test tôi sẽ giữ nếu chỉ được giữ một. Key của environment A khi truy cập
channel environment B nhận 404; test assert response **byte-identical** với response cho
channel không tồn tại ở đâu cả. Isolation làm lộ sự tồn tại của hàng xóm không phải
isolation; nó là directory.

Revocation đáng một câu về điều *không* có. FR-AUT-05 đặt bound năm giây để revoked key
ngừng hoạt động, thường được đọc là “invalidate cache đủ nhanh”. Cách đọc rẻ hơn là không
có cache: một indexed lookup mỗi request trong connection pool request vốn dùng, và bound
đúng by construction trên mọi instance, không gì cần invalidate. Nếu profiling sau này
chứng minh đó là bottleneck, fix sẽ là cache cùng invalidation channel — fabric từ 2.6
làm thêm việc thứ hai; nó thuộc chương đo được nhu cầu, không thuộc chương này.
...
Hai lần retire seam chạm hai mươi chín file mà các chương trước đã cho bạn xem. Tất cả
đều có mặt ở đây, vì chương thay code bạn đã copy mà không nói là cách tutorial bắt đầu
nói dối.

Phần lớn thay đổi chỉ ba dòng: suite đổi header thành bearer credential, walk script hỏi
API lấy token thay vì tự ký. Hãy đọc chúng như một việc thay vì hai mươi chín việc — đây
là chi phí của seam, được trả đủ vào ngày nó bị retire.
...
@@ -124,10 +126,22 @@ export class SignupController {
     });
...
Chạy `pnpm typecheck`, `pnpm lint`, `pnpm test:unit` và `pnpm test:integration`.

Kết quả mong đợi: **109 unit test**, **76 integration test**, tất cả xanh — gồm cả journey
suite chương 2.8, giờ chạy hoàn toàn bằng credential thật.

Sau đó chạy seam check; nó không được in ra bất cứ dòng nào. Cuối cùng chạy walk khi cả
hai service đang hoạt động.

Kết quả: key gửi message và mint token; token mở socket. Sau đó token bị từ chối ở
key-only route với `wrong_credential_type`, còn key bị từ chối ở socket bằng close code
4001.

</Checkpoint>

<ChapterFooter id="3.2" locale="vi" />

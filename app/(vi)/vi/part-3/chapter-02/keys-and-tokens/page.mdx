...
export const metadata = {
  ...
  description:
    "Hai credentials, phân biệt bằng prefix: một API key có secret chỉ hiện một lần và được hash at rest, và một end-user token được api verify bằng signing secret riêng của environment. Chương cuối cùng xóa header mà mọi request đã tự assert từ 2.2.",
  alternates: {
...
Chương 3.1 build các containers nơi thế giới của một customer sống và signup
tạo ra chúng. Nó chưa build cách để *chứng minh* bạn sở hữu một cái. Mọi
request từ 2.2 đến giờ gọi tên tenant bằng một header mà nó chỉ đơn giản tự
assert:
...
Ai truy cập được api cũng có thể gõ header đó, gọi tên bất kỳ environment nào, và
đọc nó. Lúc đó tôi gọi nó là một dev-mode seam và hứa Phần 3 sẽ thay thế nó.
Đây là chương đó, và seam này không sống sót qua chương.
...
Thứ thay thế chúng là hai credentials — và chủ đề thật của chương không hẳn là
credential nào trong hai cái. Nó là lỗi người ta mắc phải với cặp này.

<SkipAhead locale="vi">

Nếu bạn buộc phải đọc lướt: table `api_keys` nhập vào schema bên dưới tenant
boundary, giữ một public lookup id, một salted SHA-256 của secret chỉ hiện đúng
một lần, và prefix `rk_dev_`/`rk_live_` mirror environment kind. Authentication
chạy trong **middleware** — đã đo, vì Nest build request-scoped providers trước
guards — và tạo một `principal` mang `environmentId`, thứ repository factory
bây giờ đọc thay cho header. End-user tokens được verify bởi **api**, không phải
gateway: gateway post token tới `POST /internal/session` và được trả lời caller
là ai và họ được nghe gì, thay thế memberships call nó vốn đã gọi, nên connect
path không thêm round trip. Signup bàn giao key đầu tiên của environment, vì
khi chưa có console session thì không có thứ nào khác bootstrap được nó. Trạng
...
Một **application** — backend của một công ty, giữ credential trong environment
variables của chính nó — nói *tôi là environment này*. Nó gửi messages thay cho
ai tùy ý, vì nó là customer. Một **end user** — tài xế trong xe tải, một browser
tab — nói *tôi là người này trong environment đó*. Nó chỉ được act as chính nó.

Một credential không thể trung thực mang cả hai claims, nên có hai credential.

<Figure
  caption="Hai credential classes, mỗi cái được present cho đâu, và hai refusals tạo thành chủ đề của chương này."
  code={figTwoCredentials}
/>

Cả hai resolve về cùng một shape, và đó chính là điểm quan trọng.
...
Một API key phải giải một vấn đề không query nào khác trong codebase này gặp.
Mọi repository method từ 2.1 bắt đầu với một environment; riêng method này
*sinh ra* một environment. Authentication xảy ra trước khi tenant scope tồn tại,
nghĩa là key lookup là query unscoped duy nhất trong system.

Điều đó loại bỏ design hiển nhiên. Nếu toàn bộ credential được hash, sẽ không
còn gì để lookup, và mỗi request phải scan mọi key trên platform. Vì vậy
credential có hai phần: một public handle được indexed và không phải secret, và
một secret được hash rồi không bao giờ stored.
...
Prefix là requirement, không phải decoration. FR-AUT-03 yêu cầu key mang một
...
Tôi viết parser split ở underscore cuối cùng, vì design note nói vậy. Rồi key
đầu tiên vừa mint đã parse fail.

Cả hai nửa đều là base64url, và **alphabet của base64url có chứa `_`**. Một
secret có underscore làm "split ở separator cuối" trở nên sai; một public id có
underscore làm "split ở cái đầu tiên" trở nên sai. Không có cách split an toàn
khi cả hai phía đều có thể chứa delimiter.

Fix là đưa delimiter ra khỏi một alphabet: public handle là hex, không có `_` và
có fixed length, nên boundary là chính xác bất kể secret tình cờ chứa gì. Cái
giá là một character class và một test:

```ts
const CREDENTIAL = /^(rk_dev_|rk_live_)([0-9a-f]{32})_(.+)$/;
```

Design document nói "split ở separator cuối". Lần mint đầu tiên nói ngược lại.
Đây là lần thứ ba trong series này một Phase 1 artifact được sửa bằng cách chạy
code, và correction rẻ chính vì nó xảy ra trước khi bất cứ thứ gì được publish.
...
<Why source="NFR-SEC-02 · FR-AUT-02 · research R3" locale="vi">

**Vì sao salted SHA-256, khi ai cũng nói bcrypt?**

Vì "luôn dùng password KDF" là lời khuyên cho *passwords*, còn đây không phải
password.

bcrypt và argon2 cố ý chậm. Sự chậm đó mua đúng một thứ: khả năng chống việc
guess một low-entropy secret — một password do người chọn, lấy từ một space đủ
nhỏ để search. Đó là lựa chọn đúng ở chỗ đó và là lý do không ai nên store
password bằng SHA-256.

Key secret ở đây là 32 bytes từ `crypto.randomBytes`: 256 bits, phân phối đều.
Không có work factor nào làm nó guessable, và cũng không có work factor nào làm
nó "khó guess hơn". Thứ KDF mua được là không gì cả; cái nó tốn là một delay cố
ý trên **mọi authenticated request**, cộng thêm một native dependency, ngay
trong code path chạy trước mọi thứ khác.

Vậy: SHA-256 trên một random salt per-key, compare bằng `timingSafeEqual`. Salt
xứng đáng có column riêng vì nó làm hai secrets giống hệt nhau hash ra khác
nhau. NFR-SEC-02 yêu cầu salted hash và đây chính là salted hash — requirement
được satisfy bằng cách đọc nó, không phải bằng cách với tới tool nghe có vẻ
mạnh nhất.

Cái trap thật nằm ngay cạnh đó, và nó là comparison chứ không phải hash:
`timingSafeEqual` throw khi hai buffers khác length. Compare *secrets* thì một
caller có thể crash endpoint bằng một string ngắn; compare *hashes* thì hai bên
luôn là 32 bytes.

</Why>

Table chứa nó mang environment, như mọi thứ khác bên dưới boundary. Thứ nó
không mang là constraint nào về số keys một environment được có — nhiều key
active cùng lúc là feature, không phải oversight, vì đó là ý nghĩa của rotation
without downtime.
...
## Secret bạn nhìn thấy một lần

Một organisation hoàn toàn mới không thể authenticate request để xin key đầu
tiên. Không có console session — 3.1 cố ý chưa build — nên phải có thứ gì đó
bootstrap credential, và signup là hành động duy nhất đã biết bạn là ai.
...
Walk từ 3.1 bây giờ cho thấy nó bàn giao gì, và — hữu ích hơn — lần thứ hai nó
không bàn giao gì:
...
Một returning owner không được trao secret mới. Secret cũ unrecoverable by
construction, và câu trả lời cho lost key là rotation, không phải retrieval —
mà câu trả lời đó chỉ usable vì có thể có nhiều key active cùng lúc.
...
Có hai đường ra. Ship signing secret của mọi environment tới gateway, hoặc hỏi
service sở hữu chúng. Shipping nghĩa là một tenant-scoped secret nằm trong một
process cố ý không giữ tenant state, nhân lên theo mọi gateway instance, cộng
thêm câu chuyện rotation mỗi khi một cái thay đổi. Hỏi thì tốn một HTTP call.

Và gateway vốn đã gọi một cái. Từ 2.5, lúc connect nó đã hỏi api "user này được
nghe những channels nào?" Thay câu hỏi đó bằng "đây là ai, và họ được nghe gì?"
tốn cùng một round trip và chuyển quyết định identity sang service duy nhất có
thể ra quyết định đó.

<Figure
  caption="Connect sau chương này. Gateway forward token nó nhận được và được trả lời caller là ai — vẫn đúng một call nó đã gọi, chỉ hỏi câu tốt hơn."
  code={figConnect}
/>
...
JWT vulnerability nổi tiếng nhất không phải là signature bị broken. Nó là hỏi
token cách tự check chính nó.

Header của JWT tự gọi tên algorithm của nó. Một verifier đọc field đó rồi làm
theo sẽ accept `{"alg":"none"}` — một token có signature rỗng mà ai cũng viết
được. Đổi sang `RS256` trước một verifier đang expect HMAC, và vài libraries sẽ
vui vẻ dùng public key làm HMAC secret, trong khi nó là public.

Defence là một argument:

```ts
await jwtVerify(token, key, { algorithms: ["HS256"] });
```

`jose` từ chối mọi thứ không nằm trong list đó trước khi nhìn vào key material.
Test chứng minh điều này tự build một unsigned token, vì bạn không thể mint nó
qua library:
...
Đây là toàn bộ lý do một dependency xuất hiện trong chương này sau khi 3.1 đã
cố tránh thêm dependency nào. Hand-roll HS256 là khoảng ba mươi dòng và ba lỗi
classic — lỗi này, quên `exp`, và compare signatures bằng `===`. Convenience có
thể đáng để type quanh nó. Security code thì không.

</Trap>

Có một điểm tinh tế trong order of operations. Api phải biết dùng secret của
environment *nào* để check signature, và nơi duy nhất có thông tin đó là bên
trong token — chưa được verified. Vì vậy environment claim được đọc trước, không
trust, chỉ để chọn key; signature sau đó mới quyết định có tin nó hay không.
Đọc claim *sau* khi verify thì bạn không verify được gì cả; trust claim cho bất
cứ việc nào khác thì token từ environment A đi vào environment B.
...
Nhìn kỹ type đó nói gì. Ba outcomes, không phải hai — vì chuyển verification
sang api tạo ra một failure gateway chưa từng có: verifier bị *down*. Một token
bị refused và một api không reach được đều làm socket không open, và trả lời
chúng cùng một cách sẽ là nói dối theo một hướng. 4001 nói với client credential
của nó sai, nên đừng retry; 1011 nói chúng ta đang broken, nên hãy retry.
Chương 2.5 đã vẽ distinction đó cho memberships lookup, và việc move lookup
không được âm thầm xóa nó.
...
repository factory của 2.2 tự đọc request thay vì trust rằng guard đã stash thứ
gì đó. Finding đó ràng buộc chương này: authentication bây giờ liên quan tới
database lookup, và guard vẫn sẽ chạy quá muộn đối với factory cần câu trả lời.

Middleware chạy sớm hơn. Nhưng "chạy sớm hơn" là điều tôi tin chứ chưa đo trên
path này, nên trước khi build dựa vào nó tôi log một dòng từ mỗi trong ba nơi
và gửi một request:
...
Middleware, rồi factory, rồi guard. Design giữ được, fallback đã gọi tên không
cần dùng, và phép đo mất bốn phút.

<Figure
  caption="Thứ tự được đo, không giả định. Authentication phải xảy ra trước khi factory đọc scope; job của guard hẹp hơn trước."
  code={figAuthOrder}
/>
...
SRS có một design note dưới authentication requirements, và nó cụ thể khác
thường:

> Nhầm lẫn giữa API keys và user tokens là failure phổ biến nhất ở lần
> integration đầu tiên. FR-AUT-03 và FR-AUT-09 tồn tại riêng để giảm nó, và
> error message khi dùng sai credential type phải gọi tên lỗi đó một cách rõ
> ràng.
...
Endpoint ở giữa transcript đó là của FR-AUT-09, và nó tồn tại vì một lý do:
...
<Why source="NFR-SEC-06 · EIR-WS-05" locale="vi">

**Vì sao token của socket nằm trong query string, khi credentials không bao giờ
được nằm trong URLs?**

Vì browser không thể set headers trên một WebSocket upgrade. Constructor
`new WebSocket(url)` nhận một URL và không gì khác — không headers, không body.
Mọi browser-based realtime product đều gặp vấn đề này, và các lựa chọn thành
thật là query parameter, cookie, hoặc first frame sau khi socket open.

Chúng ta dùng query parameter (shape của EIR-WS-05, không đổi từ 2.5), và đáng
nói thẳng ra thay vì để nó trông như một contradiction. Những mitigations khiến
nó defend được: token short-lived by construction (tối đa 24 giờ, FR-AUT-07),
nó chỉ cấp quyền mà một end user được làm, và nó là nơi *duy nhất* trong system
một credential xuất hiện trong URL — mọi thứ khác là bearer header, và api từ
chối credentials từ query strings bằng cách đơn giản là không bao giờ đọc chúng
từ đó.

<ForwardRef part="Phần 5" locale="vi">Phương án first-frame — open socket khi
chưa authenticated rồi authenticate bằng một frame — dù sao cũng là thứ protocol
cần để client có thể bàn giao refreshed token giữa connection. Đó là clause thứ
hai của FR-AUT-11, và nó đến cùng các chương SDK trong Phần 5.</ForwardRef>

</Why>

## Cả hai seams, biến mất

Header trước. Retire nó là một increment riêng vì một seam half-retired còn tệ
hơn cả hai trạng thái còn lại, và blast radius được đo chứ không đoán:
**mười một files**, trong đó ba file là production code paths và phần còn lại là
suites đang present header hoặc identity headers đi cùng nó.

Shared development secret của gateway là increment thứ hai, và nó chạm thêm bảy
file: cánh cửa của gateway, hai suites của nó, e2e harness, và ba walk scripts
vốn đều tự mint tokens bằng một secret chúng biết.
...
Không gì cả. Không phải "không gì ngoài comments" — comments mô tả retired seam
không quote nó, nên check không cần exclusions để sạch. Một check cần caveat là
một check rồi sẽ có người explain away.
...
Và factory 2.2 viết đọc principal ở nơi trước đây nó đọc header. Đó là one-line
swap mà chương đã hứa, và lý do nó vẫn là một dòng là seam đã được gọi tên và
isolate từ ngày nó được introduce:
...
**Refreshed token trên một live connection (FR-AUT-11, clause thứ hai).** Một
socket đã established sống lâu hơn token của nó — verification xảy ra lúc
connect, và không timer nào re-check. Đó là clause đầu tiên, và nó đã được
test. Nhưng internal hop forward cùng token đó, nên client giữ socket open quá
expiry của token vẫn có thể *receive* và không còn *send* được. Thay vì để nó
fail như "internal error", socket nói điều client thực sự có thể làm:
...
Đóng gap đó đúng cách cần một protocol frame — một cách để client bàn giao token
mới trên open connection — tức là change trên wire, và shape của wire thuộc về
SDK nói chuyện với nó.

**Rate-limited authentication failures (FR-AUT-12).** Quotas và rate limits là
chủ đề của chương 3.6, và build nửa cái limiter ở đây nghĩa là build nó hai
lần.

**Key management.** Creating, listing và revoking keys qua API cần một console
session để authenticate người thực hiện, và session là chương dashboard trong
Phần 5. Revocation hôm nay đã được implement và test ở repository layer; thứ
thiếu là cánh cửa, không phải mechanism.

**Service-to-service credentials trên internal hop.** Gateway và api vẫn trust
network giữa chúng, đúng như chương 2.5 đã record. Thứ thay đổi là gateway
không còn *assert* identity: nó forward token của end user và api quyết định.
Trust boundary đã đi đúng hướng; nó chưa biến mất.

*Được thu hẹp bởi chương 3.11.* Gateway có credential của riêng nó —
`RELAY_INTERNAL_CREDENTIAL_GATEWAY`, một platform credential thuộc lớp mà chương
3.5 dựng cho dispatcher — vì một báo cáo sử dụng không phải hành động của người
dùng nào và không có token nào để forward. Vậy một trong bốn lời gọi api của
gateway giờ được xác thực như một service chứ không phải được tin như hàng xóm.
Ba lời gọi còn lại vẫn forward token của end user, và mạng giữa chúng vẫn được
tin.

## Các tests, và thứ chúng giữ

Mười hai invariants, mỗi cái có một cái tên nói nó protect điều gì. Chín cái
chạy real HTTP against Postgres, hai cái pure, và một cái cần socket:
...
Invariant 4 là cái tôi sẽ giữ nếu chỉ được giữ một cái. Key của environment A
present channel của environment B nhận 404 — và test assert response đó
**byte-identical** với response bạn nhận khi channel không tồn tại ở bất kỳ đâu.
Isolation mà leak sự tồn tại của neighbour thì không phải isolation; nó là một
directory.
...
Revocation đáng có một câu về thứ *không* nằm ở đó. FR-AUT-05 đưa ra bound năm
giây để revoked key ngừng hoạt động, điều thường được đọc là "invalidate cache
của bạn đủ nhanh". Cách đọc rẻ hơn là không có cache: một indexed lookup per
request, trong connection pool request đã dùng, và bound đúng by construction
trên mọi instance mà không có gì cần invalidate. Nếu profiling sau này biến nó
thành bottleneck, fix là cache cộng invalidation channel — tức fabric của chương
2.6 làm thêm job thứ hai, và nó thuộc về chương nào đo được nhu cầu đó, không
phải chương này.

## Blast radius, từng file một

Hai seam retirements chạm tới hai mươi chín files mà các chương trước đã cho
bạn thấy. Tất cả đều ở đây, vì một chương thay đổi code bạn đã copy mà không
nói ra chính là cách tutorial bắt đầu nói dối.

Phần lớn là ba dòng: một header được đổi thành bearer credential trong một
suite, một walk script hỏi api xin token thay vì tự sign token. Hãy đọc chúng
như một thứ thay vì hai mươi chín thứ — đây là chi phí của một seam, trả đủ vào
ngày nó được retire.
...
<Checkpoint tag="part3-ch2" locale="vi">
...
Expected: **109 unit tests**, **76 integration tests**, tất cả green — bao gồm
journey suite của chương 2.8, giờ chạy hoàn toàn trên real credentials.
...
Và walk, khi cả hai services đang chạy:

```bash
node scripts/credential-walk.mjs
```

Expected: một key gửi được message, mint token, token open socket — rồi token bị

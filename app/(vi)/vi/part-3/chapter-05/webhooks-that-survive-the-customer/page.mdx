...
export const metadata = {
  ...
  description:
    "Handler của chương 3.4 log một dòng rồi gọi đó là effect. Chương này trao cho nó một effect thật — một HTTP request tới cỗ máy không thuộc về chúng ta — và nhìn cái pattern từng khiến 3.4 đúng đắn ngừng áp dụng được, tới hai lần.",
  alternates: {
    canonical: "/vi/part-3/chapter-05/webhooks-that-survive-the-customer",
    languages: {
      en: "/part-3/chapter-05/webhooks-that-survive-the-customer",
      vi: "/vi/part-3/chapter-05/webhooks-that-survive-the-customer",
    },
  },
};
...
```
Thời điểm đó nó được gọi là scaffold, và từ ấy được chọn rất cẩn thận. Mọi bảo
đảm mà 3.4 chứng minh — claim event, chạy effect, commit cả hai cùng lúc, để một
cú crash cuốn luôn cái claim đi cùng với work — đều được chứng minh trên một
effect chỉ ghi một dòng ra standard output.

Chương này thay nó bằng một effect nằm trên cỗ máy của người khác.

Chỉ một thay đổi ấy đã bẻ gãy pattern ở hai chỗ tách biệt, và không chỗ nào là
lỗi cần sửa cả. Effect không còn có thể tham gia vào transaction của claim, vì nó
xảy ra trên một server không thuộc về chúng ta. Và claim cũng không còn được thực
hiện bởi chính đoạn code đang làm việc, vì hiến pháp IV dành riêng quyền ghi
PostgreSQL cho api service — còn thứ đang gửi request thì không phải nó.

<SkipAhead>

Nếu buộc phải lướt: webhook endpoint được lưu kèm signing secret **mã hoá kiểu
envelope** (AES-256-GCM), xoay vòng được, với cả secret hiện tại lẫn secret trước
đó cùng hợp lệ trong cửa sổ 24 giờ, để khách hàng roll secret mà không cần một
đợt deploy đồng bộ. Request được ký HMAC-SHA256 trên canonical string
`v1:{timestamp}:{raw body}` — **đúng những byte được truyền đi**, không bao giờ là
bản serialise lại của object đã parse. Một event được nở ra thành một dòng
`webhook_deliveries` cho mỗi endpoint khớp, ghi **bên trong** transaction claim
của chương 3.4, nên một event bị gửi lại không thể nhân đôi webhook của khách
hàng. Lịch retry là cột `next_attempt_at` do một relay thứ hai trong api rút cạn —
chính vòng lặp `SELECT … FOR UPDATE SKIP LOCKED` của chương 3.3 với thêm đúng một
predicate — bởi vì độ trễ do broker giữ đã được **đo đạc** là vẫn chiếm một slot
acknowledgement trong lúc chờ, khiến endpoint chết bỏ đói endpoint khoẻ mạnh. Bảy
lần thử (`now → 1s → 5s → 30s → 5min → 30min → 2h`), rồi một dead letter giữ lại
bảy ngày. Dispatcher là một **deployable mới**, không ghi gì vào PostgreSQL và chỉ
chạm tới state qua một internal HTTP seam. Chặng cuối là **at-least-once, được nói
thẳng**, với event id trao cho khách hàng làm khoá deduplicate. Trạng thái cuối
chương là tag **`part3-ch5`**.

</SkipAhead>

## Pattern ngừng áp dụng được, tới hai lần

Lập luận về tính đúng đắn của chương 3.4 chỉ có một bộ phận chuyển động. Claim và
effect nằm trong cùng một transaction, nên giữa chúng không hề có khoảng hở:
...
```
Giờ thử viết lại điều đó với một HTTP request nằm bên trong. Cái `await` ở giữa là
một POST tới server của khách hàng. Nó đã xảy ra rồi vào lúc transaction commit, và
rollback transaction không hề thu hồi được nó. Không có cách sắp xếp nào khiến hai
thao tác này trở thành atomic, bởi một trong hai không thuộc quyền rollback của ta.

Thế nên câu hỏi đổi khác. Nó không còn là *làm sao tránh khoảng hở* — khoảng hở là
không thể tránh — mà là **ta muốn sai theo hướng nào**:

| Thứ tự | Một cú crash trong khoảng hở phải trả giá gì |
|---|---|
...
<Figure caption="Khoảng hở không thể khép lại. Thứ chọn được là cú crash sẽ rơi về phía nào của nó." code={figThePatternStops} />

Nền tảng chọn nhận bản trùng, và lý do đáng được nói thẳng thay vì để nó trôi qua
như một sở thích. Một mất mát không ai phát hiện được thì tệ hơn một bản trùng mà
người nhận đã được trao sẵn phương tiện để hấp thụ. Chương 3.3 đã dành trọn một
chương để loại đúng loại hỏng hóc thứ nhất khỏi đường publish; tái tạo nó ở chặng
cuối — chặng mà khách hàng thực sự cảm nhận — sẽ phá bỏ công sức ấy ngay chỗ nó
quan trọng nhất.

Quyết định đó đi kèm một nghĩa vụ, và chính nghĩa vụ mới làm nó trung thực thay vì
tiện lợi. **Nếu bạn chọn at-least-once, bạn phải trao cho người nhận phương tiện
để deduplicate.** Event id đi trong envelope, có tài liệu hẳn hoi, ổn định qua mọi
lần thử của cùng một delivery. Khách hàng lưu nó lại và bỏ qua bản lặp là làm
đúng; khách hàng không làm vậy thì đã được cho biết họ đang chọn điều gì.
...
Cả hai đều là at-least-once, và đáng để nói chính xác chỗ nào bảo đảm thay đổi.
*Bên trong* nền tảng, ledger của 3.4 khiến việc handle trở thành once-in-effect:
delivery là at-least-once, và claim `consumed_events` hấp thụ các bản lặp. Ở chặng
của khách hàng thì không có ledger nào ta kiểm soát, nên bản lặp sẽ tới nơi. Bảo
đảm yếu đi không phải vì thiết kế tệ hơn — mà vì ranh giới đã dời sang một cỗ máy
không chạy code của chúng ta.

</Why>

## Endpoint, và cái secret ta buộc phải trả lại được

Một endpoint gồm URL, danh sách event type, và một signing secret. Secret mới là
cột thú vị, vì đây là giá trị đầu tiên trong nền tảng này **bắt buộc phải khôi
phục được ở dạng plaintext**.

Chương 3.2 lưu API key dưới dạng salted hash và có quyền làm thế: key được trình
ra rồi so sánh, không bao giờ cần tái tạo. Signing secret thì khác. Dispatcher
phải tính HMAC bằng nó ở mỗi lần thử, mà HMAC thì cần chính secret. Một hash không
ký được.

Nên đây là envelope encryption chứ không phải hashing — đúng cái quyết định mà
NFR-SEC-02 đã lường trước với hai nhánh, một cho credential được đem so sánh và
một cho secret được đem sử dụng:
...
```
Hai chi tiết trong đó là quyết định chứ không phải cơ khí.

**Cửa sổ xoay vòng** là chi tiết thứ nhất. `activeSigningSecrets` trả về một secret
trong điều kiện bình thường và *hai* trong hai mươi tư giờ sau một lần rotate, còn
request thì mang theo một chữ ký cho mỗi cái. Khách hàng rotate secret phải cập
nhật cả bộ verify lẫn cấu hình của họ, và trong một hệ thống thật thì hai việc đó
không bao giờ đồng thời. Không có cửa sổ, rotate đồng nghĩa với một phút được chọn
sẵn để verify thất bại; có cửa sổ, cả hai đều chạy và cái cũ tự hết hạn.

**Cách phân giải khoá** là chi tiết thứ hai, nhỏ hơn nhưng đã từng cắn repository
này. `encryptionKey()` đọc environment ở mỗi lần gọi chứ không phải lúc import. Một
module ném lỗi ngay khi đang được import sẽ hạ cả service lúc khởi động chỉ vì một
tính năng mà phần lớn request không bao giờ chạm tới, và — tệ hơn với chúng ta — nó
khiến chính lỗi ấy không thể chạm tới từ một unit test, bởi file test không thể
import nổi cái module mà nó muốn assert.

<Trap>

Fallback dành cho môi trường phát triển được làm ồn ào có chủ đích, và nó từ chối
...
```
Một default im lặng ở đây đồng nghĩa production lặng lẽ mã hoá signing secret của
mọi khách hàng bằng một khoá in trong sách hướng dẫn. Bản ồn ào tốn đúng một biến
môi trường lúc deploy và xoá sổ nguyên một hạng mục sự cố.

</Trap>

## Migration, và bề mặt dựng trên nó

Ba bảng tới cùng lúc, và migration được đọc lại trước khi chạy — kỷ luật mà chương
2.1 thiết lập sau khi một migration sinh tự động bị đem áp dụng mà chẳng ai đọc:
...
```
Bề mặt quản trị là CRUD bình thường với ba quyết định nằm trong đó:
...
```
**URL bắt buộc phải HTTPS và không được trỏ vào bên trong.** Endpoint là một URL mà
nền tảng sẽ tự đi fetch thay mặt khách hàng, từ bên trong chính mạng của nền tảng —
đó đúng là định nghĩa của server-side request forgery nếu không ai kiểm tra nó trỏ
đi đâu. Loopback, dải private và địa chỉ link-local đều bị từ chối, và lời từ chối
là một validation error khách hàng đọc được, chứ không phải một timeout họ chẳng
hiểu gì.

**Năm endpoint cho mỗi environment.** Không phải vì con số năm đặc biệt, mà vì
"không giới hạn" cũng là một con số, và là con số sai: expansion ghi một dòng cho
mỗi endpoint khớp, nên một environment với mười nghìn endpoint sẽ biến một event
thành mười nghìn dòng trong đúng một transaction.

**Secret được trả về đúng một lần.** Lúc tạo, và không bao giờ nữa — cột đó giữ
ciphertext, còn plaintext chỉ tồn tại đúng khoảng thời gian response được
serialise:
...
```
## Ký đúng thứ mình thực sự gửi đi

Chữ ký phủ lên một canonical string, và chuỗi ấy được dựng từ **những byte thô của
body** — đúng những byte sẽ lên dây:
...
```
Timestamp nằm trong phần được ký, không phải bên cạnh nó, và đó chính là thứ chặn
một request bị bắt lại đem replay vô thời hạn. Còn body là một chuỗi được
serialise **đúng một lần** rồi vừa đem ký vừa đem gửi. Đó không phải một tối ưu.

<Trap>

**Ký một bản render rồi gửi một bản khác thì chẳng gì chạy cả, vì những lý do
không ai nhìn thấy được.** Đây là cách phổ biến nhất khiến một webhook integration
đang chạy tốt bỗng gãy, và nó gần như không bao giờ trông giống cái lỗi mà nó là:
```js
// Both of these are "the same JSON".
const body = JSON.stringify(payload);
const sig  = sign(JSON.stringify(JSON.parse(body)));   // ← a different string
```
Trong Node, trong cùng một process, vòng lặp parse-rồi-stringify ấy thường trả lại
đúng những byte nó nhận — và đó chính xác là lý do con bug này sống sót qua mọi
test viết bằng chính ngôn ngữ của service. Nó chỉ lộ ra vào ngày một khách hàng
verify bằng Go, hoặc một proxy chuẩn hoá JSON, hoặc ai đó spread object sang một
object mới để thêm một field.

Chính script walk viết cho chương này đã mắc lỗi ấy với chính nó. Cờ
`--print-signing-material` ký payload object theo đúng cách script dựng nó, trong
khi nền tảng ký thứ đi ra từ `jsonb` — mà PostgreSQL thì không bảo toàn thứ tự
khoá. Chữ ký được in ra là chữ ký cho một bản render chưa bao giờ lên dây, và phải
chạy thật với một endpoint thật mới nhận ra.

</Trap>

Bài test cho chuyện này được viết từ *tài liệu*, không phải từ bộ ký, và đó là cách
duy nhất nó chứng minh được điều nó tuyên bố:
...
```
Một test gọi `signDelivery` hai lần rồi so sánh kết quả chỉ chứng minh rằng hàm đó
tất định. Nó không chứng minh một chút nào về việc khách hàng có verify nổi chữ ký
hay không — mà đó mới là tính chất duy nhất đáng kể.

## Lịch trình là một cột, và lý do là một phép đo

Server của khách hàng sẽ có lúc chết. Nền tảng retry theo một lịch giãn dần rồi
cuối cùng bỏ cuộc — chừng đó nằm sẵn trong requirements. Câu hỏi thú vị là *sự chờ
đợi* sống ở đâu.

Câu trả lời hiển nhiên là broker. NATS cho phép consumer nack một message kèm độ
trễ, broker giữ nó, và nó quay lại khi độ trễ hết. Không bảng mới, không vòng lặp
mới. Chương 3.4 đã dựng sẵn consumer rồi.
...
```
Q1 là một cú pass sạch sẽ — một lần redelivery có trễ sống sót qua việc restart
process với sai số ba mili giây, tốt hơn cả mức thiết kế cần. Chỉ xét Q1 thôi thì
độ trễ do broker giữ là câu trả lời đúng.

Q2 loại nó. Một message đang nằm chờ hết độ trễ **vẫn giữ slot acknowledgement của
mình**. Ba message đang ngủ trên `max_ack_pending: 3` đã khiến hai message sẵn sàng
trở nên không fetch được — không phải chậm, mà là không fetch được. Nhân điều đó
lên quy mô nền tảng: một khách hàng có endpoint chết hai tiếng sẽ chiếm mất năng
lực giao nhận vốn thuộc về mọi khách hàng khác, và chính những endpoint đang hoạt
động tốt lại là những endpoint ngừng được phục vụ. FR-WHK-05 nói endpoint của một
khách hàng không được làm chậm delivery tới khách hàng khác, còn độ trễ do broker
giữ khiến điều đó sai ngay từ trong cấu trúc.

<Why>

**Kế hoạch được làm lại chứ không phải chắp vá.** Phép đo tới sau khi thiết kế đã
viết xong, và phản ứng trung thực trước câu "cơ chế tôi định dùng có một tính chất
vi phạm requirement" là lập kế hoạch lại, chứ không phải thêm một dòng lưu ý rồi đi
tiếp. Research R1 và R13 ghi lại điều gì đã đổi và vì sao.

Thứ thay thế lại nhỏ hơn, và đó mới là phần đáng chú ý. Sự chờ đợi trở thành cột
`next_attempt_at`, còn thứ rút cạn nó là vòng lặp relay của chương 3.3 với thêm
đúng một predicate — một cơ chế người đọc đã sở hữu sẵn.

</Why>

Vậy nên chính dòng delivery *là* lịch trình:
...
+);
```
<Figure caption="Dòng delivery chính là lịch trình. Không có gì ngủ trong broker; một delivery chưa tới hạn đơn giản là một dòng chưa query nào chọn tới." code={figTheSchedule} />

Partial index gói trọn lập luận hiệu năng trong đúng một dòng: query rút cạn chỉ
bao giờ hỏi tới các dòng `pending`, nên một index chỉ chứa các dòng `pending` sẽ
mãi nhỏ, bất kể phía sau nó chất đống bao nhiêu dòng đã delivered.

Còn bảng tier thì sống trong **api**, không phải dispatcher — thoạt nhìn có vẻ sai
cho tới khi bạn để ý ai là người đọc nó. Các tier được `recordAttemptOutcome` tra
cứu khi nó tính `next_attempt_at` kế tiếp — mà đó là một thao tác ghi database, nên
hiến pháp IV đặt nó vào api. Đặt lịch trình trong dispatcher sẽ có cảm giác tự
nhiên và sẽ sai chỗ:
...
```
## Expansion, bên trong claim

Một event có thể khớp nhiều endpoint. Biến một thành nhiều chính là khoảnh khắc mà
một bản trùng trở nên đắt đỏ, bởi một event bị gửi lại mà nở ra hai lần sẽ gửi cho
mọi khách hàng khớp một webhook thứ hai.

Kế hoạch đầu tiên publish N message lên broker, mỗi delivery một cái. Đó là một
dual write: claim commit vào PostgreSQL, rồi N lần publish đi tới NATS, và một cú
crash ở giữa để lại hai bên bất đồng. Ghi N **dòng** thay vào đó đặt expansion nằm
trong transaction của claim, nơi bảo đảm của 3.4 vốn đã bao phủ sẵn:
...
+
   async createUser(externalId: string, displayName?: string): Promise<UserRow> {
     ...
     await this.db.insert(users).values({
       ...
       externalId,
```
Số `created` và cờ `duplicate` cùng quay về, và dispatcher log cả hai. Một bản
trùng không phải lỗi — đó là claim đang làm đúng việc của nó, và một operator đọc
thấy `duplicate=true` trong một dòng log là đang chứng kiến cơ chế vận hành, chứ
không phải một vấn đề cần điều tra.

<Why>

**Lọc theo subscription diễn ra ở expansion, không phải ở delivery.** Một endpoint
không subscribe `message.created` sẽ không có dòng nào cả, thay vì có một dòng rồi
sau đó bị bỏ qua. Một dòng delivery tồn tại mà không bao giờ gửi được chính là một
lịch retry có sẵn một no-op vĩnh viễn bên trong, và operator đọc bảng ấy không có
cách nào phân biệt nó với công việc đang thực sự kẹt.

</Why>

## Vì sao dispatcher là service riêng

Chương 3.3 thêm outbox relay và chương 3.4 thêm consumer runtime, cả hai đều sống
**bên trong** api. Chương này thêm một background worker thứ ba và đặt nó vào một
deployable mới. Sự thiếu nhất quán đó cần một lập luận, nếu không nó chỉ là sở
thích.

Lập luận là: đây là worker đầu tiên có effect không phải một thao tác ghi database.

Relay đọc outbox rồi publish; consumer claim event rồi ghi dòng. Cả hai đều chạm
PostgreSQL, và hiến pháp IV nói api sở hữu các thao tác ghi đó, nên cả hai thuộc về
nơi có thao tác ghi. Effect của dispatcher là một HTTP request tới server của khách
hàng — nó không cần quyền truy cập database nào cả, và trao cho nó quyền ấy đồng
nghĩa có một service thứ hai ghi vào PostgreSQL chỉ vì tiện.

<Figure caption="Ba service, một người ghi. Dispatcher chỉ chạm tới state qua internal seam, và đó là thứ khiến hiến pháp IV mang tính cấu trúc thay vì một quy tắc người ta phải nhớ." code={figWhereItRuns} />

Thứ mà sự tách bạch này *mua được* là một tính chất mà lẽ ra chỉ là một cuộc tranh
luận. Bên trong api, "webhook không làm chậm người dùng cuối" là một tuyên bố về
event loop và connection pool — thứ bạn lập luận rồi hy vọng. Băng qua ranh giới
process, nó là một tuyên bố về process, và bạn giải quyết nó bằng cách không khởi
động cái process ấy:
...
```
Dispatcher không hề được khởi động trong file đó. Nó không tồn tại suốt hành trình,
và nền tảng được kỳ vọng là chẳng nhận ra điều đó — message vẫn tới người dùng
cuối, còn endpoint của khách hàng thì không nghe thấy gì cả.

Thứ mà sự tách bạch **phải trả giá** cũng đáng được nói thẳng như thế, bởi một
chương chỉ liệt kê lợi ích của chính quyết định mình đưa ra là đang quảng cáo:

- **Một network hop nằm ở chỗ trước đây chỉ là một lời gọi hàm.** Mọi thay đổi
  state mà dispatcher thực hiện đều là một vòng HTTP tới api. Đó là độ trễ, là một
  connection pool, và là một dạng hỏng mới — api không với tới được — phải được xử
  lý chứ không thể giả định cho qua.
- **Một credential phải quản lý.** Xác thực service-với-service trước đây không tồn
  tại trong nền tảng này; giờ thì có, và đó là thêm một secret với một câu chuyện
  rotate mà ai đó phải gánh.
- **Một deployable phải vận hành.** Thêm một image, thêm một health check, thêm một
  thứ có thể chết. File compose phình ra, và CI cũng vậy.
- **Một internal API giờ đã là một hợp đồng.** `/internal/dispatch` có hai bên tiêu
  thụ — dispatcher và các bài test — và đổi nó nghĩa là đổi cả hai.

Đánh đổi ấy có đúng hay không phụ thuộc vào tính chất đang được mua. Ở đây nó là sự
cô lập giữa một server hỏng của khách hàng này và dịch vụ của mọi khách hàng khác,
và điều đó đáng giá một network hop. Nó sẽ không đáng cho outbox relay, và đó chính
xác là lý do relay vẫn nằm trong api.

## Đường nối, và credential băng qua nó

Dispatcher không ghi được vào PostgreSQL, nên mọi thứ nó cần thay đổi đều đi qua
api. Điều đó nghĩa là một loại caller mới, và một loại principal mới:
...
```
Hãy nhìn `environmentId?: undefined` trên platform principal. Đó không phải trang
...
**Một credential với tới mọi environment thì tuyệt đối không được chấp nhận trên
route công khai.** Nếu được, một caller cầm nó có thể đọc dữ liệu của bất kỳ tenant
nào qua chính API thông thường — đó là một lỗ thủng xuyên tenant với một credential
hợp lệ đứng ngay trước mặt, đúng cái hỏng hóc mà hiến pháp I sinh ra để ngăn.

Middleware từ chối nó theo *loại* chứ không theo danh sách route, bởi danh sách
route là thứ mà một route tương lai có thể quên có tên trong đó:
...
environment rồi return sớm khi không có — và CI thì chưa bao giờ set nó. Suốt vài
lần build, cái assertion duy nhất đứng giữa một platform credential và một route
công khai đã chẳng làm gì cả, mà vẫn báo thành công. Một security test tự bỏ qua
chính mình còn tệ hơn không có test: nó trả lời một câu hỏi nó chưa từng đặt ra.
...
```
Bản thân đường nối gồm ba thao tác, và chúng được định hình bởi những gì dispatcher
*không được phép* làm, chứ không phải bởi những gì nó muốn. Nó không nở được event,
nên nó hỏi. Nó không đọc được secret, nên nó hỏi. Nó không ghi được kết quả, nên nó
báo cáo:
...
```
Cả hai phía cùng import các schema đó, và đó là thứ chặn hợp đồng trôi dạt:
dispatcher không thể gửi một hình dạng mà api không parse nổi, bởi hình dạng ấy là
một định nghĩa duy nhất chứ không phải hai bản hôm nay còn khớp nhau.
...
`InternalModule` tự cung cấp `"DB"` của riêng nó thay vì import, và cái provider
thiếu ấy đáng giá một buổi chiều mất trắng. `MessagesModule` *provide* `"DB"` nhưng
không *export* nó, nên một module tưởng rằng mình với tới được đã phân giải ra con
số không — và Nest báo điều đó thành một lỗi runtime ở bất kỳ route nào chạm
database trước tiên, chứ không phải một lỗi đấu dây lúc khởi động. Lane unit rớt từ
156 test xuống 40 và hai suite nguyên vẹn thất bại trước khi nguyên nhân lộ ra.

</Trap>

## Lại là relay

Delivery relay chính là vòng lặp của chương 3.3 với thêm một predicate. Chỉ có thế,
và việc nó không cần một abstraction mới nào chính là bằng chứng rằng 3.3 đã vạch
đường nối đúng chỗ:
...
```
`SELECT … FOR UPDATE SKIP LOCKED` làm đúng công việc nó đã làm ở 3.3 — hai instance
api cùng rút cạn một bảng sẽ lấy những dòng khác nhau thay vì giành nhau cùng một
dòng — còn `next_attempt_at <= now()` là nửa mới. Một dòng chưa tới giờ đơn giản là
không được chọn. Nó không giữ gì, không chặn gì, và không tốn gì ngoài một mục
index.

Có đúng một thứ buộc phải đổi trong publisher của 3.3, và đó là một chỉnh sửa nhỏ
cho một giả định chứ không phải một cơ chế mới. Publisher đảm bảo stream `EVENTS`
tồn tại lúc kết nối, vì ở 3.3 đó là stream duy nhất. Stream thứ hai biến câu "tôi
đảm bảo stream nào tồn tại" thành một tham số:
...
+    await ensure(nc);
   ...
   return {
```
Nếu không có nó, delivery đầu tiên publish vào `deliveries.*` trả về `503` từ một
broker chưa bao giờ được cho biết stream ấy nên tồn tại.

## Dispatcher

Hai consumer, làm hai việc khác nhau. Một cái nở event thành các dòng delivery; cái
kia post những delivery mà relay đã quyết định là tới hạn:
...
Tuần tự hoá *bên trong* một endpoint cũng quan trọng, theo chiều ngược lại. Hai lần
thử cùng lúc vào cùng một khách hàng sẽ để một retry vượt mặt chính lần thử mà nó
đang retry — và một khách hàng có server vốn đã chật vật là người cuối cùng đáng bị
trao thêm concurrency.

<Trap>

**Một lần broker gửi lại không phải một lần retry của khách hàng, và nhập nhằng hai
thứ đó là tiêu ngân sách này vào ngân sách kia.** `max_deliver: 10` trên consumer
giới hạn số lần *broker* trả lại một job mà dispatcher hoàn toàn không xử lý được —
một cú crash, hoặc một api không với tới. Bảy lần thử trong bảng tier giới hạn số
lần *khách hàng* bị liên hệ. Chúng là hai con số khác nhau đếm hai thứ khác nhau,
và code nói thẳng điều đó ở chỗ mà lẽ ra ai đó sẽ phải đoán.

</Trap>

Về phía dispatcher, expansion chỉ là một request và không gì khác. Nó không ghi
được các dòng, nên toàn bộ công việc của nó là giải mã một event nó không tin và
nhờ api làm phần việc:
...
```
Bản thân một lần thử thì buồn tẻ có chủ đích, và đó mới là điểm mấu chốt — mọi thứ
thú vị đã được quyết trước khi nó chạy:
...
Bảy lần thử không phải "cho tới khi thành công". Đó là một quyết định rằng có những
delivery sẽ không bao giờ thành công, và nền tảng dừng lại thay vì retry mãi mãi:
```ts title="services/api/src/db/repository.ts (excerpt)"
    ...
    return { outcome: "dead_lettered" as const, nextAttemptAt: null };
```
Dead letter giữ lại payload và status cuối, bởi rồi khách hàng sẽ hỏi chuyện gì đã
xảy ra và "nó hỏng" không phải một câu trả lời. Nó được giữ bảy ngày và **replay
được** — và bản replay dùng lại đúng event id gốc, đó mới là chi tiết đáng kể. Một
lần replay là chính event ấy được gửi lại, không phải một event mới, nên khách hàng
đang deduplicate theo event id sẽ xử lý đúng mà không cần được dặn thêm gì.

<Why>

**Vì sao không tự động tắt một endpoint đã hỏng nhiều ngày?** Nhật ký lần thử của
FR-WHK-06 và auto-disable của FR-WHK-07 đều được hoãn sang một chương sau, và sự
trì hoãn ấy là cố ý chứ không phải bỏ sót. Auto-disable là một quyết định chính
sách với hệ quả khách hàng nhìn thấy được — nó ngừng giao webhook cho người đang
trả tiền — và nó cần nhật ký lần thử để tự biện minh với khách hàng về sau. Ship cơ
chế mà không có bằng chứng nghĩa là tắt endpoint rồi không giải thích nổi vì sao.

</Why>

## Một walk tìm ra bug thật

Mọi chương trong loạt bài này đều ship một script người đọc chạy được. Chương này
ship hai: một endpoint cư xử tệ theo lệnh, và một walk đưa đúng một event đi trọn
đường tới đó.
...
Walk tự spawn api của riêng nó, bởi dispatcher chạm tới state qua HTTP và phải có
thứ gì đó ở đầu bên kia. Một walk bảo người đọc khởi động ba service trước là một
walk không ai chạy:
...
process.exit(0);
```
Chạy walk với `--mode=fail` và nhìn lịch trình giãn ra:
```text
...
```
Chỉ có điều lần đầu tiên nó không làm thế. Nó làm thế này:
```text
...
=== endpoint attempts: 1 ===
```
Một lần thử, rồi không gì nữa, mãi mãi.
...
**Mọi webhook thất bại đã được retry đúng không lần nào, và toàn bộ lịch trình là
không thể chạm tới.**

<Trap>

Không gì bắt được nó, và lý do còn đáng giá hơn cả con bug. Suite delivery gọi thẳng
query claim, không có broker nào trên đường. Suite của dispatcher dùng một *delivery
mới toanh* cho mỗi trường hợp. Nên chưa bài test nào trong repository từng publish
cùng một delivery hai lần — đúng cái tình huống mà mọi lần retry đều tạo ra.

Bản sửa gọn một dòng, và lời chú thích bên cạnh còn dài hơn cả thay đổi, bởi thay
đổi không phải phần thú vị:
...
Bài test hồi quy chính là walk, được đem vào CI — và nó đã được xác nhận là *thất
bại* trước khoá cũ rồi mới được giữ lại:
...
```
## Các suite thật sự nắm giữ điều gì

Ba suite unit và ba suite integration tới cùng chương này. Nhóm unit thì rẻ và ghim
chặt phần số học — bảng tier, vòng mã hoá, công thức chữ ký:
...
```
Nhóm integration mới là nơi các bảo đảm thực sự sống, bởi mỗi bảo đảm trong đó đều
nói về một thứ chỉ tồn tại khi một broker thật, một database thật và một server
thật của khách hàng cùng nằm trong một câu chuyện:
...
**Một deployable mới thoát khỏi thiết bị đo coverage mà chẳng ai gỡ nó ra cả.**
Research R12 đã dự đoán điều này và nó xảy ra đúng như mô tả, chỉ khác ở chỗ tệ hơn
dự đoán theo một hướng và tốt hơn theo một hướng khác.

Tốt hơn: thiết bị đo không cần đổi gì cả. Glob của nó là `services/*/src/**`, nên
dispatcher được đo ngay khoảnh khắc nó tồn tại, và không job CI nào phải thêm vào
bởi mọi lệnh trong workflow vốn đã chạy ở gốc workspace.

Tệ hơn: đọc nó thì thấy `expand.ts` đang ở mức **0%**. Chính suite của dispatcher
lại đi tới expansion bằng cách gọi thẳng repository vào database, nên cái consumer
giải mã một event, nhờ api nở nó ra, rồi quyết định ack-hay-terminate chưa từng một
lần chạy dưới test.

Còn `repository.ts` thì đi *xuống* — branch từ 86.30% còn 78.22%, rớt bốn ngưỡng
cùng lúc. `deliveryMaterial` và `pendingDeliveryDepth` chỉ được dispatcher gọi, mà
suite của dispatcher chạy api như một **process con**, và coverage của process con
thì không quy về đâu được. Đúng cái hàm duy nhất trong nền tảng trả lại signing
secret của khách hàng ở dạng plaintext, theo thước đo duy nhất mà hiến pháp nêu
tên, là chưa được test.

Ratchet được nâng lên sau đó, chứ không phải hạ xuống cho vừa code:
...
Nên cả ba cùng có Dockerfile trong một chương, và cái của api là cái đáng đọc — hai
cái kia cùng hình dạng nhưng ít thứ hơn:
...
`pnpm exec turbo run build --filter=@relay/api` chứ không phải
`pnpm --filter @relay/api build`: filter của pnpm chạy script trong một package và
**không** build các workspace dependency mà nó cần. Bên trong container, nơi chẳng
có gì được build sẵn, điều đó tạo ra mười bảy lỗi `TS2307 cannot find module` chỉ
thẳng vào những package đang tồn tại. Filter của turbo hiểu đồ thị phụ thuộc và
build `^build` trước.

Và `pnpm deploy --legacy`: dưới pnpm 10, `deploy` từ chối chạy trong một workspace
không có `inject-workspace-packages`, còn thông báo lỗi thì không hề nhắc tới cái cờ
khiến nó chạy được.

</Trap>

Dispatcher là một workspace package mới, không framework và dùng ESM — ADR-15 buộc
NestJS chỉ thuộc về api service, và một ứng dụng Nest thứ hai sẽ là chuyện chọn
framework theo quán tính chứ không phải theo quyết định:
...
+    "start": "node dist/main.js",
   ...
   "dependencies": {
     "@relay/protocol": "workspace:*",
```
Package mới gia nhập đồ thị build, một thay đổi gọn một dòng mà sẽ vô hình cho tới
lúc có thứ gì đó không chịu rebuild:
...
```
File compose có thêm ba service nấp sau một profile, nên `docker compose up -d
--wait` mặc định vẫn chỉ dựng bốn store và không gì khác — người đọc đi theo chương
...
+
 volumes:
   ...
   clickhouse-data:
```
Mục `extra_hosts` có mặt vì đúng một lý do. Một dispatcher bên trong container phân
...
Dựng store, build, migrate, rồi chạy walk với một endpoint khoẻ mạnh:
```console
...
```
Rồi bảo nó verify một body đã bị serialise lại, và nhìn hợp đồng làm đúng việc của
mình:
```console
...
```
Giờ nền tảng đã giao webhook tới khách hàng, ký chúng để khách hàng chứng minh được
chúng đến từ chúng ta, retry một endpoint đang hỏng theo lịch giãn dần, và bỏ cuộc
theo một cách mà ai đó giải thích lại được về sau. Nó làm tất cả những điều đó từ
một service không ghi nổi vào chính database của mình.

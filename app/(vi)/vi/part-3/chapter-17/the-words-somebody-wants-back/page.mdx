...
export const metadata = {
  ...
  description:
    "Message deletion cần tombstone, authorization và read-path semantics nhất quán. Chương này đưa delete event qua protocol, storage và mọi reader có cursor.",
  alternates: {
...
Xóa message không phải là một message mới. Nó là tombstone: event nói nội dung không còn
visible, trong khi sequence và cursor vẫn cần giữ lịch sử nhất quán.

## Tombstone không phải message

Xóa message không phải sửa content thành chuỗi rỗng. Tombstone là một trạng thái riêng: history giữ sequence và author để cursor không gãy, nhưng không còn tiết lộ text hay attachment đã bị xóa.
...
## Subject grammar thứ năm

Delete event cần subject grammar riêng để gateway route đúng channel và client phân biệt nó với message mới. Grammar là contract cross-service; đừng để một service tự ghép topic bằng string mà service khác không thể validate.
...
## Hai code, và tại sao không phải `forbidden`

Không tìm thấy message và không được phép xóa nó có observable semantics khác nhau, nhưng resource tenant khác vẫn không được làm oracle. Error code phải phân biệt action mà caller được biết, không tiết lộ resource ngoài scope.
...
## Table SAD đã publish nhưng chưa ai build

SAD đã mô tả tombstone table/state, nhưng architecture diagram không tạo migration. Chương này biến promise thành schema, index và read-path behavior có test thay vì để một table trên giấy giả làm implementation.
...
## Check chỉ đếm một hop

Authorization không chỉ kiểm tra author id. Nó đi từ caller qua membership, channel và environment đến message; bỏ một hop có thể cho id hợp lệ vượt scope. Guard phải được test với row bait ở từng quan hệ.
...
Edit và delete không nên bị trộn thành cùng một mutation. Edit giữ message sống với content mới; delete tạo tombstone và fan-out event khác. Hai lifecycle khác nhau cần audit và client rendering khác nhau.
...
## Deletion, và tại sao hai 204 không chứng minh gì

Hai lần DELETE trả 204 chỉ chứng minh operation idempotent ở HTTP surface. Nó chưa chứng minh history không lộ content, socket nhận event, hay user không phải author bị từ chối. Test phải quan sát từng hậu quả đó.

## Mọi read path làm gì với tombstone

History REST, socket replay, search hay bất cứ reader nào phải render cùng một tombstone shape. Nếu một reader quên filter, deletion chỉ tồn tại ở endpoint được demo chứ không tồn tại trong product.

## Điểm mù của cursor

Cursor theo message còn sống có thể nhảy qua sequence đã tombstone và khiến page boundary không ổn định. Tombstone vẫn chiếm vị trí trong stream; cursor phải dùng ordering key bền vững thay vì đếm item client nhìn thấy.
...
## Counter mà cursor cần

Channel sequence tiếp tục tăng và deletion event tham chiếu sequence của message bị xóa. Counter đó cho phép client merge realtime với history mà không cần phát minh một thứ tự riêng cho tombstone.

Phần trên khảo sát ba product rồi từ chối thêm counter. Phần này thêm nó vì chính kết luận
đã làm argument từ chối không còn đứng vững: **bound đã được viết ra, nhưng bound client
không thể hành động theo không phải design mà chỉ là note.** Client không phân biệt được
message đã edit với chưa edit thì không biết khi nào phải đọc lại history; “đọc lại history”
lại là remedy duy nhất ADR-07 dựa vào.

Relay lấy shape của IMAP: monotonic number thứ hai cho mỗi channel, đặt cạnh sequence, và
trả giá IMAP trả—mọi mutation phải maintain nó.

**Một column, đi cùng migration của chính chương này.** `message_edits` giữ nội dung cũ;
`channels.revision_sequence` nói message đã đổi bao nhiêu lần. Hai thứ là một subject;
tách chúng thành hai migration sẽ đánh số một thứ sau work không liên quan.

**Tăng đúng một lần cho mỗi edit hoặc deletion, trong transaction thực hiện nó, và không
bao giờ khi send.** Increment nằm trong hai path edit/delete đã fence trước đó, cùng
transaction với write nó đếm. Đặt ngoài transaction tạo statement thứ hai có thể fail;
counter có thể bỏ sót còn tệ hơn không counter vì client sẽ tin nó.

**Count được report, không bao giờ được compare.** API trả nó trên call gateway đã thực hiện:
...
Integration test bắt read path quên tombstone, gateway route sai subject và guard thiếu hop. Đó là các lỗi một unit test của repository thường không thấy vì chúng xuất hiện giữa storage, protocol và client-facing response.
...
## Requirement hiện đứng ở đâu

Người có quyền có thể xóa message, mọi reader thấy tombstone nhất quán, và stream vẫn có ordering. Đây là deletion có semantics, không phải một `DELETE` làm row biến mất.

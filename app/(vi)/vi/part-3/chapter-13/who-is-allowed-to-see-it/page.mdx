...
export const metadata = {
  ...
  description:
    "Presence chỉ có nghĩa khi đúng người được thấy. Chương này biến frame đã khai báo thành state có TTL, scope theo membership và failure semantics khi Redis unavailable.",
  alternates: {
...
Presence từng là một từ trong protocol vocabulary mà platform không bao giờ nói ra. Để nó
có meaning cần state, expiration, audience và một answer rõ ràng khi store không có.

## Một từ trong vocabulary mà không gì từng nói

Presence đã có trong vocabulary của protocol nhưng chưa service nào phát nó. Một frame được khai báo không tạo ra state, quyền quan sát hay thời hạn sống; chương này đưa cả ba vào một contract mà client có thể dựa vào.

`presence.changed` đã nằm trong frame union nhưng không có emitter. Declared frame không
phải capability; capability bắt đầu ở state transition có người quan sát được.
...
## Plumbing reader được hứa, và tại sao không phải đây

Reader của presence không nên bị nhét vào mọi request HTTP. Gateway là nơi biết một socket còn sống, Redis là nơi chia sẻ state giữa gateway instance, và client chỉ nhận update trong scope nó đã subscribe. Tách plumbing khỏi reader giữ REST surface không mang một trạng thái vốn là tạm thời.

Presence không thay history hay resume. Nó là ephemeral state; message persistence vẫn đi
qua database và cursor. Trộn hai loại state sẽ làm client suy diễn sai guarantee.
...
## Key mà sự tồn tại chính là state

Presence dùng key có TTL: key tồn tại nghĩa là user đang hiện diện; expiry nghĩa là state tự biến mất khi connection chết không sạch. Heartbeat gia hạn TTL, còn disconnect xóa key khi có thể. Không cần một cron job đoán socket nào đã mất.

Redis key có TTL đại diện online presence. Heartbeat gia hạn key; absence sau grace period
là offline, không cần một record delete bền vững.
...
## Grace period, và một fix còn tệ hơn bug

TTL quá ngắn làm mạng chập chờn biến thành một chuỗi offline/online gây nhiễu. Kéo dài grace period vô hạn lại giữ presence ma quá lâu. Chọn khoảng thời gian có đo lường, refresh trước expiry và chỉ emit transition thật sự thay vì mỗi heartbeat.

Disconnect tức thì không phải offline chắc chắn vì network jitter và reconnect. Grace period
tránh flap, nhưng fix không được giữ stale timer làm user vừa reconnect lại bị broadcast
offline.
...
## Ai được phép nhìn thấy nó

Presence chỉ được fan out tới người cùng channel hoặc quan hệ membership được policy cho phép. Cùng tenant chưa đủ: một user không được biết mọi người trong tenant đang online. Publisher phải resolve audience từ authorization data, không tin channel id do client đưa vào frame.

Presence audience theo channel membership. Người cùng tenant nhưng không phải member không
được nhận state; visibility check phải dùng cùng rule mà socket session dùng.
...
## Khi store không ở đó

Redis unavailable không được biến thành “mọi người đều online” hoặc một authorization bypass. Gateway từ chối hoặc degrade presence rõ ràng, log/measure failure và vẫn giữ message path có semantics riêng. State tạm thời phải fail closed khi visibility không thể được chứng minh.

Redis unavailable không được tạo online/offline fiction. Gateway degrade theo policy được
ghi rõ và log failure để operator biết presence không authoritative.

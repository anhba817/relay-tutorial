...
export const metadata = {
  ...
  description:
    "Một frame đã nằm trong union giao thức từ chương 1.3 và chẳng gì từng tạo ra một cái. Presence có văn phạm subject riêng, một khoá mà sự tồn tại của nó chính là trạng thái, và một phạm vi là tô-pô chứ không phải một bộ lọc.",
  alternates: {
    canonical: "/part-3/chapter-13/who-is-allowed-to-see-it",
    languages: {
      en: "/part-3/chapter-13/who-is-allowed-to-see-it",
      vi: "/vi/part-3/chapter-13/who-is-allowed-to-see-it",
    },
  },
};
...
Mai mở app. Tuấn có trong channel, tên anh nằm trong danh sách thành viên, và bên cạnh nó là một
vòng tròn chưa bao giờ có màu — bởi chẳng gì trong Relay từng nói anh có ở đó hay
không.

Cái frame để nói điều đó đã tồn tại từ chương 1.3. `presence.changed` nằm trong union
giao thức với một schema nghiêm ngặt và hai trạng thái. `frames.test.ts` khẳng định hình dạng của nó. Chương isolation
harness liệt kê nó trong số các frame mà một client không được giả mạo, kèm một lý do
viết bên cạnh. Mười hai chương của Phần 3 đã ship một từ vựng mà chẳng gì
nói ra.

Chương này cho nó một bên tạo ra, và gần như chẳng phần nào của công việc là chuyện tạo ra.

<Figure caption="Hai tấm vải trên một Redis, và cái channel mang cả hai" code={figTwoFabrics} />

## Một từ trong bộ từ vựng mà chẳng gì từng nói

`packages/protocol/src/frames.ts`, không đổi bởi chương này:
...
Lời chú thích đó đã đúng và đã nhàn rỗi từ chương socket. Phiên bản sắc nhất là của
chương isolation harness, thứ dựng một gauntlet khẳng định rằng mọi kiểu frame đi ra đều bị từ chối
khi một client thốt ra nó:
...
Dòng đó đã xanh từ khi chương isolation harness viết nó. Nó chứng minh rằng một client không tuyên bố được
một lần chuyển trạng thái presence, trên một
hệ thống mà trong đó chẳng ai tạo ra được một cái — gateway giữ các kết nối và chẳng suy ra gì
từ chúng. Một bài test có thể xanh về một năng lực không tồn tại, và bài này đã thế.

FR-RTM-05 gọi tên sáu kiểu event thời-gian-thực. Cả sáu đều có frame trong union. Trước chương
trước, **chẳng cái nào** có bên tạo ra; chương fan-out cho `message.created` một cái, còn chương này là
cái thứ hai. `message.updated`, `message.deleted`, `membership.changed` và `typing` là bốn từ đã khai báo
mà chẳng có gì đằng sau vào lúc chương này ship — chương membership-revocation cho cái thứ ba một
bên tạo ra còn chương typing cho cái thứ tư, để lại hai. Gọi tên chúng ở đây là có chủ đích — chính bản đặc tả của chương fan-out
tuyên bố rằng `typing` chẳng có frame nào cả, và đó là cái giá của một danh sách không được gọi tên.

## Cái đường ống mà một người đọc được hứa, và vì sao nó không phải thứ này

Chương 2.6 kết thúc với ba tờ giấy nợ. Tờ thứ ba:
...
Presence không tái dùng nó, và lý do thì là một màn mở đầu tốt hơn một lời xin lỗi. Đường message
được gắn kiểu vào message ở ba điểm, và chúng không nằm cả trong một file. Hai điểm là:
...
Cái thứ nhất là một chữ ký hàm. Cái thứ hai parse mọi thứ tới trên `chan:{channel_id}` rồi ghi log
`fanout.invalid_payload` cho bất cứ thứ gì không phải một message. Cái thứ ba thì ở một chỗ kém
thuận tiện hơn — fan-out trao từng frame cho một callback mà tầng session đã đăng ký, và
callback đó là nơi cái kiểu cuối cùng được viết ra:

```text title="services/gateway/src/session.ts (excerpt)"
      send(connection.socket, { type: "message.created", payload: message });
```

`session.ts` được tám chương trước chương này fence — bốn ở Phần 2 và bốn ở Phần 3 —
và `deliver` là cái hàm mà phần lớn chúng nói về. Nới rộng nó không phải một lần sửa cục bộ lên một
tấm vải; nó là một lần sửa lên một thứ mà tám chương mỗi chương đều đã dạy một người đọc cách dựng.
Một payload presence publish trên chính subject của một channel thì hôm nay dù sao cũng chẳng tới được nó: lần parse
ở trên từ chối nó, ghi log `fanout.invalid_payload`, và chẳng tạo ra frame nào.

Nên phương án thay cho một subject thứ hai chưa bao giờ là "hãy giữ cho đơn giản". Nó là "hãy sửa cái đường
lưu lượng cao nhất trong hệ thống để phục vụ lưu lượng thấp nhất trên nó", rồi đặt một lần parse union có
discriminator lên mọi message mà mọi instance nhận, mãi mãi. Trong một lần triển khai cuốn chiếu thì còn tệ hơn thế:
một instance cũ nhận payload có phong bì kiểu mới sẽ phát ra `fanout.invalid_payload` cho mọi
lần chuyển trạng thái trên mọi channel cho tới khi nó rút cạn.

Presence nhận `presence:{channel_id}` thay vào đó — một subject *suy ra từ* mỗi channel bị ảnh hưởng
chứ không phải subject của chính channel. Khán giả thì giống hệt. `fanout.ts` chẳng bị sửa gì cả.

<Why>

**Điều này mâu thuẫn với ADR-10 bằng văn bản, và đó là lý do có một ADR-19.** ADR-10 nói các lần chuyển trạng thái
publish "chỉ trên subject của các channel bị ảnh hưởng", còn hiến pháp thì nói một ADR đã được chấp nhận thì
không bao giờ được sửa đổi — muốn thay thế thì cần một bản ghi mới. Nên phần quyết định của ADR-10 không bị đụng và
mang thêm một cụm trên dòng trạng thái của nó; ADR-19 thay điều khoản về subject và không gì khác.
Presence vẫn sống trong Redis, vẫn hết hạn chứ không bị thu gom, và vẫn chẳng có service
riêng nào.

Phần khó chịu được nói ra trong ADR-19 chứ không bị bỏ ra ngoài: đây là **một nửa của chính phương thuốc
xem-lại của ADR-10, được dùng trước khi kích hoạt của nó nổ**. ADR-10 nói rằng trên ~30% lưu lượng publish của gateway,
"các subject presence sẽ có tấm vải riêng hoặc các channel tự chọn tham gia". Các subject presence giờ đã có
tấm vải riêng — vì một lý do chẳng liên quan gì tới lưu lượng, thứ mà chẳng gì ở đây đem đo.
Cái kích hoạt vẫn chưa được giải quyết và NFR-SCL-01 cũng vậy.

</Why>

Chi phí được khai báo là thêm một lượt đăng ký cho mỗi channel trên mỗi instance. Đó là một con số, nên nó được
đo chứ không được khẳng định suông — `CONFIG RESETSTAT`, sáu bài test presence, `INFO commandstats`:
...
Mười hai: sáu fan-out và sáu presence, đúng một cái cho mỗi channel trên mỗi instance, với thành viên cục bộ
thứ hai của một channel chẳng thêm cái nào. `ioredis` nhận một `subscribe` biến thiên, nên số lượng gấp đôi và
số vòng đi về thì không.
...
Export nó là một dòng, và cái dòng đó chính là toàn bộ lý do file ấy tồn tại chứ không phải ba dòng
trong `fanout.ts`:
...
## Một khoá mà sự tồn tại của nó chính là trạng thái

Chẳng có bảng presence nào, chẳng có tập user đang online nào, và chẳng có con số đếm nào. Chỉ có một khoá cho mỗi user:
...
`SET … PX ttl NX` **chính là cuộc bầu chọn**. Đúng một bên gọi trên khắp mọi instance trong đội máy nhận được
`OK` cho một user vốn vắng mặt; mọi bên khác nhận null rồi im lặng. Hai kết nối mở
trong cùng một mili giây trên hai cỗ máy thì tạo ra một `online`, và chẳng máy nào biết máy kia
tồn tại — đó là hình dạng của ADR-07 từ chương 2.6 và là lý do chẳng gì ở đây cần một leader.

Câu hỏi về client bị crash cũng tự trả lời theo cùng cách. Một instance chết thì thôi làm mới;
khoá hết hạn; user trở thành offline trong cửa sổ đó. Không bộ thu gom, không tombstone.

<Trap>

**Ba con số ba-mươi-giây khác nhau sống trong hệ thống này và chúng là ba đại lượng.**
...
Với tay lấy `PING_INTERVAL_MS` bởi nó vốn đã là 30 s và vốn đã nổ trên một kết nối đang sống
là nước đi tự nhiên và nó sai: **một TTL bằng đúng chu kỳ làm mới của chính nó thì sẽ làm hết hạn một user
đang kết nối.** Mọi lần làm mới sẽ là một cuộc đua với chính cái hạn mà nó đang gia hạn, và thua cuộc đua ấy thì tạo ra một
`offline` cho một người đang ngồi ngay đó.

Lần làm mới chạy ở 10 s — ba lần cho mỗi TTL, nên hai lần trượt liên tiếp vẫn không làm hết hạn một user
đang sống. Hằng số nói thế ngay trong file, bởi người đọc kế tiếp sẽ có cùng ý tưởng ấy.

</Trap>

## Khoảng ân hạn, và một cách sửa tệ hơn cả cái bug

FR-RTM-06 cho một user đang rời đi ba mươi giây trước khi ai đó được báo. Cách cài đặt hiển nhiên
là để TTL của khoá làm khoảng ân hạn: thôi làm mới, và nó lụi đi. Điều đó sai hai lần, và
chỉ chạy nó với một Redis thật mới cho thấy cái nào.

**Thứ nhất**: không có một lần ghim lại, khoá chết vào `last_refresh + ttlMs`, tức là sớm hơn tới cả một
chu kỳ làm mới *trước khi* khoảng ân hạn kết thúc. Một user kết nối lại bên trong khoảng trống đó sẽ thấy khoá
vắng, thắng `SET … NX`, rồi publish một `online` thứ hai cho một người chưa hề rời đi. Nên lần
đóng sẽ ghim lại khoá đúng bằng `graceMs` với `XX`, để nó chết khi khoảng ân hạn kết thúc chứ không phải
sớm hơn tới mười giây.

**Thứ hai, và đây là cái đáng mang đi**: phiên bản đầu tiên của cách sửa đó lên lịch cho
phép kiểm vào đúng `graceMs`. Chuyện đó đặt hai cái hạn lên một khoảnh khắc, được đo bằng hai
đồng hồ khác nhau. Khoá hết hạn vào `close + δ + graceMs`, với δ là một vòng đi về; bộ hẹn giờ nổ vào
`close + graceMs + ε`. Khi ε nhỏ hơn δ thì phép kiểm thấy khoá còn sống, ghi log
`presence.suppressed`, và bộ hẹn giờ một-lần của nó thì *biến mất* — user bị mắc kẹt ở trạng thái online mãi mãi.
Redis cũng giữ một khoá cho tới khi bây giờ đã vượt hẳn qua hạn của nó, nên một lần hoà cũng rơi về cùng phía sai đó.
Một biên một giây không phải sự đệm thừa; nó là khác biệt giữa một bản trùng giả và một
lời nói dối vĩnh viễn.

<Figure caption="Một lần đóng, một lần ghim lại, và cuộc bầu chọn cho đúng một instance được nói" code={figGrace} />

Mảnh cuối là cuộc bầu chọn ở đáy sơ đồ đó. **Một TTL hết hạn thì chẳng publish
gì cả.** Redis không báo cho ai rằng một khoá đã biến mất, nên phải có ai đó nhận ra rồi nói ra — và
nếu hai instance mỗi cái đang giữ một kết nối cuối và cả hai cùng đóng trong một nhịp, thì cả hai đều nhận ra. Một
lệnh `SET … NX` thứ hai lên cái dấu offline sẽ cho đúng một trong hai quyền nói. Cái dấu ấy
được xoá bởi bất kỳ ai thắng lượt `online` kế tiếp, nên lần rời đi kế tiếp lại bầu chọn lần nữa.
...
## Ai được phép thấy nó

FR-RTM-07 nói một event presence chỉ tới được những user chia sẻ ít nhất một channel với
chủ thể. **Một người đọc đi tìm bộ lọc sẽ chẳng tìm ra cái nào.** Chẳng gì trong `presence.ts` hay
`session.ts` so hai tập membership. Quy tắc được thực thi bởi ba sự thật ghép lại:
...
Một instance chỉ nhận một lần chuyển trạng thái trên các subject nó đã đăng ký, và nó chỉ đăng ký các
channel mà các thành viên cục bộ của nó thuộc về. Một user chẳng chia sẻ gì với chủ thể thì chẳng nằm trên instance nào
nghe được chuyện đó — còn nếu họ tình cờ chung một instance với một người có nghe, thì
`subscribersOf` không trả về họ.

<Figure caption="Ba cách để không nhận được một lần chuyển trạng thái, và chẳng cách nào là một bộ lọc" code={figScope} />

Một channel riêng tư chẳng cần trường hợp đặc biệt nào: một người không phải thành viên thì không đăng ký, và đó cũng chính là
cơ chế xử lý một channel công khai mà họ chưa bao giờ gia nhập. Động từ thứ ba của FR-CHN-05 — *quan sát
presence* — được thoả mãn bởi tô-pô. Bài test cho nó vẫn tồn tại, bởi "chẳng cần trường hợp đặc biệt nào"
và "trường hợp ấy chưa bao giờ được cân nhắc" thì nhìn từ bên ngoài giống hệt nhau.

Registry có thêm một phương thức, và nó trả lời một câu hỏi thuần tuý cục bộ:

```diff title="services/gateway/src/registry.ts"
@@ -64,12 +64,26 @@ export class Registry {
...
+import { type Presence } from "./presence.js";
 ...
 import {
   ...
   flushable,
   highWaterMarks,
@@ -68,21 +70,39 @@ export interface SessionServerOptions {
...
@@ -107,12 +127,36 @@ export function attachSessions({
...
@@ -200,15 +244,27 @@ export function attachSessions({
...
@@ -222,27 +278,60 @@ export function attachSessions({
     socket.on("pong", () => {
...
**Handler đóng giờ mang hai ràng buộc thứ tự ở chỗ nó từng chẳng mang cái nào.** Presence được
báo *sau* `registry.remove`, bởi nó hỏi xem đây có phải kết nối cuối của user trên
instance này không và không được tính cả cái đang rời đi. Các lệnh huỷ đăng ký đi sau cùng.

Cái thứ nhất trong số đó không phải một thay đổi về phong cách. Với lệnh gỡ bỏ đặt sau lời gọi presence,
`connectionsFor` vẫn thấy cái kết nối đang đóng, con số là 1 chứ không phải 0, chẳng lần kiểm ân hạn nào
được lên lịch, và **user ở lại trạng thái online mãi mãi**. Một bài test khẳng định việc lên lịch ấy vì đúng lý do đó,
và nó thuộc loại bài test phải được nhìn thấy đỏ mới tin được.

</Why>

<Trap>

**Ba thứ chương này không làm, nói ra ở đây chứ không để bị phát hiện.**

Chẳng có **ảnh chụp lúc connect**. Một client mở một socket thì học được về các lần chuyển trạng thái từ
khoảnh khắc đó trở đi và chẳng biết gì về trạng thái trước đó, nên một danh sách khởi đầu rỗng rồi lấp dần khi
người ta di chuyển. Mọi thiết kế presence rồi cũng cần ảnh chụp đó; cái này thì chưa có.

Một user **gia nhập một channel trong lúc đang kết nối** thì vẫn vô hình với các thành viên của channel đó cho tới khi
họ kết nối lại. Tập đăng ký được lấy một lần, lúc connect, từ response session. Đó là
sự cũ kỹ của FR-RTM-10 đội một cái mũ khác, và ở đây nó cố ý không được sửa — chương membership-revocation
sửa cả hai cái mũ bằng một cơ chế.

Một instance **chết bên trong cửa sổ ân hạn** thì chẳng publish `offline` nào. Phép kiểm đang chờ thì sống
trong chính tiến trình đó. Khoá hết hạn trong im lặng, và những người theo dõi giữ một vòng tròn xanh cũ cho tới khi
chủ thể chuyển trạng thái lần kế tiếp. ADR-10 cho phép presence được phép mất mát; một người đọc vẫn nên được cho biết
những mất mát nào là thật.

</Trap>

## Khi kho lưu trữ không có ở đó

Presence là hệ thống con duy nhất trong Relay được phép suy giảm. Một gateway mà Redis của nó chết
thì vẫn phải nhận socket và vẫn phải giao message — và rủi ro khi viết điều đó ra là
**một hệ thống con chẳng làm gì cả thì cũng thoả mãn nó y hệt**.

Chương fan-out đâm vào chuyện này đúng như thế: publisher của nó nuốt lỗi của chính mình rồi resolve, nên "lệnh gửi
trả về 201 trong khi Redis đang chết" cũng đúng với một publisher chưa bao giờ được đấu dây. Cái
khẳng định mang cái yêu cầu ấy thì không phải cái socket. Nó là dòng log:

```text title="services/gateway/src/presence.ts (excerpt)"
logger.log("error", "presence.failed", { op, error: String(error) });
```

Để dòng đó xuất hiện thì cần thêm một thay đổi nữa so với dự kiến. Mặc định `ioredis` thử lại mãi mãi
và **xếp hàng** các lệnh, nên với một cổng chết thì một lệnh `SET` chẳng thành công cũng chẳng reject — nó chờ,
và cái đường thất bại ghi trong module thì chẳng bao giờ được đi. Client lệnh được dựng với
`maxRetriesPerRequest: 0` và một connect timeout một giây vì lý do đó. Bên đăng ký giữ
giá trị mặc định, bởi nó *phải* kết nối lại khi kho lưu trữ quay lại; đó là thứ mà "lần
chuyển trạng thái kế tiếp publish được mà chẳng cần khởi động lại" dựa lên.

**Client Redis của presence sẽ cần một phần miễn trừ lint, và chương này không viết được nó.** Một quy tắc
hạn chế `ioredis` vào các file giữ bộ đếm giới hạn tần suất — và quy tắc ấy tới cùng
chương limits, chín chương sau chương này. Ở đây chẳng có gì để mà được miễn trừ khỏi.

Đó không phải một khoản tiết kiệm, và lý do đáng được nói ra trước khi ai đó viết phần miễn trừ theo
trí nhớ. **Mục của chương fan-out thì không chuyển sang được.** Lời biện minh của nó là *"client này
chẳng chạm khoá nào"* — một lệnh publish lên `chan:{channel_id}`, một UUID channel, và một subject thì chẳng
đọc được gì cả. Client của presence thì chạm khoá, và chúng có giới hạn theo environment:
`presence:{env}:{user}`, đó đúng là cái hình dạng mà phần hạn chế tồn tại để canh gác.

Lời biện minh mà presence cần chính là của bộ giới hạn tần suất: mọi khoá đều được ghép từ
environment id trên danh tính của kết nối đã xác thực, chẳng gì ở đây nhận một environment id
từ một client, và chẳng có lệnh scan, `KEYS` hay lần đọc theo mẫu nào có thể với tới khoá của tenant
khác. Phải có ai đó kiểm lại điều đó đối chiếu với code ở thời điểm ấy, và đó là lý do nó được
ghi cho chương đưa quy tắc vào chứ không phải được nhớ suông.

**Chép lời biện minh của người hàng xóm gần nhất chính là cách một sự hạn chế trở thành một thủ tục hình thức**, và một
khoản nợ thì dễ trả sai hơn một thay đổi.
...
@@ -31,21 +32,29 @@ export function createServer(logger?: Logger) {
...
`presence.changed` có một bên tạo ra lần đầu tiên kể từ chương 1.3. Sự xuất hiện của một đồng thành viên
tới được một người theo dõi đang kết nối qua hai instance; một người theo dõi chia sẻ ba channel thì nhận một
frame; một lần rời đi tạo ra đúng một `offline` sau cửa sổ và chẳng gì trước đó; một
lần kết nối lại bên trong cửa sổ thì chẳng publish gì cả; và một người không chia sẻ, một người không phải thành viên của một
channel riêng tư và một user xuyên tenant thì mỗi bên đều chẳng nhận gì trong một lần chạy mà một đồng thành viên có nhận.

    25 suite · 406 bài test tích hợp · xanh
    presence: 38 bài test tích hợp, 8 bài test unit gateway, 6 bài test unit protocol
    hai instance, ba channel: 12 SUBSCRIBE, 18 PUBLISH, 12 SET, 7 DEL

Dòng cuối được đo lại chứ không được mang sang: `CONFIG RESETSTAT`, sáu bài test của
kịch bản xuất hiện, `INFO commandstats`. **Bốn con số quay về giống hệt những con số mà
chương được viết lần đầu cùng** — đó là thứ bạn sẽ trông đợi ở một con số đếm vốn là một tính chất
của giao thức chứ không phải của cái lane, và nó đáng được kiểm chính bởi kỳ vọng ấy
dễ được giữ mà chẳng cần bằng chứng.

Nó đáng được nhìn thêm một lần nữa. Mười hai lần connect tạo ra mười hai `SET … NX` và bảy lần thắng,
mà chỉ mười tám lần publish — sáu lần chuyển trạng thái trên ba channel. Người thắng thứ bảy là
user chẳng thuộc channel nào: nó thắng cuộc bầu chọn, xoá cái dấu, rồi publish lên chẳng
subject nào cả. Đó là trường hợp suy biến của FR-RTM-07, tới nơi từ hướng ngược lại.

</Checkpoint>

<ForwardRef>

**FR-RTM-10 vẫn chưa được đáp ứng, và chương fan-out nói rằng chương này sẽ sửa nó.** Tiền đề ở đó
là presence cần đúng cái lần đẩy membership mà đường message cần. Nó không cần: presence
cần tập channel của chủ thể *vào lúc chuyển trạng thái*, và `POST /internal/session` vốn đã
cung cấp sẵn. Sự cũ kỹ thì có thật — một channel gia nhập giữa chừng kết nối thì vô hình cho tới khi kết nối lại
— nhưng nó là một lần đọc lại mà tầng session nợ, chứ không phải một cơ chế mà chương này dựng được.
**Chương membership-revocation trả nó**, và sự cũ kỹ ra đi theo: đăng ký subject presence của một channel
là một trong ba việc mà đường thêm thành viên của chương ấy làm.

**Danh sách vẫn cần một ảnh chụp.** Mọi client render presence đều sẽ muốn trạng thái lúc
connect, và hình dạng nó muốn thì không phải một dòng chảy. Đó là một endpoint đọc hoặc một frame, và nó là một
quyết định chứ không phải một sự bỏ sót.

**Bốn trong sáu kiểu của FR-RTM-05 vẫn chẳng có bên tạo ra.** `message.updated` và `message.deleted`
cần một thứ ghi vào một dòng message; `membership.changed` cần một bên tạo ra đấu vào một
bên ghi vốn đã tồn tại; `typing` thì có một frame, một hạn năm giây trong chính lời chú thích schema của nó,
và nửa còn lại của lời hứa ở chương 2.6 mà chương này đã phá một nửa. Liệu typing có tái dùng được
đường ống của fan-out hay không là câu hỏi mà nửa câu kia của 2.6 để ngỏ, và chương
này chỉ đóng lại nửa thuộc về presence.

**Chương typing đã trả lời: không.** Nó suy dẫn lại con số trong đoạn ở trên và tìm ra
bảy điểm gắn kiểu ở chỗ chương này nói là ba — nên typing cũng lấy một văn phạm subject thứ tư,
và lời hứa của 2.6 được cho nghỉ hưu chứ không được giữ.

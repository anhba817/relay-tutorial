...
export const metadata = {
  ...
  description:
    "Rate limit cần được thấy trước qua response, signal và docs. Chương này đặt fixed window counter, policy per tenant, REST/socket refusal và quota vocabulary cuối cùng được emit.",
  alternates: {
...
Giới hạn hữu ích là giới hạn developer nhìn thấy trước khi bất ngờ chạm nó. Counter, policy,
response field và docs phải cùng nói một sự thật.

```ts title="packages/protocol/src/codes.ts (excerpt)"
rate_limited: "too many requests",
```

## Requirement mà afterthought sẽ làm hỏng

## Tại sao window cố định
...
## Policy: ba column, và null không phải zero
...
## Hai vị trí trong chain, đều bị ép buộc
...
## Gateway: hai limit, hai shape refusal
...
## Một request, hai kiểu trust đối lập
...
## Signal không được tiêu một send
...
## Limit developer được kỳ vọng sẽ chạm

## Capture tìm ra gì mà test không thấy

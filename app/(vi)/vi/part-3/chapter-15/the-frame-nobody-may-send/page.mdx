...
export const metadata = {
  ...
  description:
    "Kế hoạch nói typing là kiểu thời-gian-thực duy nhất chẳng cần văn phạm subject mới nào. Suy dẫn lại con số đã quyết định điều đó thì tìm ra bảy điểm gắn kiểu ở chỗ bản ghi nói là ba. Và FR-RTM-08 đòi một cơ chế hết hạn mà nền tảng này không thực hiện được, đó mới là nửa thú vị hơn.",
  alternates: {
    canonical: "/part-3/chapter-15/the-frame-nobody-may-send",
    languages: {
      en: "/part-3/chapter-15/the-frame-nobody-may-send",
      vi: "/vi/part-3/chapter-15/the-frame-nobody-may-send",
    },
  },
};
...
Suốt hai mươi chương, một dòng trong `session.ts` đã quyết định một client được nói gì:
...
Một phép so, một hằng viết cứng. Mọi thành viên khác của union frame đều là thứ server
nói và một client chỉ được nghe. Lời từ chối ấy là bề mặt hẹp nhất trong hệ thống, nó
chưa bao giờ dịch đi, và chương này nới nó ra — đó là một thay đổi lớn hơn vẻ ngoài, bởi
đường nối đi vào là nơi một giao thức bị tấn công còn hai mươi chương test thì khẳng định rằng
đúng một kiểu được chấp nhận.

<Figure caption="Đường nối đi vào, sau chương này: hai lời từ chối, và bạn gặp cái nào thì tuỳ vào frame của bạn có parse được hay không" code={figInboundSeam} />

## Kế hoạch cho chương này thì sai, và một lệnh grep là thứ nói ra điều đó

FR-RTM-05 gọi tên sáu kiểu event thời-gian-thực. Chương fan-out cho `message.created` một bên tạo ra,
chương presence cho presence một cái, chương membership-revocation cho membership một cái. `typing` là cái thứ tư, và kế hoạch cho
các chương kết của Phần 3 nói nó là cái dễ: **kiểu duy nhất còn lại có thể tái dùng
`chan:{channel_id}` thay vì thêm một văn phạm subject thứ tư.** Theo từng channel, phù du, chẳng có
câu hỏi membership nào — lập luận của ADR-19 cho một văn phạm riêng thì trông như không áp dụng được.

Lập luận đó dựa trên một con số. ADR-19 nói đường message được gắn kiểu vào message *ở ba
điểm*, nên mang một kiểu thứ hai qua đó nghĩa là sửa ba chỗ. Ba thì rẻ. Ba thì
đáng trả để né một văn phạm thứ tư.

Suy dẫn lại nó thì trả về tám dòng phủ **bảy** điểm:
...
Kiểu của `deliver` ở `fanout.ts:62` và hai trong ba chỗ gửi `message.created` đã nằm
đó suốt từ đầu. ADR-19 đếm ra ba vào lúc nó được viết, chương membership-revocation trích lại cái ba
mà chẳng chạy lại gì, và chính mục nghiên cứu của chương này nói *bốn* trước khi một task
bắt nó chạy cái lệnh.

<Why>
Một con số trong một bản ghi quyết định là một phép đo có ghi ngày. Con số ba của ADR-19 đúng với
đoạn code trước mặt nó; hai chương sau thì cùng câu ấy mô tả một cái cây khác.
`git diff` tìm ra một dòng đã đổi còn chẳng gì tìm ra một câu thôi còn đúng bởi
code đã dịch chuyển bên dưới nó — và đó là lý do danh sách task có một pha chạy lại
các tiền đề trước khi dựng lên trên chúng, và là lý do pha này kiếm được chỗ của nó.
</Why>

**Lập luận không hề bị lời sửa chữa làm yếu đi. Nó mạnh hơn**, và giờ nó thuộc về
ba chương chứ không phải một:

> Một tấm vải sở hữu văn phạm subject của nó, và một kiểu không chia sẻ được kiểu payload thì không chia sẻ được
> một subject.

Nên typing lấy `typing:{channel_id}`, và văn phạm thứ tư được lấy chứ không được né.

<Figure caption="Bốn văn phạm trên một Redis, và tấm vải đầu tiên mà service này vừa publish vào vừa tiêu thụ từ đó" code={figFourGrammars} />

## Một hình dạng, ở chỗ membership cần hai

Chương membership-revocation cần một subject thứ hai bởi một lần thêm thành viên không đi nhờ được chính cái channel mà nó thêm bạn
vào: instance đang giữ thành viên mới thì không đăng ký channel đó, mà đó chính xác
là thứ đang đổi. Typing chẳng có trường hợp nào như thế. Một tín hiệu chỉ bao giờ thú vị với những người đã
ở trong channel, và một thành viên không nghe được subject thì chẳng có gì để mà được báo.
...
Bài test theo cặp là bài quan trọng, và nó có một nhánh mà task không hề đòi:
...
<Why>
Một `new Set` trên năm subject thì bắt được hai cái bằng nhau và không nhìn thấy được hai cái chung một
tiền tố. Hôm nay chẳng gì đăng ký theo wildcard, nên một vụ va tiền tố sẽ chẳng tốn gì hôm nay —
mà đó đúng là loại tính chất âm ỉ mà một bài test viết bây giờ thì rẻ còn một bài test
viết sau sự cố thì không.
</Why>

## Module này publish, thứ mà chẳng module nào khác làm

`chan:` được api publish. `member:` được api publish. `presence:` được publish
ở đây nhưng chẳng bao giờ được đọc ngược lại để giao. **Typing là tấm vải đầu tiên mà service này vừa
publish vào vừa tiêu thụ từ đó**: tín hiệu của một client tới gateway của chính nó qua
socket, instance đó đặt nó lên Redis, và chính instance đó đã đăng ký bởi nó giữ
các thành viên của channel ấy.

Chuyện đó có một hệ quả mà danh sách task đã hiểu sai. Nó nói một client Redis, mang theo
phát hiện của chương membership-revocation — rằng module ấy được *"viết với hai cái theo phép loại suy còn cái thứ hai thì được
tạo ra, được nghe, được đóng và chẳng dùng vào việc gì"*. Đúng với module đó, mà api của nó làm phần
publish. Đi theo nó ở đây thì tạo ra một module đăng ký thành công rồi
thất bại ở mọi lần publish nó thực hiện, bởi `fanout.ts:33` phát biểu quy tắc:

> một kết nối đã đăng ký thì không phát được các lệnh thông thường, nên bên publish và bên đăng ký phải là
> hai kết nối

<Trap>
Một bài học từ chương trước là một tuyên bố về code của chương ấy. Nó chỉ chuyển sang được
khi các hình dạng khớp nhau, và "một client, không phải hai" là một tuyên bố về một module chỉ bao giờ
nhận. Cái này là client Redis thứ bảy và thứ tám mà gateway giữ.
</Trap>
...
## Đường nối, và một cái tập tốn thứ mà một phép so thì không

Nới lời từ chối ra là ba dòng. Đây là chúng, rồi tới lý do vì sao phiên bản hiển nhiên
thì không biên dịch được:
...
@@ -19,12 +20,13 @@ import { WebSocketServer, type WebSocket } from "ws";
...
@@ -84,12 +146,27 @@ export interface SessionServerOptions {
   presence?: Presence;
...
@@ -106,12 +183,14 @@ export function attachSessions({
   logger,
   fanout,
...
@@ -137,12 +216,58 @@ export function attachSessions({
...
@@ -279,12 +404,18 @@ export function attachSessions({
...
+          typing?.subscribe(change.channel),
           ...
           () => {
             ...
             logger.log("info", "membership.applied", {
               change: "added",
@@ -335,13 +466,22 @@ export function attachSessions({
             op: "unsubscribe",
...
@@ -499,12 +639,19 @@ export function attachSessions({
...
@@ -604,14 +751,22 @@ export function attachSessions({
             });
           }),
...
@@ -758,12 +913,95 @@ export function attachSessions({
         0,
       ),
...
@@ -775,24 +1013,33 @@ export function attachSessions({
         connection.socket,
         "invalid_frame",
...
<Trap>
Thay `frame.data.type !== "message.send"` bằng `INBOUND_FRAME_TYPES.has(...)` thì biên dịch được
ở đường nối và làm vỡ đường gửi bốn mươi dòng bên dưới với ba lỗi `TS2339` — `channel`,
`text` và `idem_key` "không tồn tại trên kiểu". **Phép so đơn lẻ ấy đã và đang làm hai
việc mà chỉ một việc là nhìn thấy được**: nó từ chối các kiểu kia *và* thu hẹp
discriminated union cho mọi thứ sau nó.

Một predicate trên chuỗi kiểu thì biên dịch được, đọc lên đúng đắn, mà vẫn không thu hẹp —
TypeScript không đẩy ngược một phép thu hẹp trên `.type` về cái union mà nó đến từ. Predicate
phải nhận chính cái frame.
</Trap>

Phương án còn lại là một `if (type !== "message.send") return;` không với tới được, đặt sau nhánh
typing. Đó là code chết, trong một chương mà các ghim coverage là 100/100/100/100 — và
ratchet trong dự án này đã xoá code năm lần thay vì đi phủ nó.

## Hai hàm liền kề với hai quy tắc ngược nhau

`deliverTyping` bỏ qua mọi kết nối mà danh tính của nó là chính người phát tín hiệu. `deliverPresence`,
ngay bên dưới nó, chẳng bỏ qua ai — một user thấy được lần chuyển trạng thái presence của chính mình, và chương membership-revocation
đã xác nhận điều đó từ phía bên kia trong lúc đếm frame.

<Why>
"Bạn đang online" thì đáng báo cho các thiết bị khác của bạn. "Bạn đang gõ" thì không. Hai quy tắc
ngược nhau bởi hai sản phẩm ngược nhau, và nước đi tự nhiên — chép của hàng xóm — cho ra cái
sai, và đó là lý do file nói ra điều đó ngay tại chỗ mà ai đó sẽ chép.
</Why>

Và phép bỏ qua là theo **danh tính**, không bao giờ theo socket. Tín hiệu tới từ Redis mà chẳng có tham chiếu
socket nào, kể cả trên instance đang publish, và một user có thể giữ nhiều kết nối. Một phép so theo
socket sẽ trưng chỉ báo của chính một user lên chính thiết bị thứ hai của họ — và một bài test với một
kết nối cho mỗi user thì xanh theo cả hai đường, và đó là lý do có một bài test với ba kết nối.

## Con số mà chương này không làm cho đúng được

FR-RTM-08 nói:

> Các chỉ báo đang gõ sẽ tự hết hạn sau 5 giây không được làm mới và sẽ không
> được lưu lại.

Đọc thẳng thừng thì nửa thứ nhất là một nghĩa vụ của server. **Server không đáp ứng được nó**, và
lý do là một schema xuất bản từ chương 1.3:
...
Không có `state`. Không có hạn chót. Chẳng có frame nào để kết thúc một chỉ báo, và chẳng gì
ở đâu biết rằng một chỉ báo tồn tại — không bảng, không khoá Redis, không bộ hẹn giờ. **Một server không biết một
chỉ báo đã bắt đầu thì không thể thông báo rằng nó đã dừng.**

<Figure caption="Cái hạn hết hiệu lực, vốn là một bộ hẹn giờ mà server chẳng bao giờ chạm tới" code={figExpiry} />

Cách sửa hiển nhiên là một khoá Redis với TTL năm giây, và nó thất bại ngay trên chính các điều khoản của nó:
gateway sẽ biết rằng một chỉ báo đã lụi mà chẳng có cách nào nói ra. Cách sửa đầy đủ là một
field `state` và một frame `typing.stop`, thứ đó sửa một schema mà hai mươi chương client đang parse
để thêm một message mà sự mất mát của nó là không phục hồi được.

**Một lần làm mới bị rơi thì tự sửa trong vòng một chu kỳ. Một frame dừng bị rơi thì để lại một
chỉ báo hiện mãi mãi.** Sự bất đối xứng đó quyết định chuyện này, và nó cũng chính là phép thử mà chương membership-revocation
đã áp lên membership với các đầu vào đảo ngược: một frame typing bị mất thì hội tụ về sự thật, còn một
lần thu hồi bị mất thì hội tụ về một lời nói dối. Hiến pháp IV được thoả mãn ở đây một cách rỗng tuếch, và
chính sự rỗng tuếch ấy mới là phát hiện.

<Trap>
Điều khoản không đổi. Một yêu cầu là hợp đồng của khách hàng và một chương thì không
viết lại nó cho khớp với code của mình — nên ranh giới được ghi vào ADR-22 thay vào đó, đó là
một tài liệu đã xuất bản mà một khách hàng đọc được. Thứ bị cấm là tuyên bố đã đóng mà không có
lập luận.
</Trap>

## Hai giây, và vì sao không phải năm

Chu kỳ làm mới là của gateway, và nó là một đại lượng khác với cái hạn hết hiệu lực:

```ts title="services/gateway/src/session.ts (excerpt)"
export const DEFAULT_RENEWAL_INTERVAL_MS = 2_000;
```

Hai so với năm là **2.5 lần làm mới cho mỗi cửa sổ hết hạn**, nên một lần publish bị rơi không làm
một chỉ báo nhấp nháy. Ở 1.67 s thì biên mỏng hơn mà chẳng được gì; ở 4 s thì một lần mất duy nhất làm trắng
chỉ báo của một người chưa hề ngừng gõ.

<Why>
Chương presence lên lịch một phép kiểm ân hạn vào đúng khoảng ân hạn của chính nó rồi bỏ mặc một user online
mãi mãi — hai cái hạn trên một khoảnh khắc, đo bằng hai đồng hồ. Năm và hai là hai
đại lượng, và tỉ lệ giữa chúng chính là toàn bộ thiết kế.
</Why>

Trạng thái là một `Map` trong closure của `attachSessions`, khoá theo kết nối rồi theo channel. Không phải
một field trên `Connection`: kiểu đó sống trong `registry.ts`, thứ mà bốn chương fence, và một
giá trị hai giây thì không đáng bốn bản diff phải sinh lại. Nó có ba chỗ xoá — handler
đóng, một lần thu hồi, và **một mục bị xoá lúc đọc khi đã trôi qua**, thiếu nó thì giới hạn
sẽ là các channel mà một kết nối từng gõ vào chứ không phải các channel nó đang gõ vào.

Và một tín hiệu tới bên trong chu kỳ thì bị bỏ mà chẳng có frame, chẳng có mã đóng và **chẳng có dòng
log nào**. Nó là lời từ chối đầu tiên trên nền tảng này trả lời bằng chẳng gì cả: một dòng cho mỗi
lần gõ phím chính là loại output không giới hạn mà NFR-OBS-01 tồn tại để ngăn.

## Giao thức, và hai bộ đếm đã phải dịch


```diff title="packages/protocol/src/frames.ts"
@@ -98,12 +98,37 @@ export const typingSchema = z.strictObject({
   payload: z.strictObject({
     channel: z.string().min(1),
     user: z.string().min(1),
   }),
...
+
 ...
 export const errorFrameSchema = z.strictObject({
   ...
   payload: z.strictObject({
@@ -122,12 +147,13 @@ export const frameSchema = z.discriminatedUnion("type", [
...
Thêm một thành viên thứ mười một vào union thì làm hai bài test đỏ, ở hai lane khác nhau:


```diff title="services/gateway/src/isolation.itest.ts"
@@ -6,13 +6,13 @@ import { docsUrl, frameSchema } from "@relay/protocol";
...
@@ -729,6 +729,179 @@ describe("the socket refuses another tenant's identifiers", () => {
...
### Một module được dựng, được đóng, và chẳng bao giờ được truyền vào

Có đúng một cách để ship cả tính năng của chương này ở trạng thái chết mà vẫn xanh, và nó đáng được
nói ra trước phần đấu dây chứ không phải sau. `signalTyping` gọi `typing?.publish(...)`.
Nếu `typing` chẳng bao giờ tới được `attachSessions`, thì chuỗi tuỳ chọn làm cho mọi tín hiệu thành một
**no-op lặng lẽ** — và chẳng gì đỏ:

- `close()` được gọi trong đường tắt máy, nên lint thấy một biến đang được dùng;
- `/healthz` vẫn quảng cáo mười một frame, bởi cái đó đến từ gói protocol;
- đường nối vẫn chấp nhận `typing.send`, bởi cái đó là một tập trong `session.ts`;
- và **mọi bài test đều tiêm cái tuỳ chọn thẳng vào** chứ không đọc nó ra từ `main.ts`.

Trong thứ tự mà cuốn sách này được viết lần đầu thì đó chính xác là chuyện đã xảy ra. Cái dòng ấy chẳng có
chủ: cái pha dựng module hoãn phần destructuring lại để tự giữ mình ở trạng thái commit được —
`no-unused-vars` từ chối một binding mà bên tiêu thụ đầu tiên của nó là một pha sau — rồi
ghi phần đấu dây là việc của một task sau. Chẳng task sau nào nhận nó. Client đã niêm phong
tìm ra nó, mười một chương sau.

Nên phần đấu dây rơi xuống cùng module ở đây, và một bài test nói rằng mọi module được dựng đều là một module
được tiêm. Nó đọc mã nguồn, vì cái lý do mà `bound-port.test.ts` đưa ra: `main.ts` bị
loại khỏi coverage bởi nó chỉ được với tới khi chạy service, và thứ phải
đúng là một tính chất của văn bản.
...
@@ -32,13 +34,18 @@ describe("gateway skeleton", () => {
...
@@ -56,6 +63,61 @@ describe("gateway skeleton", () => {
...
<Trap>
Có đúng hai con số frame viết cứng trong repository này và chỉ một trong hai từng
có một task làm chủ. Con số của `isolation.itest.ts` nằm trong một bài test tích hợp; của `main.test.ts` là một bài test
**unit**, nên `test:integration` chẳng bao giờ chạy nó — còn các cổng của pha thì chạy `lint` và `typecheck` chứ
không chạy `test`. Để yên thì nó sẽ lần đầu xuất hiện chín pha sau, và bị quy cho bất cứ thứ gì
được chạm tới sau cùng.

Endpoint `/health` quảng cáo bộ từ vựng trên đường truyền, tính từ chính cái gói nên nó không
trôi dạt được. Sau chương này nó quảng cáo mười một frame qua HTTP cho bất kỳ ai hỏi.
</Trap>
...
+import { frameSchema, parseFrame } from "./frames.js";
 ...
 const message = {
@@ -34,12 +34,15 @@ const valid: Record<string, unknown> = {
   },
   "presence.changed": {
...
@@ -103,12 +106,63 @@ describe("malformed frames reject", () => {
...
@@ -8,13 +8,13 @@ import { serve } from "@relay/service-kit";
...
@@ -687,6 +687,29 @@ describe("the socket (chapter 2.5)", () => {
...
## Đấu dây, và phần miễn trừ phải gọi tên trường hợp của nó
...
@@ -45,25 +46,32 @@ export function createServer(logger?: Logger) {
...
Bốn module gateway giờ giữ một client Redis thô, và **cái quy tắc sẽ bắt mỗi cái trong số đó
phải biện luận cho trường hợp của mình thì chưa tồn tại.** `ioredis` bị chương rate-limit hạn chế, bảy chương
sau chương này, nên ở đây chẳng có gì để mà được miễn trừ khỏi.

Lập luận vẫn đáng được viết ra, bởi nó là cái sạch nhất trong bốn cái và là cái mà
ai đó sẽ chép. Module này PUBLISH và SUBSCRIBE lên một uuid channel và chẳng chạm khoá
nào cả — nên nó lấy lời biện minh của fan-out, *"client này chẳng chạm khoá nào"*, chứ không phải
cái mà presence và membership mỗi bên đã phải đưa ra về một id giới hạn theo environment. **Bốn
phần miễn trừ, ba lập luận**, và cái không chuyển sang được là của presence.
...
<Trap>
Hai trong số này ban đầu bị viết sai và các lần đỏ thì đáng giá hơn chính các bài test.

Bài test gia-nhập-giữa-chừng-kết-nối publish trên `member:{channel_id}` rồi chẳng giao gì — đó là
sự bất đối xứng trung tâm của chương trước, bị đâm vào từ bên ngoài. Còn bài test giữa-chừng-resume thì
await cái `connection.ack` của người theo dõi trước khi phát tín hiệu, mà trên đường resume thì cái đó tới
*sau* khi `api.backfill` trả về: nó ngủ qua luôn cửa sổ 800 ms mà nó đang cố test. **Một
lần chờ sai tín hiệu sẽ đóng chính cái cửa sổ mà nó định giữ mở.**
</Trap>

Bài test tấm-vải-bị-cắt cũng đo được một thứ mà yêu cầu không lường trước. FR-015 đòi
một sự kiện được ghi log khi thất bại; cắt kết nối thì tạo ra năm dòng `typing.failed`
với `op: "connection"` và **không** dòng nào với `op: "publish"`, bởi hàng đợi ngoại tuyến của ioredis
nhận cái lệnh rồi resolve nó khi kết nối lại. Một tấm vải bị cắt thì không đánh rơi một tín hiệu
typing — nó làm trễ tín hiệu ấy.

<Checkpoint>
Hai socket trên hai instance gateway chia sẻ một channel. Một cái gửi `typing.send`; cái kia
nhận đúng một frame `typing` gọi tên người phát tín hiệu, còn người gửi thì chẳng nhận gì.
Gửi tám cái liên tiếp thì tấm vải chỉ thấy một. Dừng lại, và chẳng gì tới cả — chỉ báo
biến mất trên chính bộ hẹn giờ của bên nhận, năm giây sau, chẳng cần server giúp gì.
</Checkpoint>

## FR-RTM-05 giờ đang ở đâu

Bốn trong sáu kiểu đã có bên tạo ra: `message.created` ở chương fan-out, `presence.changed` ở chương presence,
`membership.changed` ở chương membership-revocation và `typing` (chương này). `message.updated` và
`message.deleted` là hai kiểu vẫn chưa có, và chúng đang chờ một bề mặt chưa
tồn tại — chẳng có route sửa và chẳng có route xoá để mà thông báo, và đó là chương revisions.

<ForwardRef>
Chương connection-cap dựng cái trần năm kết nối của FR-RTM-09. **SAD** — chứ không phải SRS, thứ chẳng bao giờ
nhắc tới cái khoá đó — mô tả `conn:{env}:{user}` như một **set** Redis với một TTL, mà một
TTL thì theo khoá chứ không theo thành viên, nên một instance làm mới khoá sẽ giữ cho mục của một instance
đã chết sống mãi mãi. **Và chính dòng đó đã xuất bản sẵn một cách sửa**, một sorted set
được tỉa lúc đọc, và điều đó khiến việc đầu tiên của chương ấy hẹp hơn một quyết định: nó
phải quyết xem có chấp nhận một quyết định đã nằm sẵn trong cái cây hay không. Nó không chấp nhận, và ADR-23 là
lập luận — vì sao mỗi chỗ có một khoá riêng, và vì sao phương án còn lại cần một ngôn ngữ
mà repository này sẽ không cho vào nếu không có bằng chứng mà nó không thu thập nổi.
</ForwardRef>

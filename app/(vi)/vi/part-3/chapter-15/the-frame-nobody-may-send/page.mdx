...
export const metadata = {
  ...
  description:
    "Presence frame là server-originated: client không bao giờ được gửi nó. Chương này thêm rate limit, protocol rule, wiring và tests để phân biệt quyền publish của client và service.",
  alternates: {
...
Presence có frame mới, nhưng frame đó không phải input của client. Đây là boundary protocol:
gateway có thể publish presence, socket client chỉ được gửi grammar đã được cho phép.
...
## Plan của chương sai, và grep nói điều đó

Kế hoạch ban đầu giả định client và service cùng publish một frame presence. Grep các call site cho thấy frame thực tế chỉ có nghĩa khi server phát nó; giữ client publish path sẽ tạo một khả năng mà product không hề muốn hỗ trợ.
...
## Một shape, nơi membership cần hai shape

Presence event có thể dùng một shape chung, nhưng authorization không thể chỉ dựa vào shape đó. Client-originated command và server-originated broadcast có actor, audience và quyền khác nhau; protocol cần gọi tên direction để gateway từ chối đúng loại frame.
...
## Module publish, điều các module khác không làm

Một publish module tập trung nơi server tạo presence frame, resolve audience và áp policy rate limit. Nó tránh để từng handler tự serialize JSON, chọn subject và vô tình bỏ một guard.
...
## Seam, và cái giá của set so với comparison

Rate limit cần seam có thể test theo thời gian và atomicity. Set một timestamp đơn giản dễ race giữa gateway instance; counter/window hoặc compare-and-set cho phép policy được enforce ở shared store thay vì chỉ đúng trên một process.
...
## Hai function kề nhau với rule đối ngược

Client command bị rate-limit và bị từ chối nếu cố gửi server-only frame. Service publish được phép tạo frame đó nhưng vẫn phải giới hạn tần suất để một loop lỗi không flood mọi subscriber. Cùng payload không đồng nghĩa cùng quyền.

## Con số chương này không thể làm thành thật

Không có benchmark nhỏ nào chứng minh rate limit đủ cho production. Điều có thể nói trung thực là policy, key scope và behavior khi store unavailable; số capacity cần workload representative và được đo như một quyết định vận hành riêng.
...
## Hai giây, và tại sao không phải năm

Window ngắn giữ presence responsive nhưng chặn spam. Giá trị được chọn vì UX heartbeat và fan-out cost, không phải một hằng số thần kỳ; config và metric phải cho phép điều chỉnh khi traffic thật cung cấp evidence tốt hơn.

```ts title="services/gateway/src/session.ts (excerpt)"
export const DEFAULT_RENEWAL_INTERVAL_MS = 2_000;
```

## Protocol, và hai counter phải thay đổi

Protocol change phải cập nhật cả encoder lẫn decoder, cùng counter/metric quan sát accepted và refused frames. Nếu chỉ gateway đếm refusal mà publisher không đếm emission, ta không thể phân biệt client spam với service loop.
...
## Wiring, và exemption phải gọi tên case của nó

Wiring nên truyền explicit capability cho server publisher thay vì một cờ `skipRateLimit` chung chung. Exemption được đặt tên theo actor và reason để không trở thành lỗ hổng tiện lợi cho caller kế tiếp.
...
## Test, và hai test có thể chẳng chứng minh gì

Hai test chỉ chứng minh được rule khi chúng bait đúng key và đúng direction: một client gửi frame bị từ chối, service publish được fan out, và request vượt window nhận refusal. Test không tạo event hoặc không kiểm tra audience chỉ chứng minh harness chạy.
...
## FR-RTM-05 hiện đứng ở đâu

Sau thay đổi này, requirement được enforce ở wire boundary: client không thể giả mạo presence, service phát frame qua một owner, và rate limit có contract thấy được. Các code fence tiếp theo là phần implementation của những ranh giới đó.

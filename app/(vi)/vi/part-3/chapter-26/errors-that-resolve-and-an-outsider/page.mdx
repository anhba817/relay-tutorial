...
export const metadata = {
  ...
  description:
    "Phase 2 exit criterion nhận verdict: đạt một phần, phần thiếu được gọi tên thay vì ngầm hiểu.",
  alternates: {
...
<Figure caption="Ba level, và level đầu không phải rule: isolated node_modules của pnpm không có @relay directory ở workspace root nên import không resolve. Không level nào ngăn đọc source; đó là discipline chứ không phải mechanism." code={figThreeLevels} />

Exit criterion cần integration được xây chỉ từ published documentation. Claim dễ đưa ra,
nhưng vô nghĩa nếu thứ đưa claim vẫn đọc platform source. Vì vậy đây là package không thể làm
điều đó.
...
<Figure caption="Đạt một phần. Phần đạt được đã được đo; phần thiếu là một điều không test nào chạm được." code={figVerdict} />

> **ĐẠT MỘT PHẦN**, và phần còn thiếu không phải thứ chương này đặt ra để sửa.

**Điều đã đạt và cách kiểm tra.** Sealed package hoàn thành integration đầy đủ với platform
nó không start: tạo channel và repeat để lấy existing one, tạo private channel, thêm member,
mint token, tạo bot, từ chối send thiếu sender với field được gọi tên, gửi REST rồi đọc history,
nhận event trên socket, gửi attachment, typing, giữ năm connection rồi bị từ chối connection
thứ sáu bằng 4004, từ chối server-only frame, và so foreign resource với absent resource.
Mười sáu test đều pass trong CI job riêng.

Gap nổi tiếng—REST message không tới live socket—đã được reorder khép lại: sender chapter
thêm attribution để row sống qua resume, message-delivery chapter thêm API fan-out. Test giờ
gửi REST một cách có chủ ý, đúng send mà backend của integrating developer thực hiện.

**Điều chưa đạt.** Content sufficiency không phải comprehensibility. Suite đo docs có chứa
thứ integration cần; chỉ con người mới đo được người đọc không trợ giúp có thể xây integration.
Tương tự, dependency seal là mechanism còn không đọc source là discipline.

<Why title="Sáu gap, cùng disposition">

Gap được ghi trong lúc viết suite: REST-to-socket delivery đã được schedule rồi khép; compose
last-built image đã fix; compose port drift được ghi scope; credential được cung cấp bằng script;
undocumented dev-token được schedule; WebSocket của Node không phải defect mà cải thiện suite.

</Why>

<Checkpoint>

- 13 code, 13 reference section, kiểm tra hai chiều
- `docs_url` được fetch từ served site
- `pnpm test:outsider` chạy sealed suite với compose services profile
- ba seal được chứng minh fail từng cái một
- Phase 2 exit criterion: **đạt một phần**, phần thiếu được gọi tên

</Checkpoint>

<ForwardRef part="Part 3 khép lại, Part 4 mở ra">

Part 3 kết thúc ở đây. REST-to-socket publish và customer public surface đã hoàn thành trước
milestone này; khoản còn lại—outbox text retention và debt của `docs_url`—được ghi rõ cho
Part 4 thay vì ngầm chuyển đi.

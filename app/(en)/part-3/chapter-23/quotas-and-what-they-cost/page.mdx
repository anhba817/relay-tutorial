...
<Trap title="A key's send counts toward the messages — and, since the sender chapter, toward an active user">
When this chapter was written, a key-authenticated REST send carried no user —
unattributed by design since the outbox chapter, because a server-side API key is not a
person. It cost a message against the message quota and counted toward nobody.

**The sender chapter changed the second half.** A key now names a **bot user**: an identity
...
One consequence, since the sender chapter draws the line here: a bot is *billed* as an active

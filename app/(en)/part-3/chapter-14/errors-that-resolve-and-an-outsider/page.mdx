...
export const metadata = {
  title: "Milestone: errors that resolve, and an outsider — Building Relay",
  description:
    "The error envelope has carried a docs_url since chapter 1.3 and it has never resolved. Five of the thirteen codes the platform sends were not in the registry at all. And the Phase 2 exit criterion gets a verdict: met in part, with the part that is missing named rather than implied.",
  alternates: {
    canonical: "/part-3/chapter-14/errors-that-resolve-and-an-outsider",
    languages: {
...
Every error this platform sends carries four fields, and one of them has been a lie
since chapter 1.3.
...
This chapter is the one that cannot ship with it, because its other half is the SRS
Phase 2 exit criterion: *an external developer integrates using only public
documentation, with no assistance.* A developer who follows a link from an error
response to a 404 has been given no documentation at all.

## Counting the vocabulary

<Figure caption="Five of the thirteen codes the platform can emit were never in the registry. `ProtocolErrorFilter` maps a status to a code when the thrower names none, and docs_url is derived from the code — so each of those five shipped a link to a page that could not exist even in principle." chart={figThirteenCodes} />

The registry called itself the documented vocabulary while documenting eight of
thirteen:
...
<Why title="Why the code is the anchor, verbatim">

`docsUrl` returns the base plus `#` plus the code with no transform: no slug, no
case change, no separator swap. The reference's `h2` headings **are** the codes.

The alternative is a slug function in `@relay/protocol` matching `slugifyHeading` in
the tutorial site — the same transform maintained in two repositories, with no test
able to see both sides. Instead the site's function keeps `_` in its character
class, so `## quota_exceeded` anchors at `#quota_exceeded`, and the blast radius of
that change was measured before making it: zero chapter headings contain an
underscore, one docs heading does, and zero links anywhere point at a
`/docs/<slug>#anchor`.
...
<Figure caption="`HttpException`'s response is `unknown`, so eight call sites named their own error code as a bare string. A misspelling compiled, shipped, and became a URL." chart={figFourTypeGates} />
...
Narrowing `sendError`'s parameter from `string` to `ErrorCode` was the one change
...
<Why title="Why the field is named in the pipe and carried by the filter">

Only the pipe knows the path. Only the filter builds the envelope. So the field
travels the way `code` already travels — the thrower names it, the filter carries
it — and a key that is always present and usually empty teaches a client to ignore
it, so it is omitted when there is nothing to say.
...
<Why title="Why required, and why the dependency inverts">

Importing `@relay/protocol` here would end the property. So the caller supplies the
URL as data, and because the field is **required** the compiler makes it do so:
`serve()` has exactly one caller in production and cannot be given a stale
placeholder by accident.

Optional with a default host would have been the fourth instance of this chapter's
own subject. `rate_limited`, close code 4008 and `request_id` were all declared and
left unenforced, and a placeholder with a default has a longer life than one
without.

</Why>

The compiler named every call site — one in production and eight in tests:
...
The platform half of that check lives with the registry, because it can be
self-contained — no file outside the workspace, so no turbo cache hole and no
dependency on the parent repository:
...
<Trap title="The check runs as a CI step, not as a turbo task">

Two measured reasons, and both are about the gate being real.
...
`relay-platform` is independently clonable with a README promising its own checks
pass from a clean checkout, where `../docs` does not exist.

So it is a step in the platform's CI job, which already has the built protocol
package the script reads. The script uses only node builtins, so it needs nothing
from the tutorial's dependency tree — only its path.
...
<Figure caption="Three levels, and the first is not a rule: pnpm's isolated node_modules has no @relay directory at the workspace root, so the import does not resolve. What none of the three closes is reading the source, which is a discipline rather than a mechanism." chart={figThreeLevels} />
...
<Figure caption="Met in part. What is met was measured; what is not met is two things of different kinds, and only one of them is a defect anyone can fix." chart={figVerdict} />
...
REST-sent message reaches no socket and FR-RTM-05's chapter owns the choice; and the

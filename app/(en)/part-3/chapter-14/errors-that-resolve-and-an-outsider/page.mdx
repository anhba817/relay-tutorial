...
<Figure caption="Five of the thirteen codes the platform can emit were never in the registry. `ProtocolErrorFilter` maps a status to a code when the thrower names none, and docs_url is derived from the code — so each of those five shipped a link to a page that could not exist even in principle." code={figThirteenCodes} />
...
<Figure caption="`HttpException`'s response is `unknown`, so eight call sites named their own error code as a bare string. A misspelling compiled, shipped, and became a URL." code={figFourTypeGates} />

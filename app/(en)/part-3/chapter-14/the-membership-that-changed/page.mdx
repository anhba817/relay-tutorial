...
export const metadata = {
  ...
  description:
    "A clause has been unmet since chapter 2.6 and a test has asserted the violation since the fan-out chapter. Membership gets a third subject grammar, the first event addressed to a principal rather than a channel, and a periodic re-read standing in for a cursor that does not exist.",
  alternates: {
...
The fan-out chapter wrote this and left it green:
...
that could carry such a message and had no reason to send one. The channel-control chapter built the removal
route and had no way to tell anybody. The fan-out chapter gave the gap a second entrance — a REST send
...
`chan:{channel_id}` carries messages. The presence chapter added `presence:{channel_id}`. Both assume
...
The fan-out chapter paid for that collision once — two functions, one name, one import list — so this
...
Constitution II forbids publishing after a commit without an outbox row. The fan-out chapter's Redis
...
The fan-out chapter's publisher opens a down-window: after a failure it stops trying for five
...
The route it reads has been described by the protocol package since the credentials chapter and served by
nothing since:
...
exactly four of them fail — the four that are about the backstop:
    ...
    × corrects a removal the fabric never delivered, within one interval
    × changes nothing when nothing changed, and is asserted by request count
    × converges a pair that arrived in the wrong order
    × answers the route, writes the row and logs once with Redis unreachable

**Run rather than asserted**, and the four names are the reason it is worth running: a claim
that "four fail" is satisfied by any four, and the four that should fail are the four whose
subject is the re-read. The other forty-one pass with the watch gone because the fabric still
delivers — which is the point of having a backstop rather than only a fabric.
</Checkpoint>
...
**FR-RTM-09's five-connection cap is the connection-cap chapter's, and its specified shape does not
work:**
`conn:{env}:{user}` is a Redis *set* with one TTL, and a TTL is per key rather than per member —
so one instance refreshing the key keeps a dead instance's entry alive for ever. The SAD
prescribes a sorted set scored by heartbeat and pruned on read. **The connection-cap chapter builds the cap
and argues that prescription down** (ADR-23): pruning on read leaves the claim non-atomic
without Lua, and Constitution VII admits a second language only on profiling evidence a
five-channel fixture cannot produce — so each place becomes a key of its own and the TTL is
per member by construction. That is the presence chapter's `gaps.md` item 6, and it is a design
change rather than an implementation.

<ForwardRef>
The typing indicator is the next of FR-RTM-05's kinds to get a producer. **What the typing chapter

...
FR-RTM-05 names six real-time event kinds. The fan-out chapter gave `message.created` a producer,
The presence chapter gave presence one, the membership-revocation chapter gave membership one. `typing` is the fourth, and the plan for
...
there the whole time. ADR-19 counted three when it was written, the membership-revocation chapter quoted the three
...
The membership-revocation chapter needed a second subject because an addition cannot ride the channel it adds you
...
The membership-revocation chapter's finding forward — that module was *"written with two by analogy and the second was
...
directly below it, skips nobody — a user sees their own presence transition, and the membership-revocation chapter
...
indicator showing for ever.** That asymmetry decides it, and it is the same test the membership-revocation chapter
...
<Why>
The presence chapter armed a grace check at exactly its own grace period and stranded a user online
...
### A module built, closed, and never passed in

There is one way to ship this chapter's whole feature dead and green, and it is worth
stating before the wiring rather than after. `signalTyping` calls `typing?.publish(...)`.
If `typing` never reaches `attachSessions`, the optional chain makes every signal a
**silent no-op** — and nothing fails:

- `close()` is called in the shutdown path, so lint sees a variable that is used;
- `/healthz` still advertises eleven frames, because that comes from the protocol;
- the seam still accepts `typing.send`, because that is a set in `session.ts`;
- and **every test injects the option directly** rather than reading it from `main.ts`.

In the order this book was first written that is exactly what happened. The line had no
owner: the phase that built the module deferred the destructuring to keep itself
committable — `no-unused-vars` rejects a binding whose first consumer is a later phase —
and recorded the wiring as a later task's job. No later task had it. The sealed client
found it, eleven chapters away.

So the wiring lands with the module here, and a test says every module built is a module
injected. It reads the source, for the reason `bound-port.test.ts` gives: `main.ts` is
excluded from coverage because it is reached by running the service, and what has to be
true is a property of the text.
...
Four gateway modules now hold a raw Redis client, and **the rule that will make each of them
argue its case does not exist yet.** `ioredis` is restricted by the rate-limit chapter, seven
after this one, so there is nothing here to be exempt from.

The argument is worth writing down anyway, because it is the cleanest of the four and the one
somebody will copy. This module PUBLISHes and SUBSCRIBEs onto a channel uuid and touches no key
of any kind — so it takes the fan-out's justification, *"this client touches no keys"*, and not
the one presence and membership each had to make about an environment-scoped id. **Four
exemptions, three arguments**, and the one that does not transfer is presence's.
...
Four of six kinds have producers: `message.created` the fan-out chapter, `presence.changed` the presence chapter,
`membership.changed` the membership-revocation chapter and `typing` (this chapter). `message.updated` and
`message.deleted` are the two still without one, and they are waiting on a surface that does
not exist — there is no edit route and no delete route to announce, which is the revisions chapter.

<ForwardRef>
The connection-cap chapter builds FR-RTM-09's five-connection cap. **The SAD** — not the SRS, which never

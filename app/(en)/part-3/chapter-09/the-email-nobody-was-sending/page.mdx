...
passed — FR-021 forbids a secret in an email and only the received message can
...
justification, and it is FR-021:
...
## The message, and the seam that makes FR-021 true
...
never given, which makes FR-021 a property of the type rather than of a filter
...
+      # service. And the tenant limiter FAILS OPEN by design (FR-010), so a
...
- FR-021 is asserted against what a server **received**, not what the sender
  passed.

...
over the course of writing chapter 3.3, and **not one of them has ever been
read**. Three of those settings were chosen by a person. The rest are whatever
NATS does when you do not say.

This chapter fixes both halves of that. Every setting becomes a decision with a
reason attached, and the stream gets its first reader — which turns out to
reintroduce, one hop further along, exactly the failure 3.3 spent a chapter
removing.

<SkipAhead>

If you must skip: the `EVENTS` stream is configured deliberately — seven-day
`max_age` over NFR-REL-08's 24-hour floor, a 1 GiB `max_bytes` with `discard:
old`, replicas derived from the environment rather than hardcoded — and applying
that configuration is idempotent, because two api instances start together.
The subject grammar `events.{domain}.{action}.{env}` moves from the api's outbox
module into `@relay/protocol`, where both the producing and the consuming side
can share it. A `consumed_events` table keyed `(consumer, event_id)` is the
deduplication ledger; the claim on it and the handler's effect commit in one
transaction, so a handler that throws rolls its claim back with it. A durable
pull consumer fetches in batches of 25, acknowledges only after the claim
commits, retries a throwing handler up to five times, and terminates an
unparseable payload immediately. The first handler is a **scaffold** that logs
and is retired by 3.5. Delivery is at-least-once; handling is once *in effect*,
and the ledger is what makes the difference. The chapter-end state is tag
**`part3-ch4`**.
...
casually. So we asked the broker rather than the documentation:
...
The numbers each carry their reason in the code, which is where a reader will be
...
is FR-ANL-06's 0.1% reconciliation failing silently rather than loudly.

Under `limits`, the stream holds messages by age and size, and each durable
consumer keeps its own position. Everyone reads everything. That is also what
makes invariant 9 possible — a consumer stopped for N publishes receives all N
when it comes back — and what left twelve thousand events sitting there for a
...
The api keeps working because `event.ts` re-exports what it used to define — and
...
<Figure
  caption="The consumer's gap. Between doing the work and acknowledging it there is a window in which the effect is durable and the broker does not know — and a process that dies there is redelivered work that has already happened."
  code={figTheOtherGap}
/>
...
So let us make it happen rather than describe it. The walk script does by hand
what the runtime does in a loop, and dies in the gap:
...
Second attempt, explicitly redelivered, and the ledger refuses the claim — so
the effect stays at one and the message is finally acknowledged. Without that
...
earlier chapters had left in the stream, all of which had to be delivered first.
That is not a bug in `limits` retention; that is `limits` retention. The fix was
to give the runtime a `filterSubject`, which the contract had already anticipated
for 3.5's dispatcher, and scope each test to its own environment's subject.
...
that exists precisely to retry the failure, gets waved through as a duplicate.
...
It cannot acknowledge. It cannot negatively acknowledge, retry, deduplicate, or
see the raw message. It can return, or it can throw. Everything else is the
runtime's, and the runtime is where the decision table lives:

<Figure
  caption="What the runtime does with a delivery. Three outcomes, one of which — terminate — is the honest end of the road for a message nobody can parse."
  code={figOutcomes}
/>
...
around it, not the code inside it — and every consumer the SAD names belongs to
a later chapter. Giving this one a job would mean either stealing 3.5's subject
or inventing product nobody asked for.
...
FR-WHK-04 asks for a dead-letter queue with replay. That is chapter 3.5's, and
it is genuinely 3.5's rather than deferred to avoid work: dead-lettering needs a
place to put a message, a way to look at it, and a way to replay it, and all
three of those are shaped by what the webhook dispatcher needs. Building a
generic one here would be building it twice.
...
<Figure
  caption="The consumer lives inside the api service — not for convenience, but because its ledger is a Postgres write, and ADR-04 allows exactly one service to make those."
  code={figWhereItRuns}
/>
...
Starting it is two lines next to the relay's, for the same reasons — lazy
connection, so an unreachable broker leaves the api serving writes:
...
reads the broker rather than the config file, because a configuration that was
...
does it merely pass? Removing the ledger claim from the runtime — leaving
everything else exactly as it is — fails three of the ten:

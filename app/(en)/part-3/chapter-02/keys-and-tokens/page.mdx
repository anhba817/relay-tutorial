...
The tenancy chapter built the containers a customer's world lives in and the signup
...
The tenancy chapter drew a line through the data: `humans` above the tenant boundary
...
key. There is no console session — the tenancy chapter deliberately did not build one — so
...
The walk from the tenancy chapter now shows what it hands over, and — more usefully — what it
...
This is the whole reason a dependency appears in this chapter after the tenancy chapter went
...
Retired by the credentials chapter. It resolved a tenant from a header any caller could
...
The retry-and-disable chapter's subject, and building half a limiter here would mean building it
twice.
...
*Narrowed by the connection-metering chapter.* The gateway acquired a credential of its own —
`RELAY_INTERNAL_CREDENTIAL_GATEWAY`, a platform credential of the class chapter
The webhook dispatcher chapter built for the dispatcher — because a usage report is nobody's user action and
there was no token to forward. So one of the gateway's four api calls is now
authenticated as a service rather than trusted as a neighbour. The other three
still forward the end user's token, and the network between them is still
trusted.

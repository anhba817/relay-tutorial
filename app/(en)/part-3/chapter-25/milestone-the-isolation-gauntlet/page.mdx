...
a repository SELECT on the history read path. Run the gauntlet. **Seventy-one of
seventy-one passed.**

<Figure caption="The first reintroduction changed nothing observable. An outer predicate is scoped and runs first, so the unscoped query below it was never reached — the suite is sensitive to the outermost check on a path and blind to an inner one a live outer check masks." code={figWhatMaskedWhat} />

That is not a defect in the reintroduction and it was not reshaped until it failed.
It is a fact about the suite's range: `GET /v1/channels/:channelId/messages` refuses
a foreign id before `listMessages` is reached at all.

**And the second attempt found nothing either, which is the sentence worth having.**
The obvious next move is to unscope the predicate above it — `channelExists`, the
one chapter 2.8 added when the two doors disagreed. Dropping its scope too: **still
seventy-one of seventy-one.** `channelExists` is no longer on this path. The
channel-control chapter replaced existence with VISIBILITY, because an absent
channel answered 404 while a private channel a non-member read answered 200 with an
empty page, and one predicate now produces both refusals.

So the mask is `channelVisibleTo`, and it is scoped. Unscope THAT and five
assertions fire at once:
...
**Five, not one.** `channelVisibleTo` sits under four routes — the message read, the
edit, the deletion and the edit history — so one unscoped SELECT is four leaks. The
fifth is the credential attack, which mints a token in the attacker's environment and
then presents it against the victim's channel: its ORACLE is a message read, so it
inherits the leak from the route it probes with.

That last one is worth noticing. A suite of independent attacks is not independent
where two of them share a target, and the count of red assertions is a count of
assertions rather than of faults — **there is one fault here, reported five ways.**
...
Two, because both routes are one method: `setEnabled(id, enabled)` serves the enable
and the disable, so a fault planted in it is visible from either door.
...
**AND THE SECOND ATTEMPT IS A PROOF OF SOMETHING ELSE.** Unscoping `channelExists`
was the correction — right about the shape of the fault, wrong about the function,
and green either way. The predicate on that path had been replaced by a later
chapter, so a reintroduction aimed at the old one measures a function nothing calls.
**Which check is outermost is itself a thing that changes**, and a probe that names
a function by memory is a probe that can pass without testing anything.
...
The instruments above are tests. This one is a linter, and it had the failure mode
this chapter is about: **a rule that looks enforced and is not.**

`no-restricted-imports` forbids importing the driver, the query engine and the
counter store outside the layers that own them — constitution I as a build error
rather than a convention. The global-drain restriction wants the same treatment: six
repository functions that claim or count rows across every environment, which an
integration test must not reach for.

Both are the same rule. **In flat config a later block REPLACES an earlier block's
setting for a rule rather than merging into it**, and the second block has to match
`**/*.itest.ts` — every one of which the first block already matched as `**/*.ts`.
So adding the drain restriction the obvious way switches the driver ban off for
every integration test in the workspace, and nothing says so: the config parses, the
lint passes, and the ban is gone.

Measured before it was written, on a file that is on no exemption list:
...
The second run is the whole finding. The engine import is still there, still
forbidden by a rule the config still contains, and the only complaint left is that
the variable is unused.

The four sets are named and hoisted so the blocks can compose them, and each block
sets the rule to the union it needs rather than to half of it:
...
Six probes, because a composed rule has six directions to get wrong and a config
that passes its own tree proves none of them:

| the file | the import | expected |
|---|---|---|
| a plain `.itest.ts` | `drizzle-orm` | banned |
| a drain-exempt suite | `drizzle-orm` | banned |
| a driver-exempt suite | `drainOutbox` | banned |
| a plain `.itest.ts` | `outboxDepth` | banned |
| a drain-exempt suite | its own drain | allowed |
| the tree as it stands | — | clean |

**And the exemption list arrived stale.** Six suites were excused from the drain
rule; `drain-exempt.test.ts` reads the restricted names out of the rule itself and
asserts both directions against the tree, and it found that three of the six import
nothing restricted here — two name a drain only in prose explaining why they call
one instead, and the third names it as a property on a stub it builds. Three
standing exemptions over nothing, on the list's first day.

That is the shape the rest of this chapter is about, arriving through the instrument
rather than through the platform: **a list copied from somewhere it was true is a
list nobody has checked here.**
...
The scattered ones are still there, and one of them is sharper than it was — though not
because of anything this chapter does. `repository.itest.ts`'s membership assertion used to
read `false` for three different refusals: the channel is not yours, the user is not yours,
and you asked twice. Conflating the first two is the property; conflating the third with
them is a bug waiting for an endpoint — and the endpoints chapter answered it seventeen
chapters back, which is why the assertion arrives here already reading three words instead
of one boolean.

The endpoint that needed the distinction is the endpoints chapter's, and it is
already built — that chapter runs eight movements ahead of this one now. The
isolation half of it belongs here, because `not_found` covering both foreign cases
is the property FR-TEN-05 asks for and it is now written down as a word rather than
as a boolean somebody has to interpret.

A defence trusted past its range is worse than none, and this is the difference
between this suite and the scattered assertions it joins. **Twenty-five of those
predate it** — counted by title across every integration suite outside
`src/isolation/`, eleven when this chapter was first written and twenty-five by the
time the reorder put eleven more chapters in front of it. They are all still there
and still passing: the gauntlet adds to the isolation surface rather than relocating
it off the code the coverage run measures.

<Checkpoint>

- `pnpm test:integration` — 888 tests, ten tasks, green
- `gauntlet targets: 42 derived, 36 attacked, 6 exempt`
  (read 5, write 26, credential 3, list 2, exempt 6)
- `tenant paths: 24 tables — 13 direct, 3 hop, 8 spine`
- the gauntlet: 56 tests, including three that prove the attacker works
- the socket gauntlet: 31 tests, four verbs and ten frames
- three reintroductions: two red on the first attempt, the first red only on the
  THIRD, all of them reverted against a committed tree

</Checkpoint>

<ForwardRef part="the outsider milestone">

**MOST OF WHAT THIS BOX USED TO POINT AT IS BEHIND YOU.** It named two chapters
ahead: the endpoints this suite's public-surface pass found, the instruments that
verify the verifiers, and the error vocabulary. The reorder put all three earlier —
`POST /v1/channels` and `POST /v1/channels/:channelId/members` are on the router
already, which is why the gauntlet can attack them here, and the error registry is
the chapter that made `docs_url` a promise this platform has yet to keep.

What is still ahead is the sealed outsider and the Phase 2 exit criterion's verdict:
a package that integrates with this platform without importing a line of it, which
is the only reader of the contract that has no access to the answers.

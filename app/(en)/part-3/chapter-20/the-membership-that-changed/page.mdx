...
@@ -179,8 +191,50 @@ export class ChannelsController {
   async removeMembers(
...
@@ -208,7 +262,19 @@ export class ChannelsController {
     }
...
@@ -221,7 +287,14 @@ export class ChannelsController {
   async addMembers(
     @Param("channelId") channelId: string,
...
the routes are not built, so there is nothing to announce. `typing` is different: it is the one
kind that could genuinely reuse `chan:{channel_id}` rather than needing a fourth grammar,
...
<ForwardRef>
The typing indicator is the next of FR-RTM-05's kinds to get a producer, and the first that can
reuse a grammar rather than adding one.
</ForwardRef>

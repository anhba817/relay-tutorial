...
export const metadata = {
  title: "When to stop trying — Building Relay",
  description:
    "The webhook dispatcher chapter built a platform that gives up on a delivery and never on an endpoint. This chapter adds the record of what happened and the decision to stop, in that order — and the order is the argument.",
  alternates: {
...
Here is what the webhook dispatcher chapter's platform does about it. Each event produces a
delivery, each delivery is attempted seven times over two and a half hours, each
attempt fails, each one is dead-lettered. Then the next event arrives and the
whole thing happens again. The customer is not told. The endpoint stays enabled.
Nobody at Relay knows, because nothing anywhere counts.

That platform gives up on a *delivery*. It never gives up on an *endpoint*.

This chapter fixes that, and it does two things in a particular order: it builds
the record of what happened to every attempt, and then it builds the decision to
stop attempting. The order is not incidental. Switching off a paying customer's
endpoint is a thing somebody has to explain afterwards — to the customer, to
support, possibly to a lawyer — and a platform that takes that action without a
per-attempt record cannot explain it. That is why FR-WHK-07 was deferred out of
The webhook dispatcher chapter rather than shipped: the decision was ready and the evidence was not.

<SkipAhead>

If you must skip: every delivery attempt is published as an analytical event on
a new `ANALYTICS` JetStream stream, subject
`analytics.webhook.attempt.{environment_id}`, carrying identifiers, status,
latency and outcome — never a payload, a secret or a signature. The publish
happens **after** the outcome transaction commits and is **allowed to be lost**,
because constitution III says a backlogged analytics path must not affect
webhook dispatch. Nothing consumes that stream yet; Part 4's ingester is what
makes attempts queryable. Auto-disable reads two operational columns on
`webhook_endpoints` — `failure_run_started_at` and `failure_run_attempts` —
never the stream. An endpoint is disabled when its unbroken failure run has
lasted more than an hour **and** contains at least five attempts, and the check
fires from **two** places: on a recorded outcome, and from a sweep riding the
delivery relay's existing loop. The second trigger exists because measurement
showed the first never fires for a quiet endpoint. Disablement writes one
`webhook_disable_notifications` row whose `delivered_at` is null and stays null,
because this platform has no email. A synthetic test event closes the
disable-repair-re-enable loop. The chapter-end state is tag **`part3-ch6`**.

</SkipAhead>

## The record has to come first

There is a version of this chapter that is half the length. It adds two columns,
counts failures, switches the endpoint off after an hour, and stops. Everything
FR-WHK-07 asks for, nothing more.
...
The platform already writes a log line per attempt. Logs are the wrong shape for
this: they are retained by whatever the operator's aggregator retains, they are
not queryable per tenant, and they are not something a customer can ever be
shown. The SAD calls a per-attempt record an **analytical event** and routes it
to the analytical store, which is a place with a retention policy, a tenant
column and a query surface.

That is also the constitutional answer. Principle III keeps analytical volume out
of the operational database, and one row per webhook attempt is analytical volume
by any measure — a busy environment produces more attempt records in a day than
it has messages.
...
The webhook dispatcher chapter defined the internal contract the dispatcher uses to report what
happened when it posted. Look at what it already sends:
...
dispatcher has measured it since the webhook dispatcher chapter shipped. And neither `dispatch.controller.ts`
nor `recordAttemptOutcome` mentions the field anywhere — it is validated by the
schema and then dropped on the floor.

So FR-WHK-06's hardest-sounding column, *how long did the customer take to answer*,
costs nothing to obtain. The data has been arriving for a chapter; this is the
first thing that wants it.

That is worth a moment, because it is a pattern rather than a coincidence. A
contract designed around what the sender knows tends to carry more than its first
consumer needs, and the surplus is free the day something else turns up. The
alternative — widening the seam now, redeploying two services to add one integer
— is the cost the webhook dispatcher chapter avoided by writing down everything the dispatcher had.
...
extends the broker chapter's rather than inventing a second convention, and it lives in
`@relay/protocol` beside the other two for the reason the broker chapter moved the first one
...
`analyticsSubjectFor` refuses an environment id that is not a UUID, and that
check is tenant isolation rather than input tidiness.

NATS subjects are dot-delimited, and `*` and `>` are wildcards. An environment id
containing a dot would silently publish to a deeper subject than intended; one
containing `>` would turn a destination into a subscription. Neither fails at
publish time. Both would put one tenant's attempt records somewhere another
tenant's filter can reach them, which is constitution I's Sev-0 class of bug
arriving through a string-formatting bug.

Every caller in this platform holds a UUID already, so refusing anything else
costs nothing and closes the shape entirely.

</Trap>

The tests hold the grammar, including the three dangerous values:
**The file as it stands**, and then the three dangerous values this chapter adds to it.
...
`ensure` function to the publisher the outbox chapter built. That parameter already
...
## The publish is allowed to fail, and the chapter has to say so
...
Those two cannot both be maximised, and the conflict is not a technicality. To
record every attempt without loss, the record has to share a transaction with the
outcome — the outbox pattern the outbox chapter built, which exists precisely to make
"the state changed" and "the event was emitted" one atomic fact. Do that here and
a stalled analytics consumer backs up an operational table: the outbox fills, the
relay falls behind, and a metering pipeline nobody is watching starts holding row
locks on the table that decides whether a customer's webhook goes out.

Guarantee independence instead and the publish happens outside the transaction,
where a crash between commit and publish loses the record.

**Independence wins**, and the reason is that the two failure modes are not
comparable. A lost attempt record is a gap in a dashboard. A blocked outcome
transaction is a customer's webhooks stopping because a metering pipeline is
unwell — which constitution III names as a design failure in as many words.

So: **FR-WHK-06's "every" is approximate**, and this paragraph is where the
chapter says so rather than a footnote three sections later. Attempt records are
best-effort. A dashboard built on them will occasionally be short one attempt,
and the trade that buys is a delivery path that cannot be stopped by anything
downstream of it.

<Figure caption="The attempt record leaves after the commit, and can be dropped without anyone waiting." code={figWhereTheRecordGoes} />

Here is the whole publisher. It is a small file and its entire subject is one
decision:
...
**The record is built by naming every field, not by spreading the input.** A
spread would carry whatever a future caller happened to attach — a payload, a
decrypted secret, a header — onto a stream with seven-day retention that nothing
in this platform currently reads. An allow-list fails closed when somebody adds a
field. A spread fails open, silently, and the failure is a customer's message
text sitting in an analytics store that was promised it would never hold any.

**The deduplication key is `{delivery}:{attempt}`**, and that is the webhook dispatcher chapter's
bug arriving in new code. The delivery id alone is stable across all seven
attempts; the webhook dispatcher chapter used it as the broker's `msgID` for the delivery publish, and
...
`publishAttempt` never throws. Not "rarely throws" — the `catch` has no rethrow
in any branch, and that is the mechanism rather than defensive habit.

The caller has already committed an outcome and the dispatcher is waiting for an
answer. If this threw, an analytics outage would become a `500` on the dispatch
seam; the dispatcher would not acknowledge; the broker would redeliver; and the
customer would be posted a **second copy of a webhook they already received** —
caused, end to end, by a metering pipeline being unwell.

The swallow lives inside the publisher rather than at the call site so that no
future caller can create that path by forgetting a `try`.

</Trap>

The unit tests hold both properties, and the interesting assertion is the one
about extra fields:
...
**After the transaction**, so no row lock is held while a broker is consulted.
**Only when something was recorded**, which needs a word of its own.
...
## Now the decision — and a measurement that changed it
...
`recordAttemptOutcome`, in the same transaction, exactly the way the broker chapter put
...
So measure it. The webhook dispatcher chapter's retry schedule is
...
<Figure caption="Six attempts inside the hour, then nothing for two hours. The rule fires in the gap." code={figTheTimeline} />
...
**Nothing happens at one hour.** A check that runs only when an outcome is
recorded cannot fire at the threshold, because no outcome is recorded anywhere
near it. For a single failing delivery the next event after 35m36s is at
2h35m36s, so the endpoint is disabled ninety-five minutes late.

And then the part that turns a latency problem into a correctness problem: after
attempt seven the delivery is dead. If no further events arrive for that
environment, **no outcome is ever recorded again and the endpoint is never
disabled at all.** It sits enabled and failing forever — which is the exact state
FR-WHK-07 exists to end.

A busy environment hides this completely. New events keep arriving, attempts keep
happening, the check fires close enough to the hour that nobody notices. The
endpoint that stays broken silently is the **quiet** one: the low-traffic
customer, who is also the customer least likely to be watching.

<Why title="Why this is in the chapter at all">

The measurement above is arithmetic against a table that was already written
down. It took a few minutes and it disqualified the design that everything else
about this chapter would have been built on.

It is here because the failure it prevents is invisible in exactly the conditions
you would test under. Seed some events, watch the endpoint get disabled, ship it.
The bug only appears for customers whose traffic is thin enough that the schedule
runs out — and those customers do not file bug reports about webhooks they have
stopped expecting.

</Why>

**Two triggers, then.** On a recorded outcome, which catches every endpoint still
receiving attempts. And a sweep, which catches the rest.

<Figure caption="Both triggers end in the same statement, and the statement is what makes them idempotent." code={figTwoTriggers} />

## The failure run is two columns

The state auto-disable reads is not the attempt stream. It is two columns on the
endpoint, and keeping them separate is what makes a disablement independent of a
broker's health.
...
**Two columns rather than a table**, because a run is one row per endpoint by
definition: it is the *current* run, and there is only ever one. A table would
need a rule for which row is current, and that rule is the bug.

**Both null when healthy.** Any delivered outcome clears them, which is what
makes the policy generous to a flaky customer — an endpoint that succeeds once an
hour is never disabled. That is a deliberate choice about which failure is worse.
A platform that switches off endpoints which sometimes work is worse than one
that keeps trying at endpoints which never do.
...
The migration is hand-reviewed, as ADR-16 requires, and the review is written
into the file rather than performed and forgotten.

**It is 0011 and generation offered 0007** — the third chapter running to say so, and the
reason has not changed: `drizzle-kit` numbers from its own snapshot count, the
hand-written migrations after 0005 got no snapshots, and `migrate.ts` reads the directory
and sorts by filename. What the repetition is worth is the shape of the drift rather than
the arithmetic: every generated file this book adds from here on will offer a number four
lower than the one it must take, until somebody either regenerates the snapshots or
retires the journal the runner already ignores.
...
Read the first bullet of that review disposition again, because it is the one
statement in this migration that could have caused an incident.

`failure_run_started_at` is nullable **with no default**. Give it
`DEFAULT now()` — which is the reflex, since every other timestamp column in this
schema has one — and every existing endpoint in the platform acquires an open
failure run at the moment the migration applies. An hour later the sweep finds
all of them past the threshold, and the platform disables every webhook endpoint
it has, writes a notification for each, and stops delivering to customers who
never had a failure.

Nullable-and-null means every existing endpoint is healthy the moment this
applies, which is true. The `CHECK` constraints underneath do the same job in the
other direction: they make it impossible to write half a run.

</Trap>

## The policy, with nothing else in it
...
The separation is not tidiness. This is the only part of the chapter with a
number in it, and a pure function is the only version of it that can be tested
exhaustively in milliseconds. Both triggers call it; neither can disagree with the
other about what an hour means.

**Why five attempts?** Because the hour alone is not enough. The schedule reaches
a two-hour gap, so a single failure followed by silence satisfies "failing for
more than an hour" with one data point, and disabling a customer's endpoint on
one bad response is exactly what a floor is for.

Five is the largest floor a single failing delivery can still clear inside the
window — attempt five lands at +5m36s and attempt six at +35m36s, both inside the
hour, so there is a margin of one. Set it to seven and a single-delivery run could
*never* trigger, because the seventh attempt falls outside the window: the floor
would become the reason quiet endpoints stay enabled, which is the bug the sweep
exists to fix, reintroduced through the front door. Below five it stops doing its
job, since three failures arrive within six seconds and six seconds of trouble is
a blip.

The tests re-derive that from the real tier table rather than restating it, so a
later chapter that changes the schedule finds out that it has changed the floor:
...
Both triggers end in the same statement, and the statement is what enforces
FR-WHK-07 rather than a check somebody has to remember:
...
**The organisation is resolved at write time**, through the two joins that already
exist, and stored on the notification. Storing it looks redundant. It is stored
because the row records an obligation *as it stood* when the endpoint was
disabled, and an application moving between organisations later must not silently
retarget a notification that was already owed to somebody else.

**The sweep reads the last status off the endpoint's most recent delivery.** It
has no outcome in hand — it fires precisely when no outcome is arriving — so
without that lateral join the only reason it could write is "cause unknown", which
is the notification a support engineer actually receives.

**A test event's outcome is exempt from all of it**, which is the next section but
one.

<Trap title="What the row lock is actually for">

The comment on `applyFailureRun` used to say that `SELECT … FOR UPDATE` was the
whole of FR-WHK-07's concurrency story: without it, two dispatcher instances
reporting outcomes for the same endpoint at the same moment would both decide to
disable, and produce two disablements and two notifications.

Then the sabotage battery removed the lock and ran the suite. **Forty-six tests
passed.**

The claim was wrong. The `enabled = true` predicate is sufficient for
at-most-once on its own; the lock was being credited for the predicate's work.

<Why title="And the same sabotage now fails, which is the only proof the fix took">
Removing `.for("update")` from `applyFailureRun` today turns **one** test red out of
seventy-five: *"counts BOTH of two overlapping failures — the lost update the lock
prevents"*, with `expected 1 to be 2`. Re-run here rather than quoted, because a claim
that a test was added is worth nothing beside a demonstration that it goes red.

The two numbers say different things and both matter. Forty-six-of-forty-six passing was
the instrument reporting that a mechanism nobody had tested was being credited for work it
did not do. One-of-seventy-five failing is the same instrument reporting that the gap is
closed — and it is one test rather than several because the lock protects exactly one
property. A sabotage that turns ten tests red has usually broken something else.
</Why>

What the lock actually protects is the **counter**. Under `READ COMMITTED` both
transactions read `failure_run_attempts = 4`, both compute 5, and the second
`UPDATE` waits for the first and then overwrites it with 5. A lost update — the
run undercounts by one per collision. Nothing errors, nothing is logged, and the
endpoint quietly needs an extra failure to reach the floor of five. FR-WHK-07's
threshold becomes harder to reach than the requirement says, in a way no test
would ever have noticed.

There is now a test for the counter, and the comment says what the lock does.
This is what a sabotage battery is for: not confirming that the tests pass, but
finding out which mechanism is doing the work.

</Trap>

## The sweep rides a loop that already exists

The second trigger is one more statement per drain, in a worker that is already
awake, already owns a database connection, and already runs in the only service
permitted to write:
...
No new deployable, no cron, no scheduler. Constitution VII asks for the boring
option and here the boring option is also the only one that does not add a third
thing to deploy and monitor for the sake of a single query.

The sweep runs **after** the drain and only when nothing was due, so a backlog is
never made to wait behind housekeeping. An endpoint that has been failing for an
hour can wait another quarter-second; a customer's webhook cannot.
...
FR-WHK-07 asks for the endpoint to be disabled **and the organisation notified by
email**. This platform has no email transport of any kind. Not a misconfigured
one — none.
...
The quotas chapter needs the same transport for its warnings, which is why it is
not built here: building it now would mean building it for its second consumer
first, and guessing at what the first one needs.

</ForwardRef>

<Why title="Recorded, not sent — and why that is not a cop-out">

There is a real difference between "we did not do it" and "we did the part that
has to be durable, and left the part that has to be operational".

The obligation is the durable part. Which organisation is owed a message, about
which endpoint, for what window, with what evidence — all of that has to be
captured at the moment of the disablement, inside the same transaction, or it is
gone. A transport can be added afterwards and will find the queue waiting for it.

Doing it the other way round — sending an email at disable time with no record —
is the version that cannot be fixed later.

</Why>

## Proving it works again

Now the loop closes. A customer whose endpoint was disabled repairs their server.
What do they do next?

Without a test event, they re-enable and hope, and the first real event is the
experiment. If they were wrong, the endpoint accumulates another hour of failures
and is disabled a second time — which is how an endpoint gets disabled twice in a
day and a customer concludes the platform is broken.

FR-WHK-09 is a synthetic event, sent to one endpoint on demand, delivered through
the ordinary path with three deviations:
...
The route reports what the endpoint answered, and always answers `200` when the
test ran:
...
A customer's endpoint answering `503` is a **successful test**. It found out.
Returning `502` from this route would conflate "we could not run the test" with
"your endpoint is unhealthy", and at three in the morning the second is what
somebody reads.

The one case where `error` describes the platform rather than the customer is
when no attempt is reported at all — no dispatcher running. Even then the answer
is `200` with `delivered: false`, and the error says which side failed.

</Why>

**Delivered even when disabled.** This is the deviation that makes the loop
...
**Its outcome never touches the failure run.** A failed test must not push an
endpoint toward disablement — that would punish a customer for checking. A
successful test must not clear the run either, or a customer could mask a real
outage by testing until it passed. Both directions, which is why the exemption is
at the call and not inside it.

**Re-enabling clears all four columns**, so the hour is measured from the next
failure. Without that, a customer who fixed their server and switched it back on
would be disabled again by the first failure afterwards, on the strength of an
outage they had already repaired.

## Watching it happen

The walk from the webhook dispatcher chapter grew a flag. Start a permanently failing endpoint in
one terminal:
...
and in another, drive one event through the entire schedule and then past the
hour:
...
The endpoint's run is printed beside each attempt, and the disablement gets a
section of its own:
...
**Stop at the last two lines before the dead letter.** The delivery is `dead`.
The run holds seven failures. The endpoint is `enabled=true`.

That is research R1's quiet endpoint, reached by running rather than by
arithmetic. The schedule is exhausted, so no further outcome will ever be
reported for this endpoint. An outcome-only check has already run for the last
time.
...
Now run the same thing with `RELAY_DISABLE_SWEEP=off`, which is the version of
...
Seven failures, the run more than an hour old, the delivery dead-lettered, and
the endpoint enabled. Reading those two transcripts next to each other is the
only thing that makes the second trigger mean anything.

<Trap title="`--fast-forward` and `--watch-disable` move the clock, not the rule">

Both flags rewrite timestamps: one drags `next_attempt_at` into the past so the
schedule can be watched in seconds instead of two and a half hours, the other
ages `failure_run_started_at` by 64 minutes.

Neither changes a predicate. The rule that decides is the same
`shouldDisable` the tests exercise and production runs, reading the same columns.
A demonstration that skipped the logic instead of the waiting would be a
demonstration of nothing, and the distinction is worth insisting on because it is
easy to build the other kind by accident.
...
Seven attempts, seven events, **zero consumers**. That last number is the honest
state of FR-WHK-06 at the end of this chapter.
...
Here is one event, read off the stream the way Part 4's ingester eventually will:
...
Read the fields that are *not* there: no payload, no signing secret, no
signature, no URL, no header. Sizes, identifiers, statuses and durations, and
nothing else.
...
The dispatcher has a fourth outcome. When an endpoint has been disabled or
deleted since a delivery was scheduled, `deliver.ts` logs `delivery.skipped` and
moves on — and reports that one to nobody.

So no outcome is recorded and no attempt event is published, which is right
rather than a gap: a skip means no request was made, and an attempt record for an
attempt that never happened would put a row in a customer's dashboard describing
a request their server never received.

The first draft of this chapter's contract listed `skipped` as a fourth value.
Anyone who had written a consumer against it would have shipped a branch that can
never execute.
...
Three new suites and one much larger existing one. The attempt record, against a
real broker and a real api:
...
The failure run, the disablement, the sweep, and the two properties the sabotage
battery proved were untested:
...
And the test event, with this suite standing in for the dispatcher so it can
verify the signature the way a customer would:
...
`POST /test` creates a real delivery and then watches the row, because the
attempt happens in another process. Something has to make it.

Importing the dispatcher's build into the api's test lane would make this suite
fail whenever the two packages happened to build in the other order. Writing the
dispatcher's job out by hand costs thirty lines and buys the stronger test: the
signature is verified with `node:crypto` and the documented recipe, nothing from
the signing path. A test that verified with our own signing code would prove only
...
Seven mutations, each applied to the real code, each reverted afterwards with the
...
Seven out of seven, and **three of them did not work the first time**. That is
...
Mutations 5 and 7 compiled, ran, and **passed**. Both revealed real gaps: the
sweep's place in the loop was untested, and the row lock was being credited for
work the predicate was doing. Both produced a new test, and one produced a
correction to a comment that had been confidently wrong.

<Why title="What a battery is actually measuring">

Not whether the tests pass. Whether removing a mechanism makes them fail.

Those are different questions and only the second one tells you anything. A suite
where every test passes and no mutation is caught is a suite that holds nothing —
and you cannot tell the two apart by reading either the tests or the code, which
is why the check has to be mechanical.
...
The platform now records what happened on every delivery attempt, on a stream
built for analytical volume and explicitly allowed to lose one. It switches off
an endpoint that has been failing for more than an hour with at least five
attempts — from two triggers, because one of them provably never fires for the
customers this is most for. It writes down who has to be told and admits, in a
column, that it has no way to tell them. And it lets a customer prove their
repair worked before turning anything back on.

Two requirements are delivered in half on purpose. Attempts are published and not
queryable: Part 4's ingester is what finishes FR-WHK-06. The notification is
recorded and not sent: the rate-limit chapter builds the transport, for quotas as well.

</Checkpoint>

<Checkpoint title="Four files changed here without a fence, and three are instruments">

**`services/api/src/isolation/targets.ts` and `gauntlet.itest.ts`** gain one route each way:
`POST /v1/webhooks/:id/test` was named by the derivation on the build that registered it —
the fifth time in this repository — and it is the only route in that list whose damage
would land OUTSIDE the platform. A successful cross-tenant attack on it makes Relay POST to
another customer's server. Classified `write`, attacked beside the three sibling actions,
and worth one sentence rather than a diff.

**`services/api/src/consumer/consumer.itest.ts`** stopped sweeping consumers it did not
create. Its cleanup deleted by the prefix `itest-`, which was never this suite's to claim,
so it removed a live durable belonging to another suite mid-run — and the victim reported
`consumer deleted` or `consumer not found` for reasons nothing in its own file explained. A
prefix sweep deletes things it did not create; the prefix is now this suite's.

**`services/api/src/auth/credentials.itest.ts`** is the fourth, and it is the one worth
reading twice. Its credential-leak assertion derived the secret by splitting the credential
on `_` — which is in the base64url alphabet, so the needle was the fragment after the LAST
underscore. Measured over 200,000 mints: **26.9%** of credentials give a needle of twenty
characters or fewer, so a log line leaking the first thirty characters of a secret passed;
**1.57%** give one character or none, and `not.toContain` on a single character fails
against any haystack.

Both halves were found twice, independently, and the fixes agree. The published order hit
it here, on a mint ending `…_I` — the assertion had become *"no log line contains the letter
I"*, against an error body reading "this route expects an API key". This order hit it one
chapter earlier, in the coverage battery, on a mint ending `_0`. Two draws of the same
1.57%, and both arrived at the same parser and the same threshold.

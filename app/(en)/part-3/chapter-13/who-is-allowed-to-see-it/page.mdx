...
union with a strict schema and two states. `frames.test.ts` asserts its shape. The isolation
harness's direction gauntlet lists it among the frames a client may not forge, with a reason
written beside it. Twelve chapters of Part 3 have shipped a vocabulary word that nothing
speaks.
...
That comment has been accurate and idle since the socket chapter. The sharpest version is the
isolation harness's, which built a gauntlet asserting that every outbound frame type is refused
when a client utters it:
...
That row has passed since the isolation harness wrote it. It proves a client cannot claim a
presence transition, against a
...
chapter, **none** had a producer; the fan-out chapter gave `message.created` one, and this chapter is the
second. `message.updated`, `message.deleted`, `membership.changed` and `typing` were four declared
words with nothing behind them when this chapter shipped — the membership-revocation chapter gave the third a
producer and the typing chapter the fourth, leaving two. Naming them here was deliberate — the fan-out chapter's own spec
...
`session.ts` is fenced by eight chapters before this one — four in Part 2 and four in Part 3 —
and `deliver` is the function most of them are about. Widening it is not a local edit to a
fabric; it is an edit to something eight chapters have each taught a reader to build.
...
**The close handler now carries two ordering constraints where it carried none.** Presence is
told *after* `registry.remove`, because it asks whether this was the user's last connection on
this instance and must not count the one that is leaving. The unsubscribes come last.

The first of those is not a style change. With the removal after the presence call,
...
A user who **joins a channel while connected** stays invisible to that channel's members until
they reconnect. The subscribe set is taken once, at connect, from the session response. That is
FR-RTM-10's staleness wearing a different hat, and it is unfixed here on purpose — the membership-revocation chapter
fixes both hats with one mechanism.
...
The fan-out chapter hit this exactly: its publisher swallowed its own errors and resolved, so "the send
...
**Presence's Redis client will need a lint exemption, and this chapter cannot write it.** A rule
restricts `ioredis` to the files that hold rate-limit counters — and that rule arrives with the
limits chapter, nine chapters after this one. There is nothing here to be exempt from.

That is not a saving, and the reason is worth stating before somebody writes the exemption from
memory. **The fan-out chapter's entry does not transfer.** Its justification is *"this client
touches no keys"* — a publish onto `chan:{channel_id}`, a channel UUID, and a subject is not
readable at all. Presence's client touches keys, and they are environment-scoped:
`presence:{env}:{user}`, which is exactly the shape the restriction exists to guard.

The justification presence needs is the rate limiter's own: every key is composed from the
environment id on the authenticated connection's identity, nothing here takes an environment id
from a client, and there is no scan, `KEYS` or pattern read that could reach another tenant's
key. Somebody has to re-check that against the code as it then stands, which is why it is
recorded against the chapter that lands the rule rather than remembered.

**Copying the nearest neighbour's justification is how a restriction becomes a formality**, and a
debt is easier to pay wrong than a change.
...
non-member and a cross-tenant user each receive nothing in a run where a co-member receives.
    ...
    25 suites · 406 integration tests · green
    presence: 38 integration tests, 8 gateway unit tests, 6 protocol unit tests
    two instances, three channels: 12 SUBSCRIBE, 18 PUBLISH, 12 SET, 7 DEL

The last row was re-measured rather than carried: `CONFIG RESETSTAT`, the six tests of the
arrival scenario, `INFO commandstats`. **The four numbers came back identical to the ones the
chapter was first written with** — which is what you would expect of a count that is a property
of the protocol rather than of the lane, and is worth checking exactly because that expectation
is easy to hold without evidence.

It is worth one more look. Twelve connects produced twelve `SET … NX` and seven wins,
...
**FR-RTM-10 is still unmet, and the fan-out chapter said this chapter would fix it.** The premise there
...
— but it is a re-read the session layer owed, not a mechanism this chapter could have built.
**The membership-revocation chapter pays it**, and the staleness goes with it: subscribing to a channel's presence
subject is one of the three things that chapter's addition path does.
...
and chapter 2.6's other half of the promise this chapter half-broke. Whether typing can reuse
the fan-out's plumbing is the question that half of 2.6's sentence leaves open, and this
chapter closed only presence's half of it.

**The typing chapter answered it: no.** It re-derived the count in the paragraph above and found
seven typed points where this chapter says three — so typing takes a fourth subject grammar
too, and 2.6's promise is retired rather than kept.

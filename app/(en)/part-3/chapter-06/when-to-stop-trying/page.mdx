...
export const metadata = {
  ...
  description:
    "Chapter 3.5 built a platform that gives up on a delivery and never on an endpoint. This chapter adds the record of what happened and the decision to stop, in that order — and the order is the argument.",
  alternates: {
    canonical: "/part-3/chapter-06/when-to-stop-trying",
    languages: {
...
Here is what chapter 3.5's platform does about it. Each event produces a
...
3.5 rather than shipped: the decision was ready and the evidence was not.
...
Chapter 3.5 defined the internal contract the dispatcher uses to report what
...
So FR-001's hardest-sounding column, *how long did the customer take to answer*,
...
— is the cost 3.5 avoided by writing down everything the dispatcher had.
...
outcome — the outbox pattern chapter 3.3 built, which exists precisely to make
...
platform stamps it. That asymmetry is all of FR-009, and it is the difference
...
FR-008 rather than a check somebody has to remember:
...
whole of FR-008's concurrency story: without it, two dispatcher instances
...
endpoint quietly needs an extra failure to reach the floor of five. FR-007's
...
Chapter 3.7 needs the same transport for quota warnings, which is why it is not
built here: building it now would mean building it for its second consumer first,
and guessing at what the first one needs.
...
The walk from chapter 3.5 grew a flag. Start a permanently failing endpoint in
one terminal:
...
recorded and not sent: chapter 3.7 builds the transport, for quotas as well.

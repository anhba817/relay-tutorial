...
export const metadata = {
  ...
  description:
    "The outbox chapter left twelve thousand events in a stream that nobody had ever read. This chapter chooses every setting on that stream, builds the first consumer, and kills it in the gap between doing the work and admitting it.",
  alternates: {
...
Twelve thousand nine hundred and thirty events, published by the outbox chapter's relay
while the outbox chapter was being written, and **not one of them has ever been read**.
...
one hop further along, exactly the failure the outbox chapter existed to remove.
...
logs and is retired by the webhook dispatcher chapter. Delivery is at-least-once; handling is once *in
...
The outbox chapter created the stream in four lines, and said so at the time: the
...
Two settings can never be changed, and the outbox chapter happened to get both right. Had it
...
The outbox chapter's exists to prevent.
...
the webhook dispatcher the webhook dispatcher chapter, the analytics ingester (Part 4), and the
...
`events.msg.created.{env}` as its worked example. The outbox chapter implemented that
...
The outbox chapter's whole subject was the window between a committed message and a
...
The outbox chapter promised it in writing: "embraced, not mitigated."
...
`SIGKILL`, not a thrown exception — for the reason the outbox chapter gave and this
...
anticipated for the webhook dispatcher chapter's, and scope each test to its own environment's
subject.
...
The outbox chapter's rows: **a shared store needs a per-run handle, and a per-run handle
...
recorded `members` and the outbox chapter recorded the outbox's partial index.
...
learned that on idempotency keys and the tenancy chapter learned it again on signup, which is
...
So the claim and the effect share a fate, which is the outbox chapter's insight
...
left exactly where the outbox chapter found it. **The guarantee lives where the work happens.**
...
Three more, in the same file, hold the envelope schema against what the outbox chapter
publishes.
...
later chapter anyway, so giving this one a job would mean either stealing the webhook dispatcher chapter's
subject or inventing product nobody asked for.

<ForwardRef>The retirement is named rather than implied: the webhook dispatcher chapter replaces
...
FR-WHK-04 asks for a dead-letter queue with replay. That belongs to the webhook dispatcher chapter,
...
writer. So this one sits inside the api, exactly as the outbox chapter's relay does under
...
discovering it later: **the webhook dispatcher chapter's is meant to be its own
...
in the webhook dispatcher chapter, in the open.
...
`RELAY_EVENT_CONSUMER=off` exists for the same reason the outbox chapter added
...
**And the file that measures it has never been shown.** `vitest.coverage.config.mts` arrives
with the tooling rather than with a chapter — feature 024 built it, and no chapter teaches
it — which left the book in an odd position: **ten chapters after this one amend that file,
and an amendment needs something to amend.** A reader following along had a diff whose
pre-image did not exist for them, and the fence chain said so ten times over.

So here it is in full, once, at the commit where the repository gains it. Nothing below is
this chapter's work and none of it is this chapter's subject; the file's own header carries
the argument for why it exists, which is the same argument the constitution makes about
measuring a bar across both lanes rather than one package at a time.
...
The structural check went red again, and this is the useful part: not for the same reason
as last chapter.
...
The outbox holds a copy of tenant data and argues that nothing reads it on a tenant's
behalf. `consumed_events` holds an event id and a timestamp. There is nothing in a row
here to leak at all, which is a stronger answer and a different one — so it goes in the
spine with its own words rather than sharing the outbox's:
...
Two chapters, two tables, two arguments. A pattern matching `%_events` would have
absorbed both and taught neither.

## The suite that depended on another suite

Serialising the lane last chapter had a side effect nobody asked for: it made the file
order deterministic, and this suite came out first.
...
It had never applied the schema. It deleted from a table it assumed somebody else had
created — and while the order was arbitrary, somebody usually had.

<Trap>
An ordering dependency between test files is not something a test file can state. There
is no syntax for "run me after that one", which means a suite either sets up what it
needs or it is relying on luck it cannot describe. This one was relying on luck for two
chapters.
</Trap>
...
The same file also hardcoded the broker URL six times, and the outbox suite hardcoded it
once more — as `nats://localhost:14222`, where every other site in the repository said
`4222`. The production code has exported `DEFAULT_NATS_URL` since the last chapter; the
tests simply were not using it.
...
A default written down in seven places is seven chances to write it down differently, and
this one took the seventh.

## What this chapter leaves for later, on purpose

**Webhook delivery and the dead-letter path the webhook dispatcher chapter.** Signing, retry tiers,
...
**Ordering.** Still not promised, for the reason the outbox chapter gave: `data.seq` orders
...
chapter 2.8's journey and the outbox chapter's suite, unchanged.

...
The seal was demonstrated failing, one level at a time — and a fourth line was added
to the block after the three, because the block is LAST and a last block that sets
only its own patterns switches the driver ban off for the one package that most needs
it. `pg` resolves here by the ordinary parent walk even though `@relay/*` does not,
which is the same mechanism level 1 depends on:
...
That is the endpoints chapter's finding a second time, in code written by whoever
had just finished fixing the first instance. The block is now last in the file and carries
...
**THERE ARE THREE LANES, AND EACH IS TOLD IN A DIFFERENT LANGUAGE.** The unit lane
cannot see this package because it declares no `test` script. The integration lane is
told by a turbo filter in `package.json`. And `pnpm coverage` decides membership with
a glob over the filesystem — `packages/*/src/**/*.itest.ts` — so it found the suite
anyway and reported **one failed file and ten skipped tests, every run**, until
`packages/outsider/**` went into that config's `exclude` too.

A filter in a script says nothing about a glob in a config. **Ask of every lane,
separately, whether it can still see the thing you just told one lane to ignore.**
...
<Figure caption="Met in part. What is met was measured; what is missing is one thing, and it is the one no test can reach." code={figVerdict} />
...
call and gets the existing one, creates a **private** channel, adds two members who
did not exist, mints a token for one of them, creates a bot and has a send without
one refused with the field named, sends over REST and reads history back, sends over
REST and receives the event **on a socket**, receives two attachments in order, says
it is typing and is heard by a second member, holds five connections and is refused a
sixth with 4004, has a server-only frame refused by name, and confirms that a foreign
resource and an absent one answer identically. **Sixteen tests, all passing**, in CI
as its own job.

**AND THE GAP THIS MILESTONE WAS FAMOUS FOR IS CLOSED — BY THE REORDER, NOT BY THIS
CHAPTER.** The exercise's first run recorded that a REST-sent message reached no live
socket: an integration that sends over REST and waits on a socket could not succeed,
and no document said so. That was two faults wearing one symptom. The sender chapter
removed the first, so a public send attributes a sender and the row survives a
resume; the message-delivery chapter removed the second, which was the whole of what
remained — the api published to no fan-out at all.

In the published order both fixes came AFTER this exercise, so the milestone reported
the gap and scheduled it. Both are behind this chapter now. The test that used to say
"SENT over the socket" in capitals, because a REST send could not work, sends over
REST on purpose — **the send an integrating developer's backend actually makes.**

That is what a subject ordering buys, said as plainly as it can be: the same suite,
the same platform, and a verdict that improves because the chapter that measures a
gap no longer runs before the chapters that close it.

**What is not met.** One thing, and no test can reach it. **Content sufficiency is
not comprehensibility.** This chapter measured whether the documentation *contains*
what an integration needs. Whether a person reading it without help can build one is
a different question, and a person is the only instrument for it.
...
against the documentation, run it, and record every place the documentation ran out.
  ...
  5. **`POST /auth/dev-token` answers 200 and nothing documents it** — scheduled
     with the channel-control chapter
...
Part 3 ends here, and **two of the three things this chapter used to hand forward are
behind it.** The publish that makes a REST-sent message reach a socket is the
message-delivery chapter's and is done — the suite above sends over REST on purpose.
The public surface a customer drives — channel listing, unread counts, user profiles,
and the private channel type — is the channel-control chapter's and is done; this part
no longer refuses `type: "private"` at the door, which is why the sealed suite creates
one.

What it does leave behind is written down rather than implied:

- the outbox keeps message text for ever, and FR-MOD-06's chapter owns the prune
- `docs_url` names a host that does not resolve. The error-registry chapter opened
  that on its first page, called it "a debt with a due date somewhere in Part 4", and
  nothing since has moved the date — the count went from one code to two and then to
  every code in the registry
- hosted media, the queryable attempt log, and the audit log FR-MOD-03 asks for
- and the criterion's harder half: whether a person can read this and build an
  integration without help. Every instrument in both repositories compares bytes.

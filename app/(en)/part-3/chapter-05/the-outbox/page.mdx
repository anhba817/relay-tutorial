...
The platform can now tell who is asking and which tenant they belong to. What it
cannot do is tell anyone else that something happened.
...
This is the second such exception in Part 3, after the credentials chapter's key lookup, and both
...
accounting, it belongs to webhook delivery (FR-WHK-03, FR-WHK-06), and it is the
webhook dispatcher chapter's. This relay retries by *not marking a row done*, which
...
The isolation harness went red before any of this ran:
...
That is the harness working, on the first chapter to add a table since it was built. The
answer is the third option, and the reason is the one the schema comment already gives:
an outbox row is not a tenant's record, it is work the platform owes itself. The
environment travels inside `subject` and `payload`, so a consumer can filter, and no
request path joins this table on anybody's behalf.
...
<Trap>
That reason is about **reads**, and it does not cover retention. The payload is a full
copy of the message, `text` included, and the relay marks rows published rather than
deleting them — so a message deleted from `messages` still has its words in here, and
nothing on this platform removes them. It is a real gap and it is not a tenancy gap. It
is written into the spine entry rather than argued away, because the next person to read
that list deserves to know which half of the problem the entry answers.
</Trap>

Had the gauntlet been built at the end of this part instead of before it, this table
would have been classified nine chapters after it was written, by somebody reconstructing
the argument from the schema. Doing it here cost one list entry and a paragraph.

## The suites could not run alongside each other

The last chapter serialised turbo's tasks, because three of nine api suites were failing
with a Postgres catalogue error. That fix was necessary and it was not sufficient:
...
Turbo runs one package at a time now, but vitest still runs **files** in parallel within a
package. Every suite here applies migrations before it starts, so several of them issued
`CREATE TYPE` against the same schema at the same moment.

It could not have been found last chapter. With the schema already applied, every suite
finds nothing to do and the race has no window — this chapter's migration is what opened
one.
...
**The subject taxonomy, streams, and every consumer — the next chapter.** This chapter
...
**Webhook delivery, signing, retry tiers and dead-lettering — movement VI.** The reason
...
The tenancy chapter's signup suite asserted that a failed provisioning left the
*global* organisation count unchanged. That passed for two chapters and failed
here with `expected 884 to be 883`, because this chapter's crash tests spawn child
...
`services/api/dist` — the gateway's socket cases from the credentials chapter and this chapter's
...
latent since the credentials chapter and only surfaced when a second suite needed the same files.

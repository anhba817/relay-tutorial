...
string `v1:{timestamp}:{raw body}` — the **raw bytes as transmitted**, never a
re-serialisation of the parsed object. An event is expanded into one
...
the api — chapter 3.3's `SELECT … FOR UPDATE SKIP LOCKED` loop with one more
predicate — because a broker-held delay was **measured** to hold an
acknowledgement slot while it waits, which lets dead endpoints starve healthy
ones. Seven attempts (`now → 1s → 5s → 30s → 5min → 30min → 2h`), then a dead
letter retained for seven days. The dispatcher is a **new deployable** that
writes nothing to PostgreSQL and reaches state only over an internal HTTP seam.
The last hop is **at-least-once, stated**, with the event id handed to the
customer as the deduplication key. The chapter-end state is tag **`part3-ch5`**.
...
So the question changes. It is no longer *how do we avoid a gap* — the gap is
unavoidable — but **which way do we want to be wrong**:
...
<Figure caption="The gap that cannot be closed. What can be chosen is which side of it the crash falls on." chart={figThePatternStops} />

The platform takes the duplicate, and the reasoning is worth stating plainly
rather than leaving as a preference. A loss nobody can detect is worse than a
duplicate the recipient was handed the means to absorb. Chapter 3.3 spent an
...
That decision has an obligation attached, and it is the obligation that makes it
honest rather than convenient. **If you choose at-least-once, you must hand the
recipient the means to deduplicate.** The event id travels in the envelope,
documented, stable across every attempt of the same delivery. A customer who
stores it and ignores repeats is correct; a customer who does not has been told
...
Both of these are at-least-once, and it is worth being precise about where the
...
So this is envelope encryption rather than hashing — the same decision NFR-SEC-02
already anticipated with two branches, one for credentials that are compared and
one for secrets that are used:
...
Two details in there are decisions rather than mechanics.
...
The **key resolution** is the second, and it is smaller but it has bitten this
repository before. `encryptionKey()` reads the environment on every call rather
than at import time. A module that throws while being imported takes the whole
service down at startup over a feature most requests never touch, and — worse
for us — it makes the failure unreachable from a unit test, because the test
file cannot import the module it wants to assert on.
...
the discipline chapter 2.1 established after a generated migration was applied
without being read:
...
network — which is the definition of a server-side request forgery if nobody
...
**Five endpoints per environment.** Not because five is special, but because
"unbounded" is a number too, and it is the wrong one: expansion writes one row
per matching endpoint, so an environment with ten thousand endpoints turns one
event into ten thousand rows inside a single transaction.
...
The signature covers a canonical string, and the string is built from the **raw
body bytes** — the exact ones that go on the wire:
...
delay deliveries to another, and the broker-held delay makes that false by
construction.

<Why>

**The plan was re-done rather than adapted.** The measurement arrived after the
...
becomes a `next_attempt_at` column, and the thing that drains it is chapter
3.3's relay loop with one more predicate — a mechanism the reader already owns.
...
<Figure caption="The delivery row is the schedule. Nothing sleeps in the broker; a delivery that is not yet due is simply a row that no query has selected yet." chart={figTheSchedule} />
...
And the tier table lives in the **api**, not the dispatcher, which looks wrong
until you notice who reads it. The tiers are consulted by `recordAttemptOutcome`
when it computes the next `next_attempt_at` — and that is a database write, so
...
operator reading `duplicate=true` in a log line is seeing the mechanism work
rather than a problem to investigate.

<Why>

**Subscription filtering happens at expansion, not at delivery.** An endpoint
that does not subscribe to `message.created` gets no row at all, rather than a
...
<Figure caption="Three services, one writer. The dispatcher reaches state only over the internal seam, which is what makes constitution IV structural rather than a rule people remember." chart={figWhereItRuns} />
...
connection pools — something you reason about and hope. Across a process
boundary it is a claim about processes, and you settle it by not starting one:
...
What the split **costs** is worth stating with the same directness, because a
chapter that lists only the benefits of its own decision is advertising:
...
route.** If it were, a caller holding it could read any tenant's data through the
ordinary API, which is a cross-tenant hole with a valid credential in front of
it — the exact failure constitution I exists to prevent.
...
environment and returning early when it was absent — and CI never set it. For
several builds, the one assertion standing between a platform credential and a
public route did nothing at all, and reported success. A security test that
skips itself is worse than no test: it answers a question it never asked.
...
now be a platform one, and stopped compiling the moment the union changed —
which is the entire value of making the distinction a type rather than a
convention:
...
is *not* allowed to do rather than by what it wants. It cannot expand an event,
so it asks. It cannot read a secret, so it asks. It cannot record an outcome, so
it reports one:
...
dispatcher cannot send a shape the api does not parse, because the shape is one
...
`InternalModule` provides its own `"DB"` rather than importing one, and the
missing provider was worth a lost afternoon. `MessagesModule` *provides* `"DB"`
but does not *export* it, so a module that assumed it could reach it resolved
nothing — and Nest reports that as a runtime failure in whatever route touches
the database first, not as a wiring error at startup. The unit lane dropped from
156 tests to 40 and two whole suites failed before the cause was obvious.
...
instances draining the same table take different rows rather than fighting over
the same one — and `next_attempt_at <= now()` is the new half. A row whose time
has not come is simply not selected. It holds nothing, blocks nothing, and costs
nothing but an index entry.

One thing did have to change in 3.3's publisher, and it is a small correction to
an assumption rather than a new mechanism. The publisher ensured the `EVENTS`
...
structural rather than a tuning choice. FR-WHK-05 says one customer's endpoint
must not delay deliveries to another. Process a batch one message at a time and
that becomes false the moment a single endpoint hangs, because every delivery
behind it waits out a timeout it had nothing to do with. Grouping by endpoint
and running the groups concurrently makes the property hold by construction.
...
attempt it is retrying — and a customer whose server is already struggling is
the last one to hand extra concurrency.
...
The attempt itself is deliberately dull, which is the point — everything
interesting was decided before it ran:
...
will never succeed, and that the platform stops rather than retrying forever:
...
for seven days and it is **replayable** — and the replay reuses the original
event id, which is the detail that matters. A replay is the same event being
delivered again, not a new one, and a customer deduplicating on the event id
will treat it correctly without being told anything extra.
...
and the deferral is deliberate rather than an oversight. Auto-disable is a
policy decision with a customer-visible consequence — it stops delivering
webhooks to somebody who is paying for them — and it needs the attempt log to
justify itself to the customer afterwards. Shipping the mechanism without the
evidence would mean disabling endpoints and being unable to explain why.
...
unit ones are cheap and pin the arithmetic — the tier table, the encryption
round trip, the signature recipe:
...
And `repository.ts` went *down* — 86.30% branches to 78.22%, failing four
...
rather than run, and that turned out to expose something. Two services had been
running for eight chapters with no container image between them, because
`pnpm dev` was always enough. A third service made "how does this actually
ship" a question that could not be deferred any longer.
...
a framework by momentum rather than by decision:
...
The new package joins the build graph, which is a one-line change that would be
invisible until something did not rebuild:
...
`docker compose up -d --wait` still brings up four stores and nothing else — a

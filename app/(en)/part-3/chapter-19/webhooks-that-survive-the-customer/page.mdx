...
export const metadata = {
  ...
  description:
    "The broker chapter's handler logged a line and called it an effect. This chapter gives it a real one — an HTTP request to a machine we do not own — and watches the pattern that made the broker chapter correct stop applying twice over.",
  alternates: {
    canonical: "/part-3/chapter-19/webhooks-that-survive-the-customer",
    languages: {
      en: "/part-3/chapter-19/webhooks-that-survive-the-customer",
      vi: "/vi/part-3/chapter-19/webhooks-that-survive-the-customer",
    },
  },
};
...
Here is the broker chapter's handler, in full:
...
guarantee the broker chapter proved — claim the event, run the effect, commit both together, so
...
`webhook_deliveries` row per matching endpoint, written **inside** the broker chapter's
...
the api, which is the outbox chapter's `SELECT … FOR UPDATE SKIP LOCKED` loop with one
...
The broker chapter's correctness argument had one moving part. The claim and the effect
were in the same transaction, so there was no gap between them:

```ts title="services/api/src/consumer/runtime.ts (excerpt)"
// The broker chapter: the claim and the effect commit together or not at all.
...
duplicate the recipient was handed the means to absorb, and the outbox chapter spent an
...
guarantee changes. *Inside* the platform, the broker chapter's ledger made handling
...
The credentials chapter stored API keys as salted hashes and could afford to: a key is
...
**It is numbered 0010 and generation offered 0006**, which is worth a sentence before the
file rather than only inside it. `drizzle-kit` numbers from its own snapshot count; this
directory's snapshots stop at 0005 and five later migrations were hand-written and got
none. So generation proposed a number `0006_member_roles.sql` has held since the
channel-control chapter — and `migrate.ts` reads the DIRECTORY and sorts by filename, so a
0006 here would run before four migrations it must follow. The snapshot keeps its own
number because its lineage is right: it is 0005 plus these three tables, table for table,
which is checkable and was checked.

```sql title="services/api/migrations/0010_webhooks.sql"
...
The management surface is ordinary CRUD with four decisions in it:
...
**An event type outside the declared set is refused, and the set it is checked against
is not the set this platform emits.** Those are two different lists and the gap between
them is a customer's subscription. `WEBHOOK_EVENT_TYPES` declares the eight names
FR-WHK-02 names; five of them carry `emitted: true` and three — `channel.created`,
`user.connected`, `user.disconnected` — carry `false`, because they are published in the
specification and nothing produces one yet.

Checking a subscription against the five would refuse `user.connected` to a customer who
read the documentation and subscribed to a documented event. **They made no mistake, and
the published platform measured 741 stored rows naming one.** So the check runs against
the declared eight: a name outside them is a typo and earns `webhook_event_type_unknown`
with the accepted set spelled out in the message, while a declared name with no producer
is stored and simply never fires.

What that replaced is worse than a refusal. **A typo used to be stored.** The endpoint was
created, `201` came back, the customer saw their subscription listed — and nothing was ever
delivered to it, because no event would ever match `mesage.created`. A validation error a
customer can read is a better outcome than a subscription that looks correct for ever.

<Why title="Why a flag on the type rather than two lists">

The obvious shape is two constants — the declared names and the emitted ones — and it is
the shape that rots. Two lists agree because somebody remembers to change both, and this
repository has already paid for that: a lint exemption list and the tree it exempts drifted
apart until a test read the rule instead of restating it.

One table with a boolean per row cannot disagree with itself. Shipping a producer means
flipping `false` to `true` on the line that already names the event, and the compiler finds
every reader through `satisfies Record<string, { emitted: boolean }>` if the shape ever
changes.
...
expires. No new table, no new loop. The broker chapter already built the consumer.
...
becomes a `next_attempt_at` column, and what drains it is the outbox chapter's relay
...
+  boolean,
   ...
   pgTable,
   primaryKey,
@@ -576,6 +577,167 @@ export const consumedEvents = pgTable(
...
the expansion inside the claim's transaction, where the broker chapter's guarantee already
covers it:
...
@@ -301,12 +306,393 @@ export async function drainOutbox(
...
@@ -784,12 +1170,57 @@ export class UserBannedError extends Error {
...
@@ -807,12 +1238,203 @@ export class Repository {
...
The outbox chapter added the outbox relay and the broker chapter added the consumer runtime,
...
@@ -18,13 +18,47 @@ export interface ApplicationPrincipal {
...
+  | PlatformPrincipal;
  ...
  * exactly that way. */
@@ -38,13 +72,15 @@ export interface RequestWithPrincipal {
 }
...
+import { parseApiKeyCredential } from "./api-key";
 ...
 // session.itest.ts.
@@ -367,13 +368,37 @@ describe("credentials", () => {
       }
     } finally {
...
@@ -536,7 +561,34 @@ describe("credentials", () => {
...
controller written in the credentials chapter read `.environmentId` off a principal that can
...
@@ -188,12 +188,125 @@ export const internalSessionResponseSchema = z.strictObject({
...
database first, not as a wiring error at startup. On the run that found it the unit lane
dropped from 156 tests to 40 and two whole suites failed before the cause was obvious —
figures from that session, not from this tree, where the same lane is 361 tests and the
mistake is no longer there to make.

</Trap>

## The relay, again

The delivery relay is the outbox chapter's loop with one more predicate. That is the
whole of it, and the fact that it needed no new abstraction is the evidence that
The outbox chapter drew its seam in the right place:
...
`SELECT … FOR UPDATE SKIP LOCKED` does the same job it did in the outbox chapter: two api
...
One thing did have to change in the outbox chapter's publisher, and it is a corrected
assumption rather than a new mechanism. The publisher ensured the `EVENTS`
stream on connect, because in the outbox chapter that was the only stream there was. A second
...
@@ -160,12 +160,57 @@ export default defineConfig({
...
# The api. The repository's containers arrive with the dispatcher:
...
@@ -34,12 +35,13 @@ import { RequestContextMiddleware } from "./request-context.middleware";
     ChannelsModule,
     UsersModule,
     InternalModule,
     TenancyModule,
...
@@ -387,24 +387,34 @@ export async function boot({ gateways = 2 } = {}): Promise<System> {
       "RELAY_REDIS_PORT",
...
$ pnpm test && pnpm test:integration && pnpm coverage
 Tests  361 passed (361)     # 7 packages
 Tests  686 passed (686)     # 5 packages
 Tests  1038 passed (1038)   # 68 files, 338.6 s
```

**Those three numbers say nothing about this chapter.** They are the whole tree, and in
this order the webhook surface arrives behind eighteen chapters rather than four — so the
lanes were already large before a line of it was written. What the numbers are good for is
the shape of the gap between them: the unit lane is 361 tests and costs seconds, the
integration lane is 686 and costs minutes, and the coverage lane runs both in one process
and costs 338.6 seconds. A suite moves between those columns by needing a database, a
broker, or a second service — and this chapter added the first thing in the book that needs
all three.
...
<Checkpoint title="Fourteen files changed here without a fence, in two groups">

**SIX ARE THIS ORDER'S OWN WORK AND HAVE NO PUBLISHED COUNTERPART.** They exist because
this chapter arrives nineteenth rather than fifth, behind instruments that did not exist
when the webhook surface was written:

`services/api/src/isolation/targets.ts` gains eleven classifications and
`gauntlet.itest.ts` nine attacks, with `fixtures.ts` seeding an endpoint per tenant so a
foreign endpoint id exists to forge. The derivation named all eleven the moment the module
registered — seven `/v1/webhooks*` and the four `/internal/dispatch/*` beneath them — and
three of the four dispatch routes are filed `exempt` with a reason rather than `write` with
no attack, because they take one opaque delivery id and derive the tenant from the row.
There is no cross-environment request to make.

`packages/protocol/src/codes.ts` and `codes.test.ts` gain five refusal codes. That is a
defect this chapter shipped and could not see: all five of the service's 422s were bare,
and `ProtocolErrorFilter` derives a code from the status for 400, 401, 403 and 404 only, so
each one went out as `code: "internal_error"` with a correct status and a correct message
beside it. The names were already published in `docs/08-error-reference.md`; the registry
was behind.

`packages/test-harness/src/bound-port.test.ts` gains a declaration. It derives its list
from `services/*/src/main.ts` so a new service arrives without anyone remembering — and the
dispatcher arrived with no `listen`, no `createServer` and no `PORT`, because it is not a
server. Declared by name with its reason, and asserted in both directions.

**EIGHT ARE BUILD FILES, AND THE CHAPTER SHOWS THE ONE THAT CARRIES AN ARGUMENT.**
`services/api/Dockerfile` is fenced whole because the multi-stage build, the pruned
workspace and the non-root user are the decisions; `services/gateway/Dockerfile` and
`services/dispatcher/Dockerfile` differ from it in two lines each — the entrypoint and the
package name — and are described in prose above rather than repeated. `.dockerignore`, two
`tsconfig.build.json`, `services/dispatcher/tsconfig.json` and the dispatcher's vitest
config are configuration a reader copies once and never reads again.

`services/api/src/webhooks/webhooks.itest.ts` is the fourteenth, and the exception in that
group: it is 243 lines of assertions the chapter argues for at length and does not show.
The section above says what it holds. What was ADDED here is one test — the five refusals
by code, `docs_url` and field — and it is the assertion whose absence let the
`internal_error` body ship.

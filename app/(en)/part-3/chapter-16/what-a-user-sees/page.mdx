...
**145×, and the test lane could not have told you.** A million messages is not an unusual
number for a chat platform — it is a mid-sized customer's first year. The lane's 0.87 ms is
not wrong; it is a measurement of a database small enough that a sequential scan is cheaper
than an index lookup, which is true and useless.
...
earning its keep three years of chapters later.
...
The eleventh is `eslint.config.mjs`, and it is the one that taught the rule its limit: the
line carrying that citation is **authored inside `fences/post-series.md`**, as an added line
in an appendix hunk. So the appendix carries the correction and this chapter fences nothing —
and the classification that found the other 22 citations missed this one, because it searched
the platform repository and the line lives in the tutorial's.
...
options are a reader or a migration that drops it, and neither is this feature's work.
...
- `pnpm check:fences` — 0 problems; `pnpm check:figures` — 196 figures

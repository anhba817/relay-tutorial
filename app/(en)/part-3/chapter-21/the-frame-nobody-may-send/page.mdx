...
<ForwardRef>
Chapter 3.22 builds FR-RTM-09's five-connection cap, and its first job is a decision rather
than an implementation. The SRS describes `conn:{env}:{user}` as a Redis **set** with one TTL,
and a TTL is per key rather than per member — so one instance refreshing the key keeps a dead
instance's entry alive for ever. **What that chapter has to decide is how a per-member expiry
is expressed**, and whichever structure it picks, the argument it owes is why the members

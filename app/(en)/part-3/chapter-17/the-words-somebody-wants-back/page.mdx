...
Relay is already the first shape. Both alternatives add a per-channel counter, and this
chapter chose to write the bound down rather than add one — into ADR-07, whose loss argument
rests on *"the client's cursor + sequence-gap detection recovers anything missed"* and has now
been narrowed twice: once by the membership-revocation chapter for a payload with no
sequence, and once here for a payload whose sequence belongs to a message the client already
holds.
</Why>
...
@@ -147,14 +147,15 @@ describe("POST /v1/channels/:channelId/messages", () => {
...
@@ -311,12 +311,71 @@ describe("resume across a real fabric", () => {
     await settle(300);

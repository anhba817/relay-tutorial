...
Relay has answered none of them for twenty-two chapters — while carrying, the whole time,
...
`presence:{channel_id}` in 3.19, `member:{channel_id}` and `member:{env}:{user}` in 3.20,
`typing:{channel_id}` in 3.21.
...
`0008_message_edits.sql` — a number `0008_limit_policy.sql` has held since chapter 3.9 —
containing this table and then six whole `CREATE TABLE`s and fourteen `ALTER TABLE`s replayed
from migrations 0008 through 0013. Its snapshot sits at 0007 while the directory sits at 0013,
because those six were hand-written too.

Applied to any database that has run them, it fails on `CREATE TABLE "quota_notifications"`.
...
<Trap>
**"This is not a weakening" is a claim, so falsify it.** Two temporary tables against the live
database: one with no foreign key at all, and one whose only chain leads into the spine
(`→ organisations →` nothing). Both still fail totality, both named in the message. Reachability
reports the **direct** tables a chain arrives at, never the intermediate ones, so the invariant
the test already asserted — every entry in `via` is itself `direct` — holds unchanged.
</Trap>
...
been narrowed twice: once by chapter 3.20 for a payload with no sequence, and once here for a
payload whose sequence belongs to a message the client already holds.
</Why>
...
The coverage ratchet fired last, on `messages.controller.ts`, and its answer was to delete two
...
of the same two context fallbacks, one per publish site. Branches finished above where the
chapter found them, so the pin went up.
...
<Checkpoint>
The outsider suite is the only check in this repository that boots the shipped binary and
speaks to it the way a customer would — Node's global `WebSocket`, no workspace import. It
found chapter 3.21's worst defect: a module built, its `close()` awaited, and never passed to
`attachSessions`, inert past 1,174 coverage tests.

**No task in this chapter's plan wrote a test there**, and the audit task at the end lists that
file among the ones this chapter adds tests to. An audit was scheduled over work nobody did.
The test edits a message over REST and asserts a member's socket hears `message.updated` with
the original sequence, exactly once, and **no second `message.created`** — which is the whole
of ADR-24, checked against a real Redis and a real gateway process.
</Checkpoint>

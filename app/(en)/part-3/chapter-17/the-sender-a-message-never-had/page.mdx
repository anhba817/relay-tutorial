...
export const metadata = {
  ...
  description:
    "A message sent by a customer's server named nobody. The requirement to name somebody had been in the SRS since v1, and chapter 3.3 satisfied it by doing the opposite — which was the right call when nothing read the sender.",
  alternates: {
    canonical: "/part-3/chapter-17/the-sender-a-message-never-had",
    languages: {
...
Then chapter 3.15 made the sender decide whether a private channel is visible, chapter 3.16
...
The capability had been required for as long as the document existed. Chapter 3.3 satisfied it
...
platform mints is the one chapter 3.10 argued against, in a `<Trap>` this chapter has had to
correct: inventing a user inflates the dimension a customer is billed on. A **bot user** the
...
Every reader built since chapter 3.15 reads `users`. A separate table would have meant teaching
...
Postgres 11 promised and chapter 3.16 measured for `last_activity_at`. It is **not** true of the
...
The census predicted 27 call sites. The compiler named **28**.

The 28th was `messages.service.ts` — the only production caller. The prediction had counted
sites that *omit* `userId`; the service does not omit it, it passes `string | undefined`, which
is refused just as firmly. Three numbers described this file's send calls — 100 by naive grep,
46 HTTP sites in the blast-radius research, 27 omitting the property — and the true answer to
"what breaks" was a fourth.

<Figure caption="Two blast radii, and neither is the one that broke" code={figBlastRadius} />
...
check that existed only because a key send carried no user, and another around the usage insert
for the same reason. Both unreachable the moment the type changed. Fourth time this project has
met a guard that stopped meaning something — `addMember`'s `rowCount ?? 0` in 3.12, two in
`upsertUser` in 3.16.

The plan said to grep for the pattern rather than fix the two known instances. Good instinct.
The grep returns **seven**:
...
<Trap title="Sweeping the pattern would have deleted a capability chapter 3.15 shipped">

Line 3694 reads `if (channel.type === "private" && userId !== undefined)`. A key send skipped
the membership check because there was no user — and that is chapter 3.15's FR-005, delivered on
...
nowhere. On the mint, an unknown identifier answers **200 with a token** — chapter 3.16 made it
...
`usage_active_users` looked like one question about money. It is two questions, and the second
one refuses sends.

<Figure caption="One table, two readers, and only one of them says no" code={figTwoCounters} />

The insert records usage. A separate block compares a count against the configured ceiling and
throws. A bot inherits both by default — so a customer's own software would take the last slot,
and **their next human to post that period would be refused**. The harm lands on somebody who
did not cause it.

The answer is billed and exempt: the ceiling bounds a customer's human population, and their own
infrastructure should not be able to lock their people out. And the SRS had already separated the
two — FR-ANL-05 *meters* unique active users, FR-RTL-05 *enforces* a quota on them, two clauses
in two families. Only one of them is narrowed.

<Trap title="The exemption has two halves, and the second one decides whether it works">

Returning early so a bot is not refused is the visible half. The count the ceiling compares
against must **also** exclude bots — otherwise the bot's row still occupies a slot and a person
is still refused, and the bot's own send passing makes it look fixed.

The test for this is a *person* sending after a bot, never the bot itself. Its first version set
the ceiling to the number of people who had already sent, which meant the next person was over
the limit either way: it passed with half the fix applied, verified by removing the other half
and watching all 26 quota tests stay green. The arithmetic has to separate the two versions —
persons alone under the ceiling, persons plus the bot at it.

Sixth entry in this project's family of tests that were green while proving nothing, and the
first written by somebody who had read the other five.

</Trap>

One consequence, stated because it is a support call otherwise: the billed figure and the
enforced figure now differ by the number of bots. A tenant can see "5 of 5 active users" on a
usage page while their people still send.
...
Chapter 3.12 recorded that a REST-sent message reaches no socket, and listed two independent
...
turned out to be half closed. Chapter 3.18 is the other half.
...
Thirty-five files in the platform repository changed to make this true. Twenty-seven of them are
fenced above. The remaining eight are fenced by no chapter at all — including the README, which
is the file an outsider is told to read.
...
Chapter 3.10 carried a `<Trap>` titled *"An unattributed send counts toward the messages and
toward nobody"*, and its body argued against this chapter's decision on the grounds this chapter
decided the other way. It survived fifteen analysis passes of this feature. It is corrected now,
along with chapter 3.13's stated reason for a seam that no longer holds — both by a person
reading, because there is no instrument for it.

</Trap>

## The 19 files this chapter changes and does not teach
...
Chapter 3.16 met the same shape from the other side: eleven files it changed by exactly one
...
The lane cost per test did not move. 589 tests at a mean of 193.55 s against the previous
chapter's 550 tests at a mean of 193.55 s — the same figure to two decimals, because the lane
costs per *suite* and this chapter added no suite file. First time a lane-cost model here made a
forecast that held.

One run of twenty-six failed, in a mail-relay test this chapter did not touch, and the mechanism
is not identified. It is recorded as a gap with an owner rather than fixed, because fixing a
flake whose mechanism is unknown means changing code until the symptom stops.
...
chapter 3.12's gap this chapter did not close, and chapter 3.18 is the publish.

Presence is FR-CHN-05's third verb and it has never been built. The clause has three: read, send,
observe presence. Two of them held before this chapter and hold after it. Chapter 3.19 is the
third.

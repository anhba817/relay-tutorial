...
connection-minutes. The quota chapter built two of them and stopped, and the stopping
...
of them **forwards the end user's own token**. The credentials chapter made that so, and
...
So the gateway speaks for itself, for the first time. The webhook dispatcher
chapter already built the credential class this needs: `PlatformPrincipal`,
`@Accepts("platform")`, the `rk_svc_` prefix. The gateway becomes its second
holder.
...
header, trust it only for logging — is the pattern the credentials chapter spent
itself removing. "It's only for logs" is the sentence under which an asserted header
survives a review.
</Trap>

The credentials chapter's fenced comment says the gateway "holds no signing
secret", and that is still true after this chapter. A platform credential signs
nothing and verifies nothing; it says which service is talking, on the one call
that is the gateway's own rather than a user's.
...
The quota chapter refused the same shape for distinct users and bounded it by users
...
because a handler happened to arrive is the "declared, so use it" that the
rate-limit chapter refused by name.
</Why>
...
borrowing the rate-limit chapter's raw 429 at the same door. That shape had a stated
...
left is the shape the rate-limit chapter used for a **bad token**: complete the
handshake so that a close code has a socket to arrive on. EIR-WS-05 asks that of
an invalid credential; EIR-WS-06 asks the same of quota exhaustion.
...
<Trap>
The rate-limit chapter shipped a test called **"STILL emits close code 4008 from
nowhere"**, which greps the gateway's sources and asserts no match. Its comment
says why:
...
The api's half is the quota chapter's refusal, unchanged in shape and named by the
thrower:
...
gateway as `internal_error` — which is the finding the quota chapter's second analysis
...
after — so it writes its own threshold crossings, exactly as the quota chapter's send
...
The quota chapter wrote the price down, twice — in `0013_quotas.sql` and in
`quotas/config.ts`:

> the connection-metering chapter adds connection-minutes and FR-MED-12 later
> adds media bytes, and neither needs a table migration — a new dimension is a
> new key … the shape below is enforced by a CHECK that ENUMERATES the two
...
`environmentLimits`", because the quota chapter had refused to put a usage join inside
...
recovered 0.8 ms of it. The mechanism is the one the quota chapter already recorded: an
...
an instrument built specifically to avoid the quota chapter's uncontrolled benchmark.
...
has since the rate-limit chapter. Inherited deliberately and not compounded: this
chapter adds no third instance.

**The debt is already written down, and its due date is not in this part.** The
error-registry chapter opens on this exact envelope — three of its four fields are
true — and says `relay.example` does not resolve, that constitution V calls
`docs_url` a reachable-page promise, and that the fix belongs somewhere in Part 4.
Twenty-one chapters later the count has gone from one to two and the due date has
not moved.
</ForwardRef>
...
Everything this chapter changed, replayed onto the repository at
`rework/part3-ch23`. New files in full; everything else as a hunked diff against
the state the quota chapter left behind.
...
@@ -946,22 +946,71 @@ export const usagePeriods = pgTable(
...
+      .hard,
 ...
 export interface QuotaNotificationRow {
   id: string;
@@ -4717,42 +5036,27 @@ export class Repository {
     dimension: Dimension,
     before: number,
     after: number,
     caps: { hard: number | null; soft: number | null },
...
@@ -76,13 +79,42 @@ export class SessionController {
...
@@ -91,12 +123,12 @@ export class SessionController {
...
@@ -378,12 +378,57 @@ export type InternalDeliveryOutcomeRequest = z.infer<
...
@@ -392,6 +437,15 @@ export type InternalMembershipsResponse = z.infer<
...
@@ -54,12 +54,25 @@ export interface Connection {
...
@@ -41,27 +50,42 @@ export type Authentication =
...
@@ -899,12 +937,18 @@ export function attachSessions({
       buffer: [],
       overflowed: false,
...
@@ -1079,12 +1123,22 @@ export function attachSessions({
...
+      meter.closed(connection, new Date());
       ...
       //
@@ -1589,14 +1643,23 @@ export function attachSessions({
...
@@ -64,12 +64,16 @@ function stubApi(overrides: Partial<ApiClient> = {}): ApiClient {
...
@@ -213,13 +217,13 @@ async function boot(
   });
...
@@ -1133,34 +1137,48 @@ describe("the socket's limits", () => {
     });

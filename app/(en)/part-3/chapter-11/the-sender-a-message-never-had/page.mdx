...
+    if (patch.description !== undefined) set["description"] = patch.description;
     ...
     if (Object.keys(set).length > 0) {
       const updated = await this.db
         ...
         .set(set)
         .where(
@@ -1446,14 +1590,14 @@ export class Repository {
         ),
       )
       .returning({ id: users.id });
...
@@ -571,12 +579,117 @@ describe("a user's channel listing", () => {
   });
...
+    // What matters here is that a second add of a bot behaves exactly    // as a second add of a person: nothing about `kind` reaches the members table.
...
@@ -586,12 +699,207 @@ describe("a user's channel listing", () => {
...
@@ -737,13 +1045,17 @@ describe("a user's channel listing", () => {
       avatar_url: string | null;
       metadata: Record<string, unknown>;
     };

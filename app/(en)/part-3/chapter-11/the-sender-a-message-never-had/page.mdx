...
describe — and there are 121,250 of them in the test lane, out of 394,808.
...
phrase "on behalf of", and for eleven chapters that was enough.
...
"No backfill needed" is true of the columns — 4.7 ms against 134,067 rows is metadata-only, as
Postgres 11 promised and the user-surface chapter measured for `last_activity_at`. It is **not** true of the
CHECKs: each validates every existing row. At this size that is nothing; the shape is O(n), and
"the migration needs no backfill" is the sentence somebody will quote.
...
<Figure caption="The five refusals, what each reveals, and why sender resolution cannot be fourth" code={figRefusalOrder} />
...
not otherwise reach. The two are no longer adjacent — one is first and one is last:
...
121,250 messages in this lane have no sender, across 10,077 environments. The chapter's rule is
...
The user-surface chapter met the same shape from the other side: eleven files it changed by exactly one
word, which gave it eleven **fences with no subject**. These are the inverse — subjects a
reader does not need, on paths the chain still has to account for.

...
The query schema caps `limit` at FR-MSG-09's 200 and *refuses* anything
above it — `.max(200)` on a zod number is a validation, not a transform, so
a client asking for 500 gets a 400 naming the field:
...
One more shape decision earns a sentence: the page cap **refuses** rather
than clamps. A client asking for `limit=500` gets a 400 with `field:
"limit"`, not a page of 200 — because a clamp would answer a question the
caller did not ask, and the number they get back would silently disagree
with the number they sent. A caller who meant 500 has a bug, and a refusal
naming the field is how they find it in one request rather than in a
paging loop that never terminates the way they expect.

This paragraph said the opposite for two parts. It argued for a clamp,
described a clamp, and sat above code that has always refused — `.max(200)`
is a validation. Chapter 4.8 mirrored this schema for a new surface, ran
the bounds, and found the words wrong rather than the code.

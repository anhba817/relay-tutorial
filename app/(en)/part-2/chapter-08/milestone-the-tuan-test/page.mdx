...
**The port numbers will not be those ones.** The harness binds port 0 and lets the operating
system assign, then reads the assignment out of each child's own `listening` line. It used to
take a fixed 4100 and count upward, which put three ports inside a range another suite's map
registers to itself — and, worse, made a teardown that did not wait for its children into a
collision. What matters in the transcript is the order of the lines, not the numbers.

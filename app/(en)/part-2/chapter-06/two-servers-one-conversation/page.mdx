...
it entered. And presence (FR-RTM-06) and typing (FR-RTM-08) both still
need a fabric of some kind — ADR-10 puts presence in Redis with TTLs, and
the natural assumption is that it rides the very subjects this chapter
built. Chapter 3.19 finds it cannot: `deliver` is typed to messages, ten
chapters fence it, and presence ends up with a subject grammar of its own
*beside* this one rather than inside it. Same Redis, same reference
counting, same shape of module — a different subject. Typing's half of

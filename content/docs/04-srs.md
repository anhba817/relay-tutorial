...
| FR-RTL-05 | The system shall enforce configurable quotas per environment on four quantities. **Three are monthly flows** — messages sent, unique active persons, and connection-minutes — accumulated in `usage_periods` against a calendar month and reset when it turns. **Stored media bytes are a level, not a flow**: the figure is the sum of the tenant's live media objects, it falls when objects are deleted, and it does not reset on the first of the month. A storage quota shall therefore be enforced against that sum and shall not promise a resume date. | P3 | T |
...
| FR-MED-04 | Every uploaded object shall be virus-scanned and content-probed (dimensions for images; duration for audio/video) asynchronously before transitioning `pending → ready`. Objects failing the scan shall transition to `rejected` and be deleted, retaining only the audit record. **PARTLY MET AS OF CHAPTER 4.13, BY DECISION AND NOT BY OVERSIGHT** — the scan, the declaration check and the image dimensions ship; **duration does not**. A PNG's dimensions are 24 bytes of a 4,722-byte file and the other three image formats are fixed offsets or a short segment walk, where duration is four unrelated container parsers with MP3 variable bitrate as a genuinely hard case: a VBR file's length is not in its header. What a client gives up is a scrubber length before playback. *(Revision 1.20.)* | P3 | T |
...
| ID | Requirement | Pri | Ver |
|---|---|---|---|
| FR-MED-06 | A message may attach a `media_id` in state `pending` or `ready`, provided the media object belongs to the same environment. **For a user token the object must additionally have been uploaded by the sending user, or by the tenant itself** — an object whose uploader is unrecorded was taken by an application credential acting for the environment, and a user of that environment may attach it. For an application credential every object of the environment is attachable. Attaching another tenant's or another user's media shall fail, and **all three refusals — another tenant's object, another user's, and an id no object has — shall be indistinguishable to the caller**, because a refusal that named the cause would report whether somebody else's object exists. *(Revision 1.18: the uploader clause said only "was uploaded by the sending user", which withheld permission for an unrecorded uploader and mandated refusal for neither case — leaving the commonest server-side shape undefined.)* | P3 | T |
| FR-MED-07 | Real-time and history delivery shall include each attachment's state. **UNMET AS OF CHAPTER 4.11, BY DECISION AND NOT BY OVERSIGHT** — the clause cost nothing while no attachment had a state, and that chapter delivers the first ones that do while asserting the state is absent. Movement VI's verification chapter is where it is met; until then no attachment carries a state on any door, and nothing in the schema can hold one but `pending`. *(Revision 1.18.)* **AND CHAPTER 4.13 MAKES IT REACHABLE WITHOUT MEETING IT**: migration `0018` widens the constraint to three states and the media worker produces the other two, so an attachment now HAS a state that a delivery could carry — and this clause asks for a `media.updated` EVENT, which is a producer no chapter has written. It moves from *vacuous* to *unmet with a reachable subject*, which is a different sentence and a smaller gap. *(Revision 1.20.)* A `media.updated` event shall be delivered on the referencing channels when a pending attachment becomes `ready` or `rejected`, so clients can replace placeholders without polling. | P3 | T |
| FR-MED-08 | Media objects shall be readable only via signed delivery URLs with a validity of 1 hour, issued only to callers authorised to read a referencing message. *Authorised to read* is the platform's own channel-visibility rule, not membership: a user token may read any message of a public channel of its environment and a private channel's messages only as a member, and an application credential may read any message of its environment. Where an object is referenced by more than one message, the caller is authorised if **any** referencing message sits in a channel it may read. An object with **no** referencing message is readable by nobody, including the credential that uploaded it. Object storage shall not be publicly readable. | P3 | T |
...
| FR-MED-12 | The system shall meter stored media bytes per tenant per day and count uploads by kind, enforced as FR-RTL-05's storage quota — **against the current sum, not against a monthly total** — and visible in the dashboard alongside other usage (FR-DSH-04/05). The daily figure is a time series for the dashboard; the quota reads the level. | P3 | T |
...
> inherits channel visibility rather than inventing a parallel ACL system. *Inherits* is
> load-bearing and was measured against the platform at chapter 4.12: reading the clause's
> earlier wording literally, as a membership check, would have refused a user the photo in a
> message whose text they can read — 11,557 public channels against 1,016 private on the
> development lane, so the common case is a channel where membership is not the rule. The
> clause is satisfied by construction only if no second predicate is written, which is why the
> delivery route calls the same `channelVisibleTo` every read route calls. The same reasoning
> settles the unreferenced object: granting its uploader access would be a second authorisation
> rule that does not follow a message, and FR-MED-10 hard-deletes such objects after 24 hours,
> so it would be a read path to a thing already scheduled for destruction.
...
| 1.17 | 2026-09-19 | — | **FR-RTL-05 amended to say which kind of quantity each of its quotas is, and FR-MED-12 re-read against it** (feature 056, chapter 4.10). The clause read *"configurable monthly quotas on messages sent, unique active persons, and connection-minutes"* — three quantities, none of them storage — while **two other clauses cite it for a fourth**: FR-MED-02 refuses an upload on *"the environment's storage quota"* and FR-MED-12 says stored bytes are *"included in quota enforcement (FR-RTL-05)"*. Two clauses citing a third for something it does not define. **AND THE MISSING WORD IS `MONTHLY`, WHICH IS THE PART THAT WOULD HAVE BEEN IMPLEMENTED WRONG.** The three existing dimensions are flows: `usage_periods` is keyed on a calendar month and `creditFor`'s own comment says *"the one thing this function must never do is subtract from a bill"*. Stored bytes are a **level** — the figure falls when objects are deleted — so putting it in that row would need a subtraction the function forbids, and would reset on the 1st: **a tenant holding 100 GB would start every month at zero and be allowed another 100**. The cap is configuration in `environments.quota_config` beside the other three; the accounting is `sum(declared_bytes)` over the tenant's media rows. The clause now says which is which, and says that a storage refusal must not promise a resume date — the platform's other three do, in the sentence `QuotaExceededError.publicMessage()` builds, and for this dimension that date would be false. `media_storage_exhausted` exists so the refusal never reaches that sentence. **AND THE READ AND THE WRITE NEEDED A LOCK, WHICH A TRANSACTION IS NOT.** Postgres defaults to READ COMMITTED, so two slot requests inside two transactions both read the same sum, both find room, and both insert — measured by hand with two clients: **1,200 bytes committed against a cap of 1,000**. `SELECT ... FOR UPDATE` on the environment row serialises them per tenant, at no extra statement because the cap is on that row. **And the obvious probe for it measured the wrong lock**: `media_objects` has a foreign key to `environments`, so the INSERT takes `FOR KEY SHARE` on the parent and blocks against a `FOR UPDATE` holder whether or not the method asked for anything. A holder at `FOR NO KEY UPDATE` — which conflicts with `FOR UPDATE` and not with `FOR KEY SHARE` — is the discriminator, and the test goes red without the explicit lock only under that mode. |
| 1.18 | 2026-09-19 | — | **FR-MED-06 gains the case it left undefined, and FR-MED-07 is recorded as unmet by decision** (feature 057, chapter 4.11). The uploader clause read *"(for user tokens) was uploaded by the sending user"*; a slot taken by an application credential records no uploader, which the first sentence withheld permission for and the second did not mandate refusal for — **the clause was silent, not restrictive**, on the commonest shape a customer's backend produces. Chapter 4.10 made that column nullable expressly so this chapter could ask the question, and answering "an unrecorded uploader always fails" would have made the nullability pointless because any sentinel would have done. The clause now states three cases and requires the three refusals to be indistinguishable, which was built and tested as one property rather than three assertions that agree. FR-MED-07 is the adjacent row and this chapter turns it from vacuous to unmet: it is the first to deliver an attachment that HAS a state. |
| 1.19 | 2026-09-20 | — | **FR-MED-08's authorisation rule replaced with the platform's own, and two cases it never named written down** (feature 058, chapter 4.12). The clause said *"authorised to read the referencing message (channel membership or API key)"*, and **the parenthesis is stricter than the message it guards.** This platform checks membership for `private` channels only — `repository.ts:5589` has answered history that way since the channel chapter — so a literal implementation would refuse a user the photo in a message whose text they can read. Measured on the development lane: **11,557 public channels against 1,016 private**, so the case the literal reading breaks is the common one, not an edge. The clause now names the platform's rule and the note's *"inherits channel membership"* becomes *"inherits channel visibility"*, which is what makes the no-parallel-ACL sentence satisfiable by construction: the delivery route calls the same `channelVisibleTo` every read route calls, and the *"or API key"* arm is that helper's own `userId === undefined` branch rather than a second path. **AND THE SINGULAR WAS WRONG.** *"The referencing message"* does not describe a platform where FR-MSG-11 has allowed the same id twice since chapter 3.24 — forwarding a photo is the ordinary way one object acquires a second reference — so authorisation is a disjunction over every referencing channel. A lookup that stopped at the first reference would refuse a caller whose channel happened to be second, which presents as flakiness rather than as a refusal; the test that catches it fails with `expected 404 to be 200` when the lookup is truncated. **AND THE OBJECT WITH NO REFERENCING MESSAGE IS NOW ANSWERED.** The clause withheld permission and mandated nothing, exactly as FR-MED-06 did before 1.18. It is readable by nobody, including its uploader: the permissive reading is the parallel ACL the clause's own note forbids, and FR-MED-10 destroys unreferenced objects after 24 hours, so it would be a read path to a thing already scheduled for deletion. **What the clause still cannot promise is revocation.** A URL issued at minute 0 to a caller removed from the channel at minute 1 works until minute 60; the store checks a signature and has never heard of a channel, and standing the api in front of the bytes is what ADR-13 was decided against. Published as a cost with the window named rather than hidden (`gaps.md` 058-1). |
| 1.20 | 2026-09-26 | — | **FR-MED-04 recorded partly met, FR-MED-07 re-read now that a state can exist, and FR-MED-03's *"contradict their declaration"* settled as EXACT** (feature 059, chapter 4.13). The third of those is the one that changed an implementation. FR-MED-03 says objects that *"contradict their declaration"* shall be rejected, and this feature's own specification and tasks both read that as *"materially larger"* without giving a tolerance — **and the quota is what settles it**. `reserveMediaSlot` charges `sum(declared_bytes)` (revision 1.17), so under any tolerance a client that under-declares is billed for the declaration and stores the difference: the tolerance would be a storage discount with no clause behind it. Exact comparison makes the quota correct by construction — for every `ready` object, `verified_bytes = declared_bytes`, asserted over the tenant's whole set rather than over one row. The tests are **one byte**, in both directions, because a five-thousand-byte mismatch is rejected under a tolerance too. **AND THE TYPE HALF CANNOT BE ASKED OF THE STORE.** A presigned PUT of twelve MP4 bytes sent with `content-type: image/png` answers `HEAD` with `image/png` — measured — so the store echoes the client's own claim and a worker reading that header has verified the declaration against itself. `content-length` is the store's own count and is evidence; `content-type` is not. The type comes from the bytes, for all ten types FR-MED-02 allows. **AND THE SCAN RUNS FIRST, UNCONDITIONALLY**, because FR-MED-04's *"every uploaded object"* cannot be read one way for the type check and another way for the size check — an object declaring one byte while holding five megabytes is at least as worth scanning as one whose type is wrong. **AND A SCANNER THAT CANNOT ANSWER PRODUCES NO VERDICT AT ALL** (FR-009): not `rejected`, which would destroy a customer's bytes to record an outage, and not `ready`, which would let an unscanned object through. There is deliberately no `retry` verdict in the contract, because a row saying *"we could not tell"* is one somebody later reads as a fact. |

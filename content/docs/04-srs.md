...
| DR-09 | Raw events shall be retained for 90 days; daily aggregates for 25 months. **The second half went unimplemented for three features and was filed as absent twice** — `gaps.md` 047-3 and 048-4 both read *"no clause says how long metering history is kept"* while quoting DR-10, one row below this one. Chapter 4.6 applied it: `TTL toDateTime(day) + INTERVAL 25 MONTH` on both rollups. It does not contradict FR-003a's *"metering must not lose history when raw events do"* — 25 months against 90 days outlives the source roughly eightfold, and *"not the raw table's TTL"* was all that decision ever asked for. |

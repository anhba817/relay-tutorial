...
| FR-MED-07 | Real-time and history delivery shall include each attachment's state. **UNMET AS OF CHAPTER 4.11, BY DECISION AND NOT BY OVERSIGHT** — the clause cost nothing while no attachment had a state, and that chapter delivers the first ones that do while asserting the state is absent. Movement VI's verification chapter is where it is met; until then no attachment carries a state on any door, and nothing in the schema can hold one but `pending`. *(Revision 1.18.)* A `media.updated` event shall be delivered on the referencing channels when a pending attachment becomes `ready` or `rejected`, so clients can replace placeholders without polling. | P3 | T |
